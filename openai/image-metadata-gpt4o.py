"""
Image Metadata Gpt4O Pipeline

This module provides functionality for image metadata gpt4o pipeline.
...
def get_image_tech_meta(image_path: Path) -> Dict[str, Optional[Any]]:
    ...
    try:
        with Image.open(image_path) as im:
            ...
            created_date = datetime.fromtimestamp(created_ts).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            return {
...
def main():
    ...
    input_path = Path(
        input("📂 Enter the full path to the folder containing image files: ").strip()
    )
    ...
    if not image_paths:
        logger.info(
            "⚠️ No image files found in the specified directory or its subdirectories."
        )
        return
