...
This module provides functionality for research assistant.
...
class ResearchAssistant:
    ...
    def analyze_with_ai(
        ...
        if memories:
            memory_text = Path("\n").join(
                [f"- {m.get('memory', m)}" for m in memories[:3]]
            )
...
Provide a comprehensive, well-structured answer."""
    ...
    def research(
        self, question: str, use_claude: bool = False, skip_web: bool = False
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    if len(sys.argv) < 2:
        logger.info(
            "Usage: python3 research_assistant.py <question> [--user USER_ID] [--claude] [--history]"
        )
        logger.info("\nExamples:")
        logger.info(
            "  python3 research_assistant.py 'What are the latest AI breakthroughs?'"
        ...
        logger.info(
            "  python3 research_assistant.py 'Explain quantum computing' --claude"
        )
