    ...
    def finishedDownloading(self, newscriptwrapper):
        if not len(newscriptwrapper.scriptMap) == 0:
            ...
            if self.clipEditorWindow is None:
                ...
                self.clipEditorWindow.videoWrapper.scriptWrapper.addScriptWrapper(
                    newscriptwrapper
                )
...
class clipEditor(QMainWindow):
    ...
    def updateDisplay(self):
        ...
        if len(mp4file.split("/")) > 2:
            ...
            self.mediaPlayer.setMedia(
                QMediaContent(
                    QUrl.fromLocalFile(f"{current_path}/TempClips/{mp4file}.mp4")
                )
            )
        self.mediaPlayer.setVolume(audio * CONSTANT_100)
    ...
    def incrimentSelection(self):
        if (
            not self.mainCommentIndex + 1
            > self.videoWrapper.scriptWrapper.getCommentAmount() - 1
        ):
    ...
    def introFileDialog(self):
        ...
        if fileName:
            try:
                ...
                if width != int(CONSTANT_1920) or height != int(CONSTANT_1080):
                    self.uploadFail(
    ...
    def outroFileDialog(self):
        ...
        if fileName:
            try:
                ...
                height = vid.get(cv2.CAP_PROP_FRAME_HEIGHT)
                width = vid.get(cv2.CAP_PROP_FRAME_WIDTH)
                if int(width) != CONSTANT_1920 or int(height) != CONSTANT_1080:
                    self.uploadFail(
                        "Incorrect resolution for file %s.\n Resolution was %sx%s, required 1920x1080"
                        % (fileName, width, height)
                    )
        ...
        if fileName:
            ...
            try:
                ...
                if int(width) != CONSTANT_1920 or int(height) != CONSTANT_1080:
                    ...
                    new_name = (self.firstClipPath.split("/")[name - 1]).replace(
                        ".mp4", ""
                    )
                    ...
                    duration = media_info.tracks[0].duration / CONSTANT_1000
    ...
    def exportVideo(self):
        ...
        if (
            ...
            if settings.enforceIntro:
                ...
                duration_intro = media_info_intro.tracks[0].duration / CONSTANT_1000
            ...
            if settings.enforceInterval:
                ...
                duration_interval = media_info_interval.tracks[0].duration / CONSTANT_1000
            ...
            if settings.enforceOutro:
                ...
                duration_outro = media_info_outro.tracks[0].duration / CONSTANT_1000
    ...
    def videoExportConfirmation(self):
        ...
        if buttonReply == QMessageBox.Yes:
            ...
            if not firstClipCheck:
                msg += (
                    "No first clip selected, but first clip expected (see config.ini)\n"
                )
