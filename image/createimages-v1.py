...
This module provides functionality for createimages.
...
def generate_description(detail):
    ...
    prompt = (
        f"Generate a compelling description for a T-shirt with the theme: '{detail}'. Maximum CONSTANT_150 characters."
    )
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": prompt}],
...
for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    data = {"width": CONSTANT_512, "height": CONSTANT_512, "text_prompts": [{"text": image_prompt, "weight": 0.5}]}
