...
class EnhancedGalleryBuilder:
    ...
    def load_images_data(self) -> None:
        ...
        images_data_path = self.gallery_config.get("images_data_file", "images_data.json")
        try:
    ...
    def build_all_layouts(self) -> None:
        ...
        for layout_name in self.layouts.keys():
            try:
                ...
                logger.info(f"   Description: {self.layouts[layout_name]['description']}")
        ...
        for layout_name, config in self.layouts.items():
            logger.info(f"   • public/index_{layout_name}.html - {config['description']}")
...
def main():
    ...
    parser.add_argument("--switcher", action="store_true", help="Create layout switcher")
