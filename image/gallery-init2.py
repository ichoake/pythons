...
This module provides functionality for gallery init2.
...
def parse_args():
    ...
    For detailed documentation please refer to https://github.com/iChoake/simple-photo-gallery"""
    ...
    for path in glob.glob(os.path.join(image_source, "*")):
        ...
        if (
            ...
            if only_copy:
                shutil.copy(
                    path, os.path.join(
                        photos_dir, os.path.basename(path)))
            else:
                shutil.move(
                    path, os.path.join(
                        photos_dir, os.path.basename(path)))
...
def create_gallery_json(gallery_root, remote_link, use_defaults=False):
    ...
    # If defaults are not used, ask the user to provide input to some
    # important settings
    if not use_defaults:
...
#        )
        ...
        # f'What is your site URL? This is only needed to better show links to
        # your galleries on social media (default: "")\n')
...
def main():
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while creating the gallery.json file: {
                str(exception)}")
        sys.exit(1)
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while generating the gallery structure: {
                str(exception)}")
        sys.exit(1)
