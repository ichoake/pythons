...
def extract_song_details(file_paths):
    ...
    for file_path in file_paths:
        ...
        for item in soup.find_all("div", class_="css-79jxux"):  # Adjust class names if needed
            ...
            for match in matches:
                song_details.append(
                    {
                        "Song Title": (match[1] if match[1] else (title_element["title"] if title_element else "")),
                        "Song URL": (
                            f"http://suno.com{match[2]}"
                            if match[2]
                            else (f"http://suno.com{song_url_element['href']}" if song_url_element else "")
                        ),
                        "Cover Url": (
                            match[0] if match[0] else (cover_url_element["src"] if cover_url_element else "")
                        ),
                        "Genre": (match[4] if match[4] else (genre_element.text.strip() if genre_element else "")),
