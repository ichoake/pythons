...
class ImprovementMover:
    ...
    def _create_backup(self) -> None:
        ...
        backup_dir = self.python_dir.parent / f"python_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        for file_name in self.improved_files:
            ...
            if source.exists():
                ...
                if "upscaler" in file_name or "image" in file_name:
                    destination = self.python_dir / "02_media_processing" / "image_tools" / file_name
                ...
                elif "quality" in file_name or "analyzer" in file_name or "improver" in file_name:
    ...
    def _generate_markdown_summary(self, summary: Dict) -> None:
        ...
        with open(md_file, "w") as f:
            ...
            f.write("## Overview\n\n")
            f.write("All improved files have been moved back to the original Python directory structure.\n\n")

            f.write("## Files Moved\n\n")
            move_data = summary["move_summary"]
            f.write(f"- **Improved Files:** {move_data['improved_files_moved']} files\n")
            f.write(f"- **Documentation Files:** {move_data['documentation_files_moved']} files\n")
            f.write(f"- **Configuration Files:** {move_data['configuration_files_moved']} files\n")
            ...
            f.write("python 00_shared_libraries/batch_quality_improver.py . --batch-size 25\n\n")
...
def main():
    """Main function."""
    if len(sys.argv) != 3:
        logger.info("Usage: python move_improvements_back.py <python_directory> <copied_files_directory>")
        sys.exit(1)
