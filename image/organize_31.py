...
base_dir = Path("/Users/steven/Music/nocTurneMeLoDieS/MP3")
...
def organize_files():
    ...
    for file in files:
        ...
        mp3_path = os.path.join(album_folder, f"{album_name}.m4a")
        ...
        if file.endswith(".mp3") and not os.path.exists(mp3_path):
            ...
            if file.endswith(".m4a") and not os.path.exists(m4a_path):
            shutil.move(file_path, m4a_path)
            logger.info(f"Moved: {file} to {m4a_path}")
