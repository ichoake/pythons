...
This module provides functionality for inference.
...
def process_image(image, colorizator, args):
    ...
    """colorize_single_image function."""
...
def colorize_single_image(image_path, save_path, colorizator, args):
        ...
        if args.superr:
            ...
            plt.imsave(save_path, colorization,[int(cv2.IMWRITE_WEBP_QUALITY),CONSTANT_100])
        #
        return True
    """colorize_images function."""
    ...
    for image_name in images:
        if os.path.splitext(image_name)[1].lower() in ('.jpg', '.png', '.jpeg', '.webp'):
            ...
            if is_grayscale(file_path):
                #continue
                #logger.info("gray img!: "+str(img_path))
                pass
            else:
                plt.imsave(save_path,plt.imread(file_path))
                logger.info("color img!: "+str(file_path)+"  COPY!")
                continue			
...
def parse_args():
    ...
    parser.add_argument("-s", "--size", type = int, default = CONSTANT_576, help='Color output size' )
    parser.add_argument('-ct', '--colortile', type=int, default=0, help='Color Tile size, 0 for no tile')
    parser.add_argument('-st', '--srtile', type=int, default=CONSTANT_256, help='SR Tile size, 0 for no tile')
