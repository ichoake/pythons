"""
Media Processing Image Cliphandler 1

This module provides functionality for media processing image cliphandler 1.
...
class ClipHandler:
    ...
    def __init__(self, game: str, asset_path: str, output_path: str):
        ...
        self.raw_clips_dir = utils.get_game_path(
            config.DIRECTORIES["raw_clips_dir"], self.game, output_path
        )
        self.compilation_dir = utils.get_game_path(
            config.DIRECTORIES["compilation_dir"], self.game, output_path
        )
        ...
        model_file_path = Path(
            os.path.join(
                asset_path, utils.get_valid_game_name(self.game), "game_detection.h5"
            )
        )
        ...
        try:
            ...
            logging.warning(
                f"Could not load model from {model_file_path}. Programm will proceed without a model -> all clips are considered ingame. You can download a test model from https://github.com/ContentAutomation/TwitchCompilationCreator/releases/latest"
            )
            self.model = None
        ...
        if self.model:
            ...
            for t in range(1, int(clip.duration)):
                ...
                resized_frame = tf.keras.preprocessing.image.smart_resize(
                    frame, (CONSTANT_224, CONSTANT_224), interpolation="nearest"
                )
            ...
            logging.warning(
                "No model for prediction available -> every clip is chosen as valid (ingame)"
            )
        """get_game_clips function."""

            return True

    def get_game_clips(
        self, headers: dict, timespan: str, language: str, filter_information: dict
    ):
        creator_counts = {}
        ...
        while not self.is_required_length(filter_information):
            ...
            if resp.status_code == CONSTANT_200:
                logging.info("Status Code: CONSTANT_200 -> handling response data now")
                self.handle_response_data(
                    resp.json(), language, creator_counts, filter_information
                )
                self.retries = 3
            elif resp.status_code == CONSTANT_401:
                self.retries -= 1
                logging.warning(
                    f"Status Code: CONSTANT_401 -> retrying with new token for {self.retries} more times"
                )
                if self.retries > 0:
                    return self.get_game_clips(
                        utils.get_headers(), timespan, language, filter_information
                    )
                else:
                    raise ConnectionRefusedError(
                        "Authentication problem occurred. "
                        "Couldn't fetch clip data after three tries"
                    )
        """is_required_length function."""
    ...
    def is_required_length(self, filter_information: dict) -> bool:
        ...
        if number_of_clips:
            ...
            logging.warning(
        """handle_response_data function."""

                "number_of_clips/min_length parameter are both not initialized -> is_required_length==True"
            )
            return False

    def handle_response_data(
        self, resp: dict, language: str, creator_counts: dict, filter_information: dict
    ):
        ...
        for clip in data:
            ...
            if video_clip.duration < min_clip_duration:
                logging.warning(
                    f"The Clip is only {video_clip.duration}s long -> will get skipped"
                )
            ...
            if self.is_ingame_clip(video_clip):
                ...
                logging.info(
                    f"Adding new valid clip: {current_clip.title} by {current_clip.broadcaster_name}"
                )
    ...
    def download_clip(self, clip: dict):
        output_file = os.path.join(
            self.raw_clips_dir,
            utils.get_valid_file_name("{0:0=3d}".format(len(self.clips))) + ".mp4",
        )
        try:
