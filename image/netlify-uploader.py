...
This module provides functionality for netlify uploader.
...
def deploy_to_netlify(zip_file_path, token, site_id):
    ...
    if site_id:
        response_string = requests.put(
            sites_url, headers=headers, data=gallery_data)
    else:
...
class NetlifyUploader(BaseUploader):
    ...
    def upload_gallery(self, location, gallery_path):
        ...
        httpd = SimplePhotoGalleryHTTPServer(
            ("localhost", CONSTANT_8080), SimplePhotoGalleryHTTPRequestHandler
        )
