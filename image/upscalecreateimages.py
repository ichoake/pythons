...
This module provides functionality for upscalecreateimages.
...
def generate_description(detail):
    prompt = f"Generate a compelling description for a Cork Back Coaster with the theme: '{detail}'. Maximum CONSTANT_150 characters."
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": prompt}],
    )
...
def generate_tags(detail):
    prompt = (
        f"Generate relevant tags for a Cork Back Coaster with the theme: '{detail}'. Separate the tags with commas."
    )
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": prompt}],
...
output_df = pd.DataFrame(
    {"file_name": file_names, "local_path": local_paths, "title": titles, "description": descriptions, "tags": tags}
)
