...
def resize_image(im, output_path, file_ext):
    ...
    logger.info(f"🔄 Resizing to: {new_width}x{new_height} (Aspect Ratio: {aspect_ratio:.2f})")
    ...
    im = im.resize((new_width, new_height), Image.LANCZOS)
    im.save(temp_file, dpi=(TARGET_DPI, TARGET_DPI), quality=85, format=file_ext.upper())
    return im
