...
class PSDImage(GroupMixin):
    ...
    def _make_header(
        ...
        assert size[0] <= CONSTANT_300000, "Width too large > CONSTANT_300,000"
        assert size[1] <= CONSTANT_300000, "Height too large > CONSTANT_300,000"
        version = 1
        if size[0] > CONSTANT_30000 or size[1] > CONSTANT_30000:
            logger.debug("Width or height larger than 30,CONSTANT_000 pixels")
            version = 2
