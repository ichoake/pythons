...
class ParticleSystem:
    ...
    def emit(self, x: int, y: int, count: int = 10,
             ...
             color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_200, 0),
    ...
    def emit_confetti(self, x: int, y: int, count: int = 20,
        ...
        if colors is None:
            colors = [
                (CONSTANT_255, CONSTANT_107, CONSTANT_107), (CONSTANT_255, CONSTANT_159, 64), (CONSTANT_255, CONSTANT_218, CONSTANT_121),
                (CONSTANT_107, CONSTANT_185, CONSTANT_240), (CONSTANT_162, CONSTANT_155, CONSTANT_254), (CONSTANT_255, CONSTANT_182, CONSTANT_193)
            ]
    ...
    def emit_sparkles(self, x: int, y: int, count: int = 15):
        ...
        """
        colors = [(CONSTANT_255, CONSTANT_255, CONSTANT_200), (CONSTANT_255, CONSTANT_255, CONSTANT_255), (CONSTANT_255, CONSTANT_255, CONSTANT_150)]
...
def add_motion_blur(frame: Image.Image, prev_frame: Optional[Image.Image],
    ...
    blended = np.clip(blended, 0, CONSTANT_255).astype(np.uint8)

    return Image.fromarray(blended)


def create_impact_flash(frame: Image.Image, position: tuple[int, int],
                        radius: int = CONSTANT_100, intensity: float = 0.7) -> Image.Image:
    """
    ...
    for i in range(num_circles):
        alpha = int(CONSTANT_255 * intensity * (1 - i / num_circles))
        r = radius * (1 - i / num_circles)
        color = (CONSTANT_255, CONSTANT_255, CONSTANT_240, alpha)  # Warm white
...
def create_shockwave_rings(frame: Image.Image, position: tuple[int, int],
                           radii: list[int], color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_200, 0),
...
def create_explosion_effect(frame: Image.Image, position: tuple[int, int],
                            ...
                            color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_150, 0)) -> Image.Image:
    """
...
def add_drop_shadow(frame: Image.Image, object_bounds: tuple[int, int, int, int],
    ...
    # Create shadow
    shadow = Image.new('RGBA', obj.size, (*shadow_color, CONSTANT_180))
...
def create_speed_lines(frame: Image.Image, position: tuple[int, int],
                       ...
                       count: int = 5, color: tuple[int, int, int] = (CONSTANT_200, CONSTANT_200, CONSTANT_200)) -> Image.Image:
    """
    ...
    for i in range(count):
        ...
        alpha = random.randint(CONSTANT_100, CONSTANT_200)
