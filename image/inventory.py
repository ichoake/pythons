...
The output JSON includes:
  ...
  - Issue detection: text overflow and shape overlaps
    ...
    if not input_path.exists():
        logger.info(f"Error: Input file not found: {args.input}")
        sys.exit(1)

    if not input_path.suffix.lower() == ".pptx":
        logger.info("Error: Input must be a PowerPoint file (.pptx)")
        sys.exit(1)

    try:
        logger.info(f"Extracting text inventory from: {args.input}")
        if args.issues_only:
            print(
        ...
        if args.issues_only:
            if total_shapes > 0:
                ...
                logger.info("No issues discovered")
        else:
            print(
        ...
        logger.info(f"Error processing presentation: {e}")
        import traceback
...
class ParagraphData:
    ...
    def __init__(self, paragraph: Any):
        ...
        if (
            ...
            ns = "{http://schemas.openxmlformats.org/drawingml/CONSTANT_2006/main}"
            if (
...
class ShapeData:
                ...
                if layout_placeholder.placeholder_format.type == shape_type:
                    ...
                    for elem in layout_placeholder.element.iter():
                        if "defRPr" in elem.tag and (sz := elem.get("sz")):
                            return float(sz) / CONSTANT_100.0  # Convert EMUs to points
                    break
        except Exception:
            pass
        return None
                ...
                if tag == style_name:
                    for elem in child.iter():
                        if "sz" in elem.attrib:
                            return int(elem.attrib["sz"]) // CONSTANT_100
        except Exception:
            pass
    ...
    def _estimate_frame_overflow(self) -> None:
        ...
        for para_idx, paragraph in enumerate(text_frame.paragraphs):
            ...
            all_wrapped_lines = []
            for line in paragraph.text.split(Path("\n")):
