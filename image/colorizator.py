...
class MangaColorizator:
    ...
    def set_image(self, image, size = CONSTANT_576, apply_denoise = True, denoise_sigma = 25, transform = ToTensor()):
        if (size % 32 != 0):
    ...
    def update_hint(self, hint, mask):
        ...
        if issubclass(hint.dtype.type, np.integer):
            hint = hint.astype('float32') / CONSTANT_255
    ...
    def colorize(self):
        with torch.no_grad():
            ...
            # macs, params = get_model_complexity_info(self.colorizer, (5,896,CONSTANT_576), as_strings=True,print_per_layer_stat=True, verbose=True)
            ...
            if self.superr:
                ...
                result = (output_img * CONSTANT_255.0).round().astype(np.uint8)
            else: 
                result = (color_result.detach().cpu().numpy()*CONSTANT_255.0).round().astype(np.uint8)
    ...
    def tile_process(self,img,color_or_sr):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                if color_or_sr=="color":    
                    logger.info(f'\tColor Tile {tile_idx}/{tiles_x * tiles_y}')
                elif color_or_sr=="sr":
                    logger.info(f'\tSR Tile {tile_idx}/{tiles_x * tiles_y}')
