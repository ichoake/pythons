...
albums_dir = Path(Path("/Users/steven/Music/nocTurneMeLoDieS/Mp3"))
...
for folder in albums_dir.iterdir():
    if folder.is_dir():
        ...
        lyrics_text = transcript.read_text(errors="ignore") if transcript.exists() else "Lyrics not available."
        analysis_text = analysis.read_text(errors="ignore") if analysis.exists() else "Analysis not available."
