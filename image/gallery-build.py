...
def build_html(gallery_config):
    ...
    images_data_list = [{**images_data[image], "name": image}
    ...
    if "remote_gallery_type" in gallery_config and "remote_link" in gallery_config:
        ...
        # to keep it simple so it can be customized. A better solution is
...
def main():
    ...
    # Check if thumbnails exist and generate them if needed or if specified by
    # the user
    try:
        ...
        spg_common.log(
            f"Something went wrong while generating the thumbnails: {
                str(exception)}")
        sys.exit(1)
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while generating the images_data.json file: {
                str(exception)}")
        sys.exit(1)
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while generating the gallery HTML: {
                str(exception)}")
        sys.exit(1)
