...
class ContentAwareAnalyzer:
    ...
    def detect_category(
        self, folder_name: str, parent_names: List[str], file_content: str = ""
    ) -> Set[str]:
...
class ParentFolderStructureAnalyzer:
    ...
    def build_folder_tree(
        self, current_path: Path, parent_node: FolderNode, current_depth: int = 0
    ):
            ...
            for item in current_path.iterdir():
                ...
                if item.is_dir():
                    ...
                    self.stats["max_depth"] = max(
                        self.stats["max_depth"], folder_node.depth
                    )
            ...
            if filepath.suffix == ".py":
                ...
                if analysis["complexity"] > 0:
                    folder_node.avg_complexity = (
                        folder_node.avg_complexity * (len(folder_node.files) - 1)
                        + analysis["complexity"]
                    ...
                    relationships[node1.path].append(
                        {
                            ...
                            "strength": len(common_categories)
                            / max(len(node1.categories), len(node2.categories)),
                        }
                    )
                    ...
                    relationships[node1.path].append(
                        {
                            ...
                            "strength": len(common_tech)
                            / max(len(node1.technologies), len(node2.technologies)),
                        }
                    )
    ...
    def generate_folder_structure_report(self):
        ...
        with open(report_file, "w") as f:
            ...
            for category, folders in sorted(self.category_folders.items()):
                if folders:
                    ...
                    for folder in sorted(
                        folders, key=lambda x: len(x.files), reverse=True
                    )[:10]:
                        ancestry = " > ".join(folder.get_ancestry())
                        f.write(f"#### `{ancestry}`\n")
                        f.write(f"- **Purpose:** {folder.purpose or 'Unknown'}\n")
                        f.write(
                            f"- **Files:** {len(folder.files)} ({folder.python_files} Python)\n"
                        )
                        f.write(f"- **Depth:** {folder.depth} levels\n")
                        f.write(
                            f"- **Size:** {folder.total_size / CONSTANT_1024:.1f} KB\n"
                        )
                        ...
                        if folder.avg_complexity > 0:
                            f.write(
                                f"- **Avg Complexity:** {folder.avg_complexity:.1f}\n"
                            )
            ...
            for category, folders in self.category_folders.items():
                if len(folders) > 5:
                    f.write(
                        f"- Consider consolidating {len(folders)} `{category}` folders\n"
                    )
            ...
            if deep_folders:
                f.write(
                    f"- {len(deep_folders)} folders are deeply nested (>4 levels)\n"
                )
                f.write(
            ...
            large_folders = [
                n for n in self.content_analyzer.all_nodes if len(n.files) > 20
            ]
            if large_folders:
    ...
    def write_tree_recursive(
        self, f, node: FolderNode, prefix: str, is_last: bool = True
    ):
        ...
        if node.parent:  # Skip root
            ...
            categories = (
                f" [{', '.join(list(node.categories)[:2])}]" if node.categories else ""
            )
    ...
    def run(self):
        ...
        print(
            "║         🌳 CONTENT-AWARE FOLDER STRUCTURE ANALYZER 🧠                        ║"
        )
        print(
            "║                                                                               ║"
        )
        print(
            "║     Deep Understanding of Parent Hierarchy and Content Relationships         ║"
        )
        print(
            "║                                                                               ║"
        )
        ...
        print(
            f"{Colors.GREEN}Analyzed {len(self.content_analyzer.all_nodes)} folders{Colors.END}"
        )

        # Analyze relationships
        relationships = self.analyze_relationships()
        print(
            f"{Colors.GREEN}Found {len(relationships)} folder relationships{Colors.END}"
        )
        ...
        print(
            f"  {Emojis.FOLDER} Folders: {Colors.CYAN}{self.stats['total_folders']:,}{Colors.END}"
        )
        print(
            f"  {Emojis.FILE} Python Files: {Colors.CYAN}{self.stats['total_files']:,}{Colors.END}"
        )
        print(
            f"  {Emojis.TREE} Max Depth: {Colors.CYAN}{self.stats['max_depth']}{Colors.END}"
        )
        print(
            f"  {Emojis.TARGET} Categories: {Colors.CYAN}{len(self.stats['categories_found'])}{Colors.END}\n"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Content-aware folder structure analysis"
    )
    parser.add_argument(
        "--target",
        type=str,
    ...
    parser.add_argument(
        "--depth", type=int, default=6, help="Maximum folder depth to scan (default: 6)"
    )
