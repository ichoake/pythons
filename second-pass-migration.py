...
def analyze_remaining_files():
    ...
    for py_file in base_path.glob("*.py"):
        if py_file.name.startswith("analyze_migration") or py_file.name.startswith(
            "migrate_projects"
        ):
        ...
        if any(
            ...
            for pattern in ["img", "image", "photo", "picture", "gallery"]
        ):
            categories["image_processing"].append(py_file.name)
        elif any(
            pattern in name_lower
            for pattern in ["quiz", "tts", "speech", "audio", "voice"]
        ):
            categories["quiz_tts"].append(py_file.name)
        elif any(
            pattern in name_lower
            for pattern in ["convert", "transform", "change", "format"]
        ):
            categories["conversion_tools"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["html", "web", "gallery", "website"]
        ):
            categories["gallery_html"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["csv", "data", "table", "spreadsheet"]
        ):
            categories["csv_data"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["ocr", "text", "extract", "read"]
        ):
            categories["ocr_processing"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["clean", "sort", "organize", "fix"]
        ):
            categories["cleanup_utilities"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["test", "debug", "check", "verify"]
        ):
            categories["test_scripts"].append(py_file.name)
        elif any(
            pattern in name_lower for pattern in ["copy", " (1)", " (2)", "backup"]
        ):
