...
class SuperFlattener:
    ...
    def create_flat_name(self, file_path: Path) -> str:
        ...
        try:
            ...
            generic = {'src', 'lib', 'test', 'tests', 'bin', 'code', 'files', 'data', 'scripts'}
            ...
            if meaningful:
                ...
                context = context.replace('01-', '').replace('02-', '').replace('03-', '')
                context = context.replace('04-', '').replace('05-', '').replace('06-', '')
                context = context.replace('07-', '').replace('08-', '').replace('09-', '')
