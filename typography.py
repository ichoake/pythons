...
def get_font(size: int, bold: bool = False) -> ImageFont.FreeTypeFont:
    ...
    font_paths = [
        ...
        Path("/System/Library/Fonts/SF-Pro.ttf"),
        "/Library/Fonts/Arial Bold.ttf" if bold else Path("/Library/Fonts/Arial.ttf"),
        # Linux fonts
        (
...
def draw_text_with_outline(
    ...
    font_size: int = 40,
    text_color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_255, CONSTANT_255),
    ...
    for offset_x in range(-outline_width, outline_width + 1):
        for offset_y in range(-outline_width, outline_width + 1):
            if offset_x != 0 or offset_y != 0:
                draw.text(
                    (x + offset_x, y + offset_y), text, fill=outline_color, font=font
                )
...
def draw_text_with_glow(
    ...
    font_size: int = 40,
    text_color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_255, CONSTANT_255),
    glow_color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_200, 0),
    glow_radius: int = 5,
    centered: bool = False,
    bold: bool = True,
) -> Image.Image:
    ...
    for radius in range(glow_radius, 0, -1):
        for offset_x in range(-radius, radius + 1):
            for offset_y in range(-radius, radius + 1):
                if offset_x != 0 or offset_y != 0:
                    draw.text(
                        (x + offset_x, y + offset_y), text, fill=glow_color, font=font
                    )
...
def draw_text_in_box(
    ...
    alpha_value = int(CONSTANT_255 * box_alpha)
...
def get_optimal_font_size(
    text: str, max_width: int, max_height: int, start_size: int = 60
) -> int:
    """
...
def scale_font_for_frame(base_size: int, frame_width: int, frame_height: int) -> int:
    ...
    base_dimension = CONSTANT_480  # Reference dimension
