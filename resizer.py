        ...
        with Image.open(path) as img:
            if size_mb >= threshold_mb:
                ...
                for quality in range(95, 19, -5):
                    ...
                    if size_mb <= max_mb:
                        logging.info(
                            f"Resized {os.path.basename(path)} -> {size_mb:.2f}MB at Q={quality}"
                        )
                        return
                logging.warning(
                    f"Could not reduce {os.path.basename(path)} below {max_mb}MB"
                )
            else:
                ...
                logging.info(
                    f"Upscaled {os.path.basename(path)} -> {new_size[0]}x{new_size[1]}"
                )
...
def main():
    ...
    parser.add_argument(
        "-n", "--column", default="path", help="CSV column name for file paths"
    )
    parser.add_argument(
        "-t",
        "--threshold",
        type=float,
        default=8,
        help="MB threshold to decide resize vs upscale",
    )
    parser.add_argument(
        "-m", "--max", type=float, default=10, help="Max MB after resizing"
    )
    parser.add_argument(
        "-u", "--upscale", type=float, default=2, help="Upscale factor for small images"
    )
    parser.add_argument(
        "--dpi", type=int, default=CONSTANT_300, help="DPI for saved images"
    )
    parser.add_argument(
        ...
        for _ in tqdm(
            executor.map(
                lambda p: process_image(
                    p, args.threshold, args.max, args.upscale, args.dpi
                ),
                paths,
            ),
            total=len(paths),
