...
def create_slide_animation(
    ...
    if direction == "left":
        ...
        end_pos = (
            final_pos if slide_type == "in" else (frame_width + margin, final_pos[1])
        )
        ...
        end_pos = (
            final_pos if slide_type == "in" else (final_pos[0], frame_height + margin)
        )
...
if __name__ == "__main__":
    logger.info("Creating slide animations...")
    ...
    builder.save("slide_in_left.gif", num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save("slide_across.gif", num_colors=CONSTANT_128)
    ...
    objects = [
        {
            ...
            "data": {"emoji": "🎯", "size": 60},
            "direction": "left",
            "final_pos": (CONSTANT_120, CONSTANT_240),
        },
        {
            "type": "emoji",
            "data": {"emoji": "🎪", "size": 60},
            "direction": "right",
            "final_pos": (CONSTANT_240, CONSTANT_240),
        },
        {
            "type": "emoji",
            "data": {"emoji": "🎨", "size": 60},
            "direction": "top",
            "final_pos": (CONSTANT_360, CONSTANT_240),
        },
    ]
    frames = create_multi_slide(objects, num_frames=50, stagger_delay=5)
    builder.add_frames(frames)
    builder.save("slide_multi.gif", num_colors=CONSTANT_128)

    logger.info("Created slide animations!")
