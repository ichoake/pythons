...
def analyze_file_type(file_type, drive1_file, drive2_file):
    ...
    total_size1 = sum(
        ...
        for item in data1
        if item["File_Size_Bytes"].isdigit()
    )
    total_size2 = sum(
        int(item["File_Size_Bytes"])
        for item in data2
    ...
    logger.info(
        f"2T-Xx total size: {total_size1 / CONSTANT_1024 / CONSTANT_1024:.1f} MB"
    )
    logger.info(
        f"DeVonDaTa total size: {total_size2 / CONSTANT_1024 / CONSTANT_1024:.1f} MB"
    )
            ...
            all_duplicates.append(
                {
                    ...
                    "drive1_path": (
                        dup["drive1_files"][0]["File_Path"]
                        if dup["drive1_files"]
                        else ""
                    ),
                    "drive2_path": (
                        dup["drive2_files"][0]["File_Path"]
                        if dup["drive2_files"]
                        else ""
                    ),
