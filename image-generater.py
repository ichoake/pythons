"""
Selenium Image Generater

This module provides functionality for selenium image generater.
    ...
    with Image(filename=bg) as bg_img:
        ...
        with Image(file=meme) as meme1:
            ...
            if w >= CONSTANT_536:
                ...
                pass
            meme1.resize(w, h)
            bg_img.composite(meme1, left=CONSTANT_100, top=CONSTANT_100)
        ...
        with Image(file=meme) as meme2:
            ...
            h2, w2, c2 = img2.shape
            if w2 >= CONSTANT_536:
                diff = w2 - CONSTANT_536
                w2 = w2 - diff

                percent = diff / CONSTANT_536
                h2 = round(h2 * (1 - percent))
            elif w2 <= CONSTANT_536:
                diff = CONSTANT_536 - w2
                w2 = w2 + diff
            ...
            meme2.resize(w, h)
            bg_img.composite(meme2, left=CONSTANT_100, top=CONSTANT_150 + h)
            ...
            with Image(filename="logo.png") as fg_img3:
                ...
                bg_img.save(filename=os.getcwd() + Path("\\images\\") + name + ".png")
