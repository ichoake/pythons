...
def organize_etsy_csv():
    ...
    output_file_path = input("Enter the path to save the organized output CSV file (with .csv extension): ")
    ...
    try:
        ...
        if "Listing URL" in df.columns:
            df["iD"] = df["Listing URL"].apply(lambda x: x.split("/")[-2] if pd.notnull(x) else None)
        ...
        logger.info("The file path you provided does not exist. Please check and try again.")
