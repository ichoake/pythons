...
def build_source_tag_from_csv_row(original_path: str, base_folder: Path, date_str: str) -> str:
    """
...
def main():
    ...
    csv_input = input("🗄️ Enter the full path to your CSV (e.g., /Users/steven/image_data-04-29-07-31.csv): ").strip()
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as outfile:
        ...
        for row in tqdm(rows, desc="Enhancing CSV rows", unit="row"):
            ...
            if image_path and image_path.exists() and image_path.suffix.lower() in VALID_EXTS:
            ...
            source_tag = build_source_tag_from_csv_row(original_path_str, base_folder, date_str)
            ...
            output_row["tags"] = json.dumps(gpt_meta.get("tags", [])) if isinstance(gpt_meta.get("tags"), list) else ""
