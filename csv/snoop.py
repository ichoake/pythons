...
CONSTANT_333333 = 333333
CONSTANT_1581638400 = 1581638400
...
def version_snoop(vers, vers_code, demo_full):
    print(f"""\CONSTANT_033[36m
  ___|
\\___ \\  __ \\   _ \\   _ \\  __ \\ 
      | |   | (   | (   | |   | 
_____/ _|  _|\\___/ \\___/  .__/  
                         _|    \CONSTANT_033[0m \CONSTANT_033[37m\CONSTANT_033[44m{vers}\CONSTANT_033[0m
""")
    ...
    console.logger.info(f"[dim cyan] $ [/dim cyan][cyan]{'python' if WINDOWS else 'python3'} snoop.py --help[/cyan] #справка")
    console.logger.info(f"[dim cyan] $ [/dim cyan][cyan]{'python' if WINDOWS else 'python3'} snoop.py --module[/cyan] #плагины")
    console.logger.info(f"[dim cyan] $ [/dim cyan][cyan]{'python' if WINDOWS else 'python3'} snoop.py nickname[/cyan] #поиск user-a")
...
def mkdir_path():
    if WINDOWS:
        dirhome = os.environ['LOCALAPPDATA'] + Path("\\snoop")
    elif ANDROID:
        if not os.access(Path("/data/data/com.termux/files/home/storage/shared"), os.W_OK):
            ...
            dirhome = os.environ['HOME'] + Path("/snoop")
        else:
            dirhome = Path("/data/data/com.termux/files/home/storage/shared/snoop")
    elif LINUX:
        dirhome = os.environ['HOME'] + Path("/snoop")
...
END_OF_LICENSE = (CONSTANT_2026, 1, 1, 3, 0, 0, 0, 0, 0) #формат даты согласно международному стандарту ISO CONSTANT_8601, год-месяц-день.
...
def web_path_copy():
    try:
        ...
        elif "source" in VERSION and ANDROID and os.path.exists(Path("/data/data/com.termux/files/home/storage/shared/snoop/web")) is False:
            shutil.copytree(f"{os.getcwd()}/web", Path("/data/data/com.termux/files/home/storage/shared/snoop/web"))
...
def mem_test():
    try:
        return round(psutil.virtual_memory().available / CONSTANT_1024 / CONSTANT_1024)
    except Exception:
        if not WINDOWS:
            console.logger.info(f"{' ' * 17} [bold red]ERR Psutil lib[/bold red]")
...
def info_str(infostr, nick, color=True):
    if color is True:
        logger.info(f"{Fore.GREEN}[{Fore.YELLOW}*{Fore.GREEN}] {infostr}{Fore.RED} <{Fore.WHITE} {nick} {Fore.RED}>{Style.RESET_ALL}")
    else:
        logger.info(f"\n[*] {infostr} < {nick} >")
...
def bad_raw(flagBS_err, bad_zone, nick, lst_options):
    logger.info(f"{Fore.CYAN}├───Дата поиска:{Style.RESET_ALL} {time.strftime('%Y-%m-%d__%H:%M:%S', TIME_DATE)}")

    if any(lst_options):
        logger.info(f"{Fore.CYAN}└────\CONSTANT_033[31;1mBad_raw: {flagBS_err}% БД, bad_zone {bad_zone}\CONSTANT_033[0m\n")
    else:
        if 4 >= flagBS_err >= 2:
            logger.info(f"{Fore.CYAN}└────\CONSTANT_033[33;1mВнимание! Bad_raw: {flagBS_err}% БД, bad_zone {bad_zone}\CONSTANT_033[0m")
        elif 12 >= flagBS_err > 4:
            logger.info(f"{Fore.CYAN}└────\CONSTANT_033[31;1mВнимание!! Bad_raw: {flagBS_err}% БД, bad_zone {bad_zone}\CONSTANT_033[0m")
        elif flagBS_err > 12:
            print(f"{Fore.CYAN}└────\CONSTANT_033[30m\CONSTANT_033[41mВнимание!!! Bad_raw: {flagBS_err}% БД, критический уровень, " + \
                  f"bad_zone {bad_zone}\CONSTANT_033[0m")

    if not any(lst_options):
        logger.info(Fore.CYAN + "     └─нестабильное соединение или I_Censorship")
        print(f"       \CONSTANT_033[36m{'├' if 'full' in VERSION else '└'}─используйте \CONSTANT_033[36;1mVPN\CONSTANT_033[0m\CONSTANT_033[36m/'\CONSTANT_033[0m" + \
              f"\CONSTANT_033[36;1m--web-base\CONSTANT_033[0m\CONSTANT_033[36m'\CONSTANT_033[0m ", end='' if 'full' in VERSION else '\n\n')
        if "full" in VERSION:
            nick = f"'{nick}'" if nick.count(" ") > 0 else nick
            print(f"\CONSTANT_033[36m\n       └─или исключите из поиска bad_zone: '\CONSTANT_033[36;1m" + \
                  f"{bad_zone.split('/')[0].replace('~', '')}\CONSTANT_033[0m" + \
                  f"\CONSTANT_033[36m'\n         └─$ {os.path.basename(sys.argv[0])} --exclude " + \
                  f"{bad_zone.split('/')[0].replace('~', '')} {nick}\CONSTANT_033[0m\n")
...
def print_error(websites_names, errstr, country_code, errX, verbose=False, color=True):
    ...
    if color is True:
        ...
        logger.info(f"[!] {websites_names}: {errstr}{country_code} {errX if verbose else ''}")
...
def print_found_country(websites_names, url, country_Emoj_Code, verbose=False, color=True):
    ...
    if color is True and WINDOWS:
        ...
        logger.info(f"[+] {websites_names}: {url}")
...
def print_not_found(websites_names, verbose=False, color=True):
    ...
    if color is True:
        ...
        logger.info(f"[-] {websites_names}: Увы!")
...
def warning_lib():
    if int(requests.urllib3.__version__.split(".")[0]) < 2 or int("".join(requests.__version__.split("."))) < CONSTANT_2282:
...
def r_session(cert=False, connect=0, speed=False, norm = False, method="get",
    ...
    if speed:
        ...
        connections = CONSTANT_200 if LINUX else (70 if WINDOWS else 40) #L/W/A.

    if req_retry:
        total = False if norm else None
        retry = requests.urllib3.util.Retry(total=CONSTANT_100, connect=CONSTANT_100, read=CONSTANT_100, status=CONSTANT_100, other=CONSTANT_100, backoff_factor=0.1)
...
# Ловушка на некот.сайтах (if response.content is not None ≠ if response.content).
    if response.content is not None and response.encoding == 'ISO-CONSTANT_8859-1':
        try:
    ...
    if len(username) < 3:
        ...
        with console.status("[bold blue] 💡 Обнаружена пасхалка...", spinner='noise'):
            try:
                ...
                console.logger.info(Panel(Markdown(r_east.text.replace("=" * 83, "")),
                                    subtitle="[bold blue]журнал snoop-версий[/bold blue]", style=STL(color="cyan")))
                console.print(Panel(f"[bold cyan]Дата создания проекта:[/bold cyan] CONSTANT_2020-02-14 " + \
                                    f"({round((time.time() - CONSTANT_1581638400) / CONSTANT_86400)}_дней).\n" + \
                                    f"[bold cyan]Последнее обновление репозитория:[/bold cyan] " + \
                                    f"{'_'.join(r_repo.get('pushed_at')[0:-4].split('T'))} (UTC).\n" + \
                                    f"[bold cyan]Сжатие репозитория:[/bold cyan] CONSTANT_2024-12-11.\n" + \
                                    f"[bold cyan]Размер репозитория:[/bold cyan] {round(int(r_repo.get('size')) / CONSTANT_1024, 1)} MB.\n" + \
...
## Создать многопоточный/процессный сеанс для всех запросов.
    if ANDROID:
        ...
        if norm is False:
            proc_ = len(BDdemo_new) if len(BDdemo_new) < 70 else (50 if len(os.sched_getaffinity(0)) < 4 else CONSTANT_140)
        else:
            proc_ = len(BDdemo_new) if len(BDdemo_new) < 70 else (60 if len(os.sched_getaffinity(0)) < 4 else CONSTANT_180)
        executor_req = ProcessPoolExecutor(max_workers=proc_ if not speed else speed)
...
# Пользовательский user-agent браузера (рандомно на каждый сайт), а при сбое — постоянный с расширенным заголовком.
        majR = random.choice(range(CONSTANT_101, CONSTANT_124, 1))
        RandHead=([f'{{"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/CONSTANT_537.36 (KHTML, like Gecko) ' + \
                   f'Chrome/{majR}.0.0.0 Safari/CONSTANT_537.36"}}',
                   f'{{"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ' + \
                   f'AppleWebKit/CONSTANT_537.36 (KHTML, like Gecko) Chrome/{majR}.0.0.0 Safari/CONSTANT_537.36"}}'])
...
# Переопределить/добавить любые дополнительные заголовки необходимые для сайта из БД, или задать U-A из CLI.
        ...
        # console.logger.info(headers, websites_names) #проверка u-агентов
...
# Дергаем объект сессии не по прямому назначению, спасаем CPU/Windows/Многопоточность на длинной дистанции.
            req_retry = True if "full" in VERSION or len(BDdemo_new) > CONSTANT_399 else False
...
## Панель вербализации.
        if not ANDROID:
            if color:
                console.logger.info(Panel("[yellow]время[/yellow] | [magenta]выпол.[/magenta] | [bold cyan]отклик (t=s)[/bold cyan] " + \
                ...
                console.logger.info(Panel("отклик сайта (t=s) | общ.время (T=s) | разм.данных | дост.память", title="Обозначение"))
        else:
            if color:
                console.logger.info(Panel("[yellow]time[/yellow] | [magenta]perc.[/magenta] | [bold cyan]response (t=s)[/bold cyan] " + \
                ...
                console.logger.info(Panel("time | perc. | response (t=s) | total time (T=s) | data | avail.ram", title="Designation"))
            ...
            if norm is False and r == "FakeNone":
                head_duble = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                              ...
                              'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) ' + \
                                            'AppleWebKit/CONSTANT_537.36 (KHTML, like Gecko) Chrome/CONSTANT_118.0.0.0 Safari/CONSTANT_537.36'}
                ...
                for num, _ in enumerate(range(3), 1):
                    ...
                    if color is True and print_found_only is False:
                        ...
                        if print_found_only is False:
                            logger.info("    ┌──└──повторное соединение")
...
# Автодетектирование кодировки при устаревшей специфике либы requests/ISO-CONSTANT_8859-1, или ее смена вручную через БД.
            try:
                if r is not None and r != "FakeNone" and r != "FakeStuck":
                    if r.content and r.encoding == 'ISO-CONSTANT_8859-1': #ловушка (if r is not None ≠ if r)
                        r.encoding = char_detect(r.content).get("encoding")
                        if r.encoding is None: r.encoding = "utf-8"
                    elif r.content and r.encoding != 'ISO-CONSTANT_8859-1' and r.encoding.lower() != 'utf-8':
...
# Ответы message (разные локации).
            if error_type == "message":
                ...
                try:
                    if r.status_code > CONSTANT_200 and param_websites.get("ignore_status_code") is None \
...
# Проверка username при статусе CONSTANT_301 и CONSTANT_303 (перенаправление и соль).
            elif error_type == "redirection":
                if r.status_code == CONSTANT_301 or r.status_code == CONSTANT_303:
                    if not norm:
                        print_found_country(websites_names, url, country_Emoj_Code, verbose, color)
                    exists = "найден!"
                    if reports:
...
# Проверяет, является ли код состояния ответа 2..
                ...
                if not r.status_code >= CONSTANT_300 or r.status_code < CONSTANT_200:
                    if not norm:
                        print_found_country(websites_names, url, country_Emoj_Code, verbose, color)
                    if reports:
                        executor_req_save.submit(sreports, url, headers, error_type, username, websites_names, r)
...
# Перенаправление.
                ...
                if CONSTANT_200 <= r.status_code < CONSTANT_300:
                    if not norm:
                        print_found_country(websites_names, url, country_Emoj_Code, verbose, color)
                    if reports:
                        executor_req_save.submit(sreports, url, headers, error_type, username, websites_names, r)
...
## Попытка получить информацию из запроса, пишем в csv.
            ...
            try: #сессия в kB
                ...
                if session_size >= CONSTANT_555:
                    session_size = round(session_size / CONSTANT_1024)
                elif session_size < CONSTANT_555:
                    session_size = round((session_size / CONSTANT_1024), 2)
            except Exception:
                session_size = "Err"
...
# Реакция.
            ...
            try:
                os.execl(sys.executable, sys.executable, *sys.argv) if len(BDdemo_new) > int(CONSTANT_403.9) else "dif_time"
            except Exception:
                pass
...
## Опция '-v'.
            if verbose is True:
                ram_free = mem_test()
                ram_free_color = "[cyan]" if ram_free > CONSTANT_100 else "[red]"
...
def set_timeout(value):
    try:
        ...
        raise argparse.ArgumentTypeError(f"\n\CONSTANT_033[31;1mTimeout '{value}' Err,\CONSTANT_033[0m \CONSTANT_033[36m" + \
...
def speed_snoop(speed):
    try:
        ...
        if WINDOWS and (speed <= 0 or speed > 60):
            raise Exception("")
        elif speed <= 0 or speed > CONSTANT_300:
            raise Exception("")
        return speed
    except Exception:
        if not WINDOWS:
            raise argparse.ArgumentTypeError(f"\n\CONSTANT_033[31;1mMax. workers proc = '{speed}' Err,\CONSTANT_033[0m" + \
                                              " \CONSTANT_033[36m рабочий диапазон от '1' до '300' целым числом.\n \CONSTANT_033[0m")
        else:
...
def update_snoop():
    print("""
\CONSTANT_033[36mВы действительно хотите:
                    __             _  
...
|_||_)(_|(_| |_(/_ __)| |(_)(_)|_) o  
   |                           |    \CONSTANT_033[0m""")
    ...
    while True:
        logger.info("\CONSTANT_033[36mВыберите действие:\CONSTANT_033[0m [y/n] ", end='')
        upd = input().lower()
        if upd == "y":
            logger.info("\CONSTANT_033[36mПримечание: функция обновления Snoop работает при помощи утилиты < Git >\CONSTANT_033[0m")
            ...
            logger.info(Style.BRIGHT + Fore.RED + format_txt("{0}└──False, [Y/N] ?", k=True, m=True).format(' ' * 25))
    sys.exit()
...
def autoclean():
    print("""
\CONSTANT_033[36mВы действительно хотите:\CONSTANT_033[0m \CONSTANT_033[31;1m
               _                _  
 _| _ |  _.|| |_) _ ._  _ .-_|_  ) 
(_|(/_| (_||| | \\(/_|_)(_)|  |_ o  
                    |             \CONSTANT_033[0m""")
...
# Определение директорий.
                path_build_del = Path("/results") if not WINDOWS else Path("\\results")
...
# Сброс кэша и удаление каталога 'results'.
                ...
                logger.info(f"\n\CONSTANT_033[31;1mdeleted --> '{rm}'\CONSTANT_033[0m\CONSTANT_033[36m {len(delfiles)} files, " + \
                      f"{round(total_size/CONSTANT_1024/CONSTANT_1024, 2)} MB\CONSTANT_033[0m")
            except Exception:
...
def license_snoop():
    with open('COPYRIGHT', 'r', encoding="utf8") as copyright:
        ...
        console.logger.info(Panel(cop, title='[bold white]COPYRIGHT[/bold white]', style=STL(color="white", bgcolor="blue")))
    ...
    if not ANDROID:
        ...
        if LINUX and 'full' in VERSION:
            ram_av = CONSTANT_3000 if os.cpu_count() > 4 else CONSTANT_700
        elif LINUX and 'demo' in VERSION:
            ram_av = CONSTANT_200

        try:
            ram = int(psutil.virtual_memory().total / CONSTANT_1024 / CONSTANT_1024)
            ram_free = int(psutil.virtual_memory().available / CONSTANT_1024 / CONSTANT_1024)
            if ram_free < ram_av:
...
def main_cli():
    ...
    flagBS = len(BDdemo)
    web_sites = f"{len(BDflag) // CONSTANT_100}00+"
...
# Назначение опций Snoop.
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                     ...
                                     epilog=(f"{Fore.CYAN}Snoop {Style.BRIGHT}{Fore.RED}demo version {Style.RESET_ALL}" + \
                                             f"{Fore.CYAN}поддержка: \CONSTANT_033[31;1m{flagBS}\CONSTANT_033[0m \CONSTANT_033[36mWebsites!\n{Fore.CYAN}" + \
                                             f"Snoop \CONSTANT_033[36;1mfull version\CONSTANT_033[0m \CONSTANT_033[36mподдержка: " + \
                                             f"\CONSTANT_033[36;1m{web_sites} \CONSTANT_033[0m\CONSTANT_033[36mWebsites!!!\CONSTANT_033[0m\n\n"))
# Service arguments.
    service_group = parser.add_argument_group('\CONSTANT_033[36mservice arguments\CONSTANT_033[0m')
    service_group.add_argument("--version", "-V", action="store_true",
                               help="\CONSTANT_033[36mA\CONSTANT_033[0mbout: вывод на печать версии ПО, snoop info и Лицензии")
    service_group.add_argument("--list-all", "-l", action="store_true", dest="listing",
                               help="\CONSTANT_033[36mВ\CONSTANT_033[0mывести на печать детальную информацию о базе данных Snoop")
    service_group.add_argument("--donate", "-d", action="store_true", dest="donation",
                               help="\CONSTANT_033[36mП\CONSTANT_033[0mожертвовать на развитие Snoop Project-а, получить/приобрести \
                                     \CONSTANT_033[32;1mSnoop full version\CONSTANT_033[0m")
    service_group.add_argument("--autoclean", "-a", action="store_true", dest="autoclean", default=False,
                               help="\CONSTANT_033[36mУ\CONSTANT_033[0mдалить все отчеты, очистить кэш")
    service_group.add_argument("--update", "-U", action="store_true", dest="update",
                               help="\CONSTANT_033[36mО\CONSTANT_033[0mбновить Snoop")
# Plugins arguments.
    plugins_group = parser.add_argument_group('\CONSTANT_033[36mplugins arguments\CONSTANT_033[0m')
    plugins_group.add_argument("--module", "-m", action="store_true", dest="module", default=False,
                               help="\CONSTANT_033[36mO\CONSTANT_033[0mSINT поиск: задействовать различные плагины Snoop:: IP/GEO/YANDEX")
# Search arguments.
    search_group = parser.add_argument_group('\CONSTANT_033[36msearch arguments\CONSTANT_033[0m')
    search_group.add_argument("username", nargs='*', metavar='nickname', action="store", default=None,
                              help="\CONSTANT_033[36mН\CONSTANT_033[0mикнейм разыскиваемого пользователя. \
    ...
    search_group.add_argument("--base", "-b <file>", dest="json_file", default="BDdemo", metavar='',
                              help=argparse.SUPPRESS if "demo" in VERSION else "\CONSTANT_033[36mУ\CONSTANT_033[0mказать для поиска 'nickname' \
                                                                                другую БД (Локально)")
    search_group.add_argument("--web-base", "-w", action="store_true", dest="web", default=False,
                              help=f"\CONSTANT_033[36mП\CONSTANT_033[0mодключиться для поиска 'nickname' к динамично-обновляемой web_БД \
                                    ({web_sites} сайтов)")
    search_group.add_argument("--site", "-s <site_name>", action="append", metavar='', dest="site_list", default=None,
                              help="\CONSTANT_033[36mУ\CONSTANT_033[0mказать имя сайта из БД '--list-all'. Поиск 'nickname' на одном указанном ресурсе, \
    ...
    search_group.add_argument("--exclude", "-e <country_code>", action="append", metavar='', dest="exclude_country", default=None,
                              help="\CONSTANT_033[36mИ\CONSTANT_033[0mсключить из поиска выбранный регион, допустимо использовать опцию '-e' \
    ...
    search_group.add_argument("--include", "-i <country_code>", action="append", metavar='', dest="one_level", default=None,
                              help="\CONSTANT_033[36mВ\CONSTANT_033[0mключить в поиск только выбранный регион, \
    ...
    search_group.add_argument("--time-out", "-t <digit>", action="store", metavar='', dest="timeout", type=set_timeout, default=8.9,
                              help="\CONSTANT_033[36mУ\CONSTANT_033[0mстановить max время ожидания ответа от сервера (секунды).\n"
    ...
    search_group.add_argument("--country-sort", "-c", action="store_true", dest="country", default=False,
                              help="\CONSTANT_033[36mП\CONSTANT_033[0mечать и запись результатов по странам, а не по алфавиту")
    search_group.add_argument("--no-func", "-n", action="store_true", dest="no_func", default=False,
                              help="\CONSTANT_033[36m✓\CONSTANT_033[0mМонохромный терминал, не использовать цвета в url \
    ...
    search_group.add_argument("--found-print", "-f", action="store_true", dest="print_found_only", default=False,
                              help="\CONSTANT_033[36mВ\CONSTANT_033[0mыводить на печать только найденные аккаунты")
    search_group.add_argument("--verbose", "-v", action="store_true", dest="verbose", default=False,
                              help="\CONSTANT_033[36mВ\CONSTANT_033[0mо время поиска 'nickname' выводить на печать подробную вербализацию")
    search_group.add_argument("--userlist", "-u <file>", metavar='', action="store", dest="user", default=False,
                              help="\CONSTANT_033[36mУ\CONSTANT_033[0mказать файл со списком user-ов. Snoop интеллектуально обработает \
                                    данные и предоставит доп.отчеты")
    search_group.add_argument("--save-page", "-S", action="store_true", dest="reports", default=False,
                              help="\CONSTANT_033[36mС\CONSTANT_033[0mохранять найденные странички пользователей в локальные html-файлы,\
                              медленный режим")
    ...
    search_group.add_argument("--headers", "-H <User-Agent>", metavar='', dest="header_custom", nargs=1, default=None,
                              help=argparse.SUPPRESS)
    _val = "60 макс. рабочих потоков." if WINDOWS else "CONSTANT_300 макс. процессов."
    search_group.add_argument("--pool", "-p <digit>", metavar='', dest="speed", type=speed_snoop, default=False,
                              help=
                              f"""
                               \CONSTANT_033[36mО\CONSTANT_033[0mтключить автооптимизацию и задать вручную скорость поиска от 1 до {_val}
    ...
    search_group.add_argument("--quick", "-q", action="store_true", dest="norm", default=False,
                              help=
                              """
                              \CONSTANT_033[36mБ\CONSTANT_033[0mыстрый и агрессивный режим поиска.
...
## Опции  '-csei' несовместимы между собой и quick-режим.
    if args.norm and 'full' in VERSION:
        logger.info(Fore.CYAN + format_txt("активирована опция '-q': «быстрый режим поиска»", k=True))
        ...
        snoopbanner.logo("в demo деактивирован переключатель '-q': «режим SNOOPninja/Quick»...",
                         color="\CONSTANT_033[37m\CONSTANT_033[44m", exit=False)
        snoopbanner.donate()
    elif args.norm is False and args.listing is False and args.speed is False and 'full' in VERSION:
        if LINUX:
            logger.info(Fore.CYAN + format_txt("активирован дефолтный поиск '--': «режим SNOOPninja»", k=True))
...
## Опция  '-a'.
    if args.autoclean:
        logger.info(Fore.CYAN + format_txt("активирована опция '-a': «удаление накопленных отчетов»", k=True))
        autoclean()


## Опция  '-H'.
    if args.header_custom:
        logger.info(Fore.CYAN + format_txt("активирована скрытая опция '-H': «переопределение user-agent(s)»", k=True), '\n',
...
# Информативный вывод.
    if args.module:
        ...
        logger.info(Fore.CYAN + format_txt("активирована опция '-m': «модульный поиск»", k=True))

        def module():
            logger.info(fPath("\n") + \
                  f"\CONSTANT_033[36m╭Выберите плагин или действие из списка\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36m├──\CONSTANT_033[0m\CONSTANT_033[36;1m[1] --> GEO_IP/domain\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36m├──\CONSTANT_033[0m\CONSTANT_033[36;1m[2] --> Reverse Vgeocoder\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36m├──\CONSTANT_033[0m\CONSTANT_033[36;1m[3] --> \CONSTANT_033[30;1mYandex_parser\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36m├──\CONSTANT_033[0m\CONSTANT_033[32;1m[help] --> Справка\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36m└──\CONSTANT_033[0m\CONSTANT_033[31;1m[q] --> Выход\CONSTANT_033[0m\n")
            ...
            if mod == 'help':
                ...
                logger.info(Style.BRIGHT + Fore.RED + "└──Неверный выбор\n" + Style.RESET_ALL)
                return module()
...
## Опция  '-С'.
    if args.cert:
        logger.info(Fore.CYAN + format_txt("активирована скрытая опция '-C': «проверка сертификатов на серверах вкл»", k=True))


## Опция  '-w'.
    if args.web:
        logger.info(Fore.CYAN + format_txt("активирована опция '-w': «подключение к внешней web_database»", k=True))


## Опция  '-S'.
    if args.reports:
        logger.info(Fore.CYAN + format_txt("активирована опция '-S': «сохранять странички найденных аккаунтов»", k=True))


## Опция  '-n'.
    if args.no_func:
        logger.info(Fore.CYAN + format_txt("активирована опция '-n': «отключены:: цвета; флаги; браузер; прогресс»", k=True))
...
## Опция '-f'.
    if args.print_found_only:
        logger.info(Fore.CYAN + format_txt("активирована опция '-f': «выводить на печать только найденные аккаунты»", k=True))


## Опция '-s'.
    if args.site_list:
        logger.info(Fore.CYAN + format_txt(f"активирована опция '-s': «поиск{Style.BRIGHT}{Fore.CYAN} {', '.join(args.username)}" + \
                                     f"{Style.RESET_ALL} {Fore.CYAN}на выбранных website(s)»", k=True), '\n',
              Fore.CYAN + format_txt("допустимо использовать опцию '-s' несколько раз"), Path("\n"),
              Fore.CYAN + format_txt("[опция '-s'] несовместима с [опциями '-с', '-e', '-i']"), sep="")


## Опция '--list-all'.
    if args.listing:
        logger.info(Fore.CYAN + format_txt("активирована опция '-l': «детальная информация о БД Snoop»", k=True))
        print("\CONSTANT_033[36m\nСортировать БД Snoop по странам, по имени сайта или обобщенно ?\n" + \
              "по странам —\CONSTANT_033[0m 1 \CONSTANT_033[36mпо имени —\CONSTANT_033[0m 2 \CONSTANT_033[36mall —\CONSTANT_033[0m 3\n")
...
# Сортируем по алфавиту для full/demo version (2!).
                ...
                for con in datajson_sort:
                    ...
                    i += 1
                    listfull.append(f"\CONSTANT_033[36;2m{i}.\CONSTANT_033[0m \CONSTANT_033[36m{S}  {con}")
                logger.info("\n~~~~~~~~~~~~~~~~\n".join(listfull), Path("\n"))
...
# Сортируем по странам для full/demo version (1!).
                ...
                for i in enumerate(sorted(listwindows, key=str.lower), 1):
                    listfull.append(f"\CONSTANT_033[36;2m{i[0]}. \CONSTANT_033[0m\CONSTANT_033[36m{i[1]}")
                logger.info("~~~~~~~~~~~~~~~~\n".join(listfull))
...
# Действие не выбрано '--list-all'.
                ...
                logger.info(Style.BRIGHT + Fore.RED + format_txt("{0}└──False, [1/2/3] ?", k=True, m=True).format(' ' * 19))
        sys.exit()


## Опция донат '-d'.
    if args.donation:
        logger.info(Fore.CYAN + format_txt("активирована опция '-d': «финансовая поддержка проекта»", k=True))
        snoopbanner.donate()
...
## Опция '-u' указание файла-списка разыскиваемых пользователей.
    if args.user:
        ...
        try:
            ...
            logger.info(Fore.CYAN + format_txt("активирована опция '-u': «розыск nickname(s) из файла:: {0}{1}{2}{3}{4}» {5}",
            ...
            with open(patchuserlist, "r", encoding="utf8") as u1:
                userlist = [(line[0], line[1].strip()) for line in enumerate(u1.read().replace(Path("\ufeff"), "").splitlines(), 1)]
            ...
            print(f"\n\CONSTANT_033[31;1mНе могу найти_прочитать файл: '{userfile}'.\CONSTANT_033[0m \CONSTANT_033[36m\n " + \
                  f"\nПожалуйста, укажите текстовый файл в кодировке —\CONSTANT_033[0m \CONSTANT_033[36;1mutf-8.\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36mПо умолчанию, например, блокнот в OS Windows сохраняет текст в кодировке — ANSI.\CONSTANT_033[0m\n" + \
                  f"\CONSTANT_033[36mОткройте ваш файл '{userfile}' и измените кодировку [файл ---> сохранить как ---> utf-8].\n" + \
                  f"\CONSTANT_033[36mИли удалите из файла нечитаемые спецсимволы.")
            sys.exit()

# good user.
        if userlists:
            _userlists = [f"[dim cyan]{num}.[/dim cyan] {v} [{k}]".replace("", "") for num, (k, v) in enumerate(userlists, 1)]
            console.logger.info(Panel.fit(Path("\n").join(_userlists).replace("%20", " "), title=f"valid ({len(userlists)})",
...
# duplicate user.
        if duble:
            ...
            logger.info(f"\n\CONSTANT_033[36mследующие nickname(s) из '\CONSTANT_033[36;1m{userfile}\CONSTANT_033[0m\CONSTANT_033[36m' содержат " + \
                  f"\CONSTANT_033[33mдубли\CONSTANT_033[0m\CONSTANT_033[36m и будут пропущены:\CONSTANT_033[0m")
...
# bad user.
        if userlists_bad:
            ...
            logger.info(f"\n\CONSTANT_033[36mследующие nickname(s) из '\CONSTANT_033[36;1m{userfile}\CONSTANT_033[0m\CONSTANT_033[36m' содержат " + \
                  f"\CONSTANT_033[31;1mN/A-символы\CONSTANT_033[0m\CONSTANT_033[36m и будут пропущены:\CONSTANT_033[0m")
...
# Short user.
        if short_user:
            ...
            logger.info(f"\n\CONSTANT_033[36mследующие nickname(s) из '\CONSTANT_033[36;1m{userfile}\CONSTANT_033[0m\CONSTANT_033[36m'\CONSTANT_033[0m " + \
                  f"\CONSTANT_033[31;1mкороче 3-х символов\CONSTANT_033[0m\CONSTANT_033[36m и будут пропущены:\CONSTANT_033[0m")
...
# Сохранение bad_nickname(s) в отдельном txt файле.
        ...
        if bool(USERLIST) is False:
            logger.info(Path("\n"), Style.BRIGHT + Fore.RED + format_txt("⛔️ Файл '{0}' не содержит ни одного валидного nickname".format(userfile),
                                                             k=True, m=True), "\n\n\CONSTANT_033[31;1mВыход\CONSTANT_033[0m\n", sep="")
            sys.exit()
...
## Опция '--update' обновление Snoop.
    if args.update:
        logger.info(Fore.CYAN + format_txt("активирована опция '-U': «обновление snoop»", k=True))
        update_snoop()
...
## Опция '-b'. Проверить, существует ли альтернативная база данных, иначе default.
    if not os.path.exists(str(args.json_file)):
        logger.info(f"\n\CONSTANT_033[31;1mОшибка! Неверно указан путь к файлу: '{str(args.json_file)}'.\CONSTANT_033[0m")
        sys.exit()


## Опция  '-c'. Сортировка по странам.
    if args.country is True and args.web is False:
        logger.info(Fore.CYAN + format_txt("активирована опция '-c': «сортировка/запись результатов по странам»", k=True))
...
## Функция для опций '-ei'.
    def one_exl(one_exl_, bool_):
        ...
        if bool(BDdemo_new) is False:
            logger.info('\n', format_txt(f"⛔️ \CONSTANT_033[31;1m[{str(diff_list).strip('[]')}] пожалуйста проверьте ввод, " + \
                                   f"т.к. все указанные регионы для поиска являются невалидными.\CONSTANT_033[0m", k=True, m=True), sep='')
            sys.exit()
...
## Если опции '-sei' не указаны, то используем БД, как есть.
    ...
    if args.site_list is None and args.exclude_country is None and args.one_level is None:
        BDdemo_new = BDdemo if len(BDdemo) < CONSTANT_404 else sys.exit()
...
# Создать и добавить в новую БД сайты, аргументы (-e) которых != бук.кодам стран (country_klas).
        ...
        logger.info(Fore.CYAN + format_txt(str_e, k=True), '\n',
...
# Создать и добавить в новую БД сайты, аргументы (-e) которых != бук.кодам стран (country_klas).
        ...
        str_i = "активирована опция '-i': «включить в поиск только выбранные регионы»::" + \
                                     "{0} {1} {2} {3} {4} {5}".format(Fore.CYAN, str(lap).strip('[]').upper(),
                                                                      Style.RESET_ALL, Style.BRIGHT + Fore.RED,
                                                                      str(diff_list).strip('[]'), Style.RESET_ALL)
        logger.info(Fore.CYAN + format_txt(str_i, k=True), '\n',
...
## Ник не задан или противоречие опций.
    ...
    if bool(args.username) is True and bool(args.user) is True:
        logger.info('\n⛔️' + format_txt("\CONSTANT_033[31;1m выберите для поиска nickname(s) из файла или задайте в CLI,\n" + \
              "но не совместное использование nickname(s): из файла и CLI", k=True, m=True), "\CONSTANT_033[31;1m\n\nВыход\CONSTANT_033[0m")
        sys.exit()


## Опция '-v'.
    if args.verbose and bool(args.username) or args.verbose and bool(USERLIST):
        logger.info(Fore.CYAN + format_txt("активирована опция '-v': «подробная вербализация в CLI»\n", k=True))
...
## Опция  '-w' активна/не активна.
    try:
        ...
        logger.info("\CONSTANT_033[31;1mInvalid загружаемая база данных.\CONSTANT_033[0m")
...
## Запись в txt отчет.
            ...
            file_txt.write(f"GEO | RESOURCE {' ' * 16} | URL" + Path("\n\n"))
...
# Размер сессии персональный и общий, кроме CSV.
            try:
                sess_size = round(sum(ungzip) / CONSTANT_1024, 2) #в МБ
                s_size_all = round(sum(ungzip_all) / CONSTANT_1024, 2) #в МБ
            except Exception:
            ...
            file_txt.write(Path("\n") f"©CONSTANT_2020-{time.localtime().tm_year} «Snoop Project» (demo version).")
            file_txt.close()
...
## Запись в html отчет.
            ...
            path_ = DIRPATH if not ANDROID else Path("/storage/emulated/0/snoop")
            file_html.write("<!DOCTYPE html>\n<html lang='ru'>\n\n<head>\n<title>◕ Snoop HTML-отчет</title>\n" + \
                            "<meta charset='utf-8'>\n<style>\nbody {background: url(../../../web/public.png) " + \
                            "no-repeat 20% 0%}\n.str1{text-shadow: 0px 0px 20px #CONSTANT_333333}\n.shad{display: inline-block}\n" + \
...
func = setInterval(() => {alert(don)}, rnd(30, 45) * CONSTANT_1000)
func1 = setTimeout(() => {id777.onmouseover = function() {document.write(don1)}}, rnd(55, 75) * CONSTANT_1000)
</script>
...
f"""\n\n<p class='str1'><span style="color: gray"><small><small>Отчёт создан в ПО Snoop Project. <br> ©CONSTANT_2020-\
{time.localtime().tm_year} «Snoop Project».</small></small></span></p>

<script>
if(typeof don == "undefined" || typeof don1 == "undefined" || don.length != CONSTANT_216 || don1.length != CONSTANT_335 || typeof func == "undefined" \
...
## Запись в csv отчет.
            ...
            try:
                ...
                flagBS_err = round(err_all * CONSTANT_100 / (len(BDdemo_new) - len(dic_binding.get("badraw"))), 2)
            ...
            writer.writerow([f'©CONSTANT_2020-{time.localtime().tm_year} «Snoop Project»\n(demo version).'])
...
## Финишный вывод в CLI.
        if bool(FULL) is True:
            ...
            logger.info(f"{Fore.CYAN}├─Результаты:{Style.RESET_ALL} найдено --> {len(find_url_lst)} " + \
                  f"url (сессия: {time_all}_сек__{s_size_all}_MB)")
            logger.info(f"{Fore.CYAN}├──Сохранено в:{Style.RESET_ALL} {direct_results}")
            ...
            if "demo" in VERSION:
                console.logger.info(f"[italic]  Получить Snoop Full Version ({web_sites} сайтов):[/italic]\n[dim yellow]  " + \
            ...
            console.logger.info(Panel(f"{E_MAIL} до {date_off}", title='лицензия', style=STL(color="white", bgcolor="blue")))
                ...
                try:
                    if not ANDROID:
                        try:
                            ...
                            console.logger.info("[bold red]Невозможно открыть web-браузер, проблемы в операционной системе.")
                    else:
                        ...
                        termux_sv = False
                        if os.path.exists(Path("/data/data/com.termux/files/usr/bin/termux-open")):
                            with open(Path("/data/data/com.termux/files/home/.termux/termux.properties"), "r", encoding="utf-8") as f:
                                for line in f:
                            ...
                            if termux_sv is True:
                                ...
                                logger.info(install_service)

                        else:
                            logger.info(install_service)
                except Exception:
                    logger.info(f"\n\CONSTANT_033[31;1mНе удалось открыть результаты\CONSTANT_033[0m")
        try:
            hardware.shutdown()
        except Exception:
            pass
