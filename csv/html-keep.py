...
class HTMLContentAnalyzer:
    ...
    def analyze_html_content(self, file_path):
        ...
        try:
            ...
            file_info["analysis"] = {
                ...
                "personal_indicators": self._count_patterns(content, self.patterns["personal_content"]),
                "generated_indicators": self._count_patterns(content, self.patterns["generated_content"]),
                "documentation_indicators": self._count_patterns(content, self.patterns["documentation"]),
                "template_indicators": self._count_patterns(content, self.patterns["templates"]),
                "duplicate_indicators": self._count_patterns(content, self.patterns["duplicate_indicators"]),
    ...
    def _is_boilerplate(self, content):
        ...
        structure_count = sum(len(re.findall(pattern, content, re.IGNORECASE)) for pattern in boilerplate_indicators)
    ...
    def classify_file(self, file_info):
        ...
        if analysis["generated_indicators"] > 2 or "documentation_" in file_info["name"].lower():
    ...
    def _generate_summary(self):
        ...
        total_size = sum(f["size"] for f in [f for files in self.analysis_results.values() for f in files])

        logger.info(f"💾 SPACE ANALYSIS")
        logger.info(f"   Total files: {total_files}")
        logger.info(f"   Removable files: {len(removable_files)}")
        logger.info(f"   Space that can be freed: {total_removable_size / (CONSTANT_1024*CONSTANT_1024):.1f} MB")
        logger.info(f"   Percentage removable: {(len(removable_files)/total_files)*CONSTANT_100:.1f}%")
            ...
            for category, files in self.analysis_results.items():
                for file_info in files:
                    row = {
                        ...
                        "personal_indicators": file_info["analysis"]["personal_indicators"],
                        "generated_indicators": file_info["analysis"]["generated_indicators"],
                        "documentation_indicators": file_info["analysis"]["documentation_indicators"],
                        "template_indicators": file_info["analysis"]["template_indicators"],
                        "duplicate_indicators": file_info["analysis"]["duplicate_indicators"],
                        "is_minimal": file_info["analysis"]["is_minimal"],
                        "is_boilerplate": file_info["analysis"]["is_boilerplate"],
                        "has_meaningful_content": file_info["analysis"]["has_meaningful_content"],
                        "modified_date": datetime.fromtimestamp(file_info["modified"]).isoformat(),
                    }
                    writer.writerow(row)
    ...
    def _create_removal_recommendations(self, timestamp):
        ...
        with open(rec_file, "w") as f:
            ...
            removable_categories = ["removable", "duplicates", "generated_content", "templates"]
            ...
            # Summary
            total_files = sum(len(files) for files in self.analysis_results.values())
            removable_files = sum(len(self.analysis_results[cat]) for cat in removable_categories)
            ...
            f.write(f"- **Removal percentage**: {(removable_files/total_files)*CONSTANT_100:.1f}%\n\n")
...
def main():
    ...
    logger.info(f"\n🎯 ANALYSIS COMPLETE!")
    logger.info(f"Use the generated reports to make informed decisions about what to keep vs remove.")
