...
class FluidDocumentSweeper:
    ...
    def __init__(self, root_dir: str = Path("/Users/steven/Documents")):
        ...
        self.quality_keywords = {
            'high': ['class', 'def', 'import', 'try', 'except', 'async', 'await', 'type', 'dataclass'],
        ...
        self.utility_patterns = [
            r'organize', r'sort', r'clean', r'process', r'convert', r'generate',
            r'analyze', r'parse', r'extract', r'merge', r'split', r'filter',
            r'backup', r'sync', r'upload', r'download', r'install', r'setup'
        ]
        
        # AI/ML patterns
        self.ai_ml_patterns = [
            r'ml', r'machine.learning', r'neural', r'tensorflow', r'pytorch',
            r'scikit', r'pandas', r'numpy', r'matplotlib', r'seaborn',
            r'openai', r'huggingface', r'transformers', r'llm', r'gpt',
            r'claude', r'gemini', r'embedding', r'vector', r'prompt'
        ]
    ...
    def analyze_file(self, file_path: Path) -> Optional[ScriptInfo]:
        ...
        try:
            ...
            if extension == '.py':
                return self._analyze_python_file(file_path, content, size, last_modified, category)
            else:
                return self._analyze_other_file(file_path, content, size, last_modified, category, extension)
                
        except Exception as e:
            logger.info(f"Error analyzing {file_path}: {e}")
            return None

    def _analyze_python_file(self, file_path: Path, content: str, size: int, last_modified: str, category: str) -> ScriptInfo:
        ...
        is_utility = any(re.search(pattern, file_path.name.lower()) for pattern in self.utility_patterns)
        is_automation = any(word in content.lower() for word in ['automate', 'schedule', 'cron', 'bot', 'script'])
        is_analysis = any(word in content.lower() for word in ['analyze', 'analysis', 'data', 'statistics', 'plot', 'chart'])
        is_web = any(word in content.lower() for word in ['flask', 'django', 'fastapi', 'requests', 'http', 'api', 'web'])
        is_data = any(word in content.lower() for word in ['pandas', 'numpy', 'csv', 'json', 'sql', 'database'])
        is_ai_ml = any(re.search(pattern, content.lower()) for pattern in self.ai_ml_patterns)
        
        # Generate description
        description = self._generate_description(file_path, content, is_utility, is_automation, is_analysis)
        
        # Generate tags
        tags = self._generate_tags(file_path, content, is_utility, is_automation, is_analysis, is_web, is_data, is_ai_ml)
        
        # Calculate quality score
        quality_score = self._calculate_quality_score(content, has_docstring, has_imports, has_functions, has_classes, has_main)
    ...
    def _analyze_other_file(self, file_path: Path, content: str, size: int, last_modified: str, category: str, extension: str) -> Optional[ScriptInfo]:
        ...
        is_utility = any(re.search(pattern, file_path.name.lower()) for pattern in self.utility_patterns)
        is_automation = any(word in content.lower() for word in ['automate', 'schedule', 'cron', 'bot', 'script'])
        is_analysis = any(word in content.lower() for word in ['analyze', 'analysis', 'data', 'statistics'])
        is_web = any(word in content.lower() for word in ['html', 'css', 'javascript', 'web', 'api'])
        is_data = any(word in content.lower() for word in ['csv', 'json', 'xml', 'sql', 'database'])
        is_ai_ml = any(re.search(pattern, content.lower()) for pattern in self.ai_ml_patterns)
    ...
    def _calculate_quality_score(self, content: str, has_docstring: bool, has_imports: bool, 
                                has_functions: bool, has_classes: bool, has_main: bool) -> int:
        """Calculate quality score based on code structure"""
        score = 0
        if has_docstring: score += 20
        if has_imports: score += 10
        if has_functions: score += 15
        if has_classes: score += 20
        if has_main: score += 10
        
        # Check for good practices
        if 'if __name__ == "__main__"' in content: score += 10
        if 'try:' in content and 'except' in content: score += 10
        if 'def main(' in content: score += 5
        
        # Check for bad practices
        if 'print(' in content and 'logging' not in content: score -= 5
        if 'TODO' in content: score -= 5
        if 'FIXME' in content: score -= 5
        
        return max(0, min(score, CONSTANT_100))

    def _generate_description(self, file_path: Path, content: str, is_utility: bool, 
                            is_automation: bool, is_analysis: bool) -> str:
    ...
    def _generate_tags(self, file_path: Path, content: str, is_utility: bool, 
                      is_automation: bool, is_analysis: bool, is_web: bool, 
                      is_data: bool, is_ai_ml: bool) -> List[str]:
        """Generate tags for the script"""
        tags = []
        
        if is_utility: tags.append('utility')
        if is_automation: tags.append('automation')
        if is_analysis: tags.append('analysis')
        if is_web: tags.append('web')
        if is_data: tags.append('data')
        if is_ai_ml: tags.append('ai-ml')
        
        # Add extension-based tags
        ext = file_path.suffix.lower()
        if ext == '.py': tags.append('python')
        elif ext in ['.sh', '.bash']: tags.append('shell')
        elif ext in ['.js', '.ts']: tags.append('javascript')
        elif ext == '.html': tags.append('html')
        elif ext == '.css': tags.append('css')
        elif ext == '.json': tags.append('json')
        elif ext == '.md': tags.append('markdown')
        
        return tags

    def sweep_documents(self, max_depth: int = 6) -> List[ScriptInfo]:
        """Perform the fluid sweep of documents directory"""
        logger.info(f"🔍 Starting fluid sweep of {self.root_dir} (max depth: {max_depth})")
        ...
        logger.info(f"✅ Sweep complete! Found {len(interesting_files)} interesting files out of {total_files} total")
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        ...
        skip_patterns = [
            ...
            'node_modules', '.DS_Store', 'Thumbs.db', '.pyc',
    ...
    def _is_interesting(self, script_info: ScriptInfo) -> bool:
        ...
        if script_info.size < CONSTANT_100 or script_info.size > 10 * CONSTANT_1024 * CONSTANT_1024:  # CONSTANT_100 bytes to 10MB
            return False
    ...
    def generate_report(self, scripts: List[ScriptInfo]) -> str:
        ...
        for script_type in types:
            ...
            if type_scripts:
                ...
                for script in sorted(type_scripts, key=lambda x: x.quality_score, reverse=True)[:10]:
                    report.append(f"- **{script.name}** - {script.description}")
                report.append("")
        
        return Path("\n").join(report)

    def save_results(self, scripts: List[ScriptInfo], output_dir: str = Path("/Users/steven")):
        ...
        with open(csv_path, 'w', encoding='utf-8') as f:
            f.write("name,path,extension,size,lines,quality_score,category,tags,description,is_utility,is_automation,is_analysis,is_web,is_data,is_ai_ml\n")
            for script in scripts:
                tags_str = ';'.join(script.tags)
                f.write(f'"{script.name}","{script.path}","{script.extension}",{script.size},{script.lines},{script.quality_score},"{script.category}","{tags_str}","{script.description}",{script.is_utility},{script.is_automation},{script.is_analysis},{script.is_web},{script.is_data},{script.is_ai_ml}\n')
...
def main():
    ...
    if scripts:
        ...
        for i, script in enumerate(top_10, 1):
            logger.info(f"{i:2d}. {script.name} (Score: {script.quality_score}) - {script.description}")
    else:
