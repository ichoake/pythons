...
class DownloadsOrganizer:
    ...
    def analyze_directory_structure(self) -> Dict:
        ...
        for root, dirs, files in os.walk(self.downloads_path):
            ...
            analysis["directories"][rel_root] = {"file_count": len(files), "subdir_count": len(dirs), "files": files}
        ...
        for file_hash, file_list in self.duplicates.items():
            if len(file_list) > 1:
                analysis["duplicate_files"].append({"hash": file_hash, "files": file_list, "count": len(file_list)})
                ...
                duplicates.append(
                    {
                        ...
                        "remove_files": [f["path"] for f in file_info[1:]],  # Remove older
                    }
                )
    ...
    def categorize_content(self) -> Dict[str, List[str]]:
        ...
        tech_extensions = {".py", ".js", ".html", ".css", ".sh", ".yml", ".yaml", ".json", ".xml"}
        tech_keywords = ["script", "code", "technical", "development", "programming", "automation"]

        creative_extensions = {".png", ".jpg", ".jpeg", ".gif", ".svg", ".pdf", ".psd", ".ai", ".mp4", ".mp3", ".wav"}
        creative_keywords = ["creative", "design", "art", "image", "video", "audio", "story", "narrative"]
        ...
        web_keywords = ["web", "website", "business", "shop", "notion", "quantumforge", "avatararts"]

        doc_extensions = {".pdf", ".doc", ".docx", ".txt", ".md", ".rtf"}
        doc_keywords = ["document", "conversation", "chat", "export", "backup"]

        media_extensions = {".mp4", ".avi", ".mov", ".mp3", ".wav", ".flac", ".png", ".jpg", ".jpeg", ".gif"}
        ...
        for file_path in self.file_hashes.values():
            ...
            if any(keyword in file_name for keyword in ai_keywords) or file_ext in ai_extensions:
                categories["ai_content"].append(file_path)
            elif any(keyword in file_name for keyword in tech_keywords) or file_ext in tech_extensions:
                categories["technical_resources"].append(file_path)
            elif any(keyword in file_name for keyword in creative_keywords) or file_ext in creative_extensions:
                categories["creative_projects"].append(file_path)
            elif any(keyword in file_name for keyword in data_keywords) or file_ext in data_extensions:
                categories["data_analytics"].append(file_path)
            elif any(keyword in file_name for keyword in web_keywords) or file_ext in web_extensions:
                categories["web_business"].append(file_path)
            elif any(keyword in file_name for keyword in doc_keywords) or file_ext in doc_extensions:
            ...
            elif any(keyword in file_name for keyword in config_keywords) or file_ext in config_extensions:
        ...
        for category, files in categories.items():
            ...
            for file_path in files:
                plan["file_movements"].append(
                    {"source": file_path, "destination": f"{target_dir}/{Path(file_path).name}", "category": category}
                )
            ...
            plan["duplicate_actions"].append(
                {
                    ...
                    "savings_mb": round(dup["total_size"] / (CONSTANT_1024 * CONSTANT_1024), 2),
                }
            )
        ...
        consolidation_opportunities = [
            {
                ...
                "files": [f for f in self.analysis_data.get("similar_names", {}).get("chatgpt_conversation", [])],
                "action": "merge_into_single_archive",
            },
            {
                "type": "ideogram_pdfs",
                "files": [f for f in self.analysis_data.get("similar_names", {}).get("ideogram_copy", [])],
                "action": "organize_by_date",
            },
            {
                "type": "python_scripts",
                "files": [f for f in self.analysis_data.get("similar_names", {}).get("scripty", [])],
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- Total Directories: {self.analysis_data.get('total_directories', 0):,}")
        report.append(
            f"- Total Size: {self.analysis_data.get('total_size', 0) / (CONSTANT_1024*CONSTANT_1024*CONSTANT_1024):.2f} GB"
        )
        report.append(f"- Duplicate Files: {len(self.analysis_data.get('duplicate_files', []))}")
        report.append(f"- Empty Directories: {len(self.analysis_data.get('empty_directories', []))}")
        report.append("")
        ...
        report.append(f"- Potential Space Savings: {total_duplicate_size / (CONSTANT_1024*CONSTANT_1024):.2f} MB")
        report.append("")

        for i, dup in enumerate(duplicates[:5], 1):  # Show top 5
            report.append(f"### Duplicate Group {i}")
            report.append(f"- Files: {dup['count']}")
            report.append(f"- Total Size: {dup['total_size'] / (CONSTANT_1024*CONSTANT_1024):.2f} MB")
    ...
    def run_full_analysis(self):
        ...
        return {
            ...
            "total_size_gb": self.analysis_data.get("total_size", 0) / (CONSTANT_1024 * CONSTANT_1024 * CONSTANT_1024),
        }
