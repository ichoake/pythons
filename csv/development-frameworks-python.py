    ...
    try:
        with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for i, line in enumerate(lines):
                ...
                if any(
                    indicator in line
                    for indicator in ["try:", "except:", "finally:", "with ", "lambda ", "list(", "dict(", "set("]
                ):
...
def categorize_file(filename, filepath):
    ...
    if any(pattern in filename_lower for pattern in ["data", "process", "analyze", "parse"]):
    ...
    if any(pattern in filename_lower for pattern in ["web", "api", "server", "flask", "django"]):
...
def main():
                    ...
                    metadata.update(
                        {
                            ...
                            "top_imports": "|".join(analysis["imports"][:5]),  # Top 5 imports
                        }
                    )
    ...
    for file_hash, paths in file_hashes.items():
        if len(paths) > 1:
            duplicate_groups.append({"hash": file_hash, "count": len(paths), "files": paths})
    ...
    # Show largest files
    logger.info(f"\nLargest files:")
    largest_files = sorted(metadata_list, key=lambda x: x["size_bytes"], reverse=True)[:10]
