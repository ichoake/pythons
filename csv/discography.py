...
This module provides functionality for discography.
...
class DiscographyMerger:
            ...
            for std_col, possible_cols in standard_columns.items():
                for col in possible_cols:
                    if col in self.nocturne_data.columns:
                        self.nocturne_data = self.nocturne_data.rename(columns={col: std_col})
                        break
    ...
    def handle_similar_titles(self):
        ...
        self.merged_data["normalized_title"] = self.merged_data["song_title"].str.lower().str.strip()
        ...
        if len(potential_duplicates) > 0:
            logger.info(f"🔍 Found {len(potential_duplicates)} potential duplicates to review:")
    ...
    def add_api_hooks(self):
        ...
        def generate_api_hook(row):
            ...
            if row.get("source") == "Suno":
                if pd.notna(row.get("url")):
                    hooks.append("SunoTransmuter API: Extract metadata and generate promos")
                if pd.notna(row.get("lyrics")):
                    hooks.append("Lyrics Alchemy API: Process lyrics for content generation")
                if pd.notna(row.get("genre")):
                    hooks.append("Genre Classifier API: Categorize and tag music styles")

            # NocTurnE-specific hooks
            elif row.get("source") == "NocTurnE":
                if pd.notna(row.get("analysis")):
                    hooks.append("Analysis Transmuter API: Convert analysis to promotional content")
                if pd.notna(row.get("promo_content")):
                    hooks.append("Promo Generator API: Auto-create YouTube descriptions")
                if pd.notna(row.get("witty_promo")):
                    hooks.append("Witty Content API: Generate viral social media content")
        ...
        self.merged_data["api_hooks"] = self.merged_data.apply(generate_api_hook, axis=1)
    ...
    def generate_summary_stats(self):
        ...
        stats = {
            ...
            "tracks_with_urls": (
                len(self.merged_data[self.merged_data["url"].notna()]) if "url" in self.merged_data.columns else 0
            ),
            "tracks_with_lyrics": (
                len(self.merged_data[self.merged_data["lyrics"].notna()]) if "lyrics" in self.merged_data.columns else 0
            ),
    ...
    def export_to_formats(self, output_dir):
        ...
        api_data = self.merged_data[["song_title", "genre", "duration", "url", "source", "api_hooks"]].copy()
