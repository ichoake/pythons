"""
Advanced Duplicate Remover

This module provides functionality for advanced duplicate remover.
...
class AdvancedDuplicateRemover:
    ...
    def find_exact_duplicates(self) -> dict:
        ...
        exact_duplicates = {hash_val: files for hash_val, files in hash_groups.items() if len(files) > 1}
    ...
    def find_name_pattern_duplicates(self) -> dict:
        ...
        pattern_duplicates = {base: files for base, files in name_groups.items() if len(files) > 1}

        logger.info(f"   Found {len(pattern_duplicates)} groups of name pattern duplicates")
    ...
    def remove_duplicates(self, dry_run: bool = True) -> dict:
        ...
        for file_group in similar_files:
            ...
            logger.info(f"   Group: {best_file.name} (keeping best of {len(file_group)})")
            for file_to_remove in files_to_remove:
                logger.info(f"      🗑️ Remove: {file_to_remove.name}")
    ...
    def save_changes_csv(self) -> str:
        ...
        csv_file = f"/Users/steven/Documents/python/advanced_duplicate_cleanup_{timestamp}.csv"
