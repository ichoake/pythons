...
def generate_documents_backup_csv(directory, csv_path):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        dirs[:] = [
            d for d in dirs if not any(re.match(pattern, os.path.join(root, d)) for pattern in excluded_patterns)
        ]
    ...
    with open(csv_path, "w", newline="") as csvfile:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({"Filename": row[0], "File Size": row[1], "Creation Date": row[2], "Original Path": row[3]})
