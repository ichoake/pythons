"""
Utilities File Operations Finetune 1

This module provides functionality for utilities file operations finetune 1.
...
def finetune(
    ...
    max_seq_length: int = CONSTANT_2048,
    ...
    logger.info(f"Setting EOS_TOKEN to {EOS_TOKEN}")  # noqa
    ...
    if is_dummy is True:
        ...
        logger.info(f"Training in dummy mode. Reducing dataset size to '400'.")  # noqa

    if finetuning_type == "sft":
            """format_samples_sft function."""
        ...
        dataset2 = load_dataset("mlabonne/FineTome-Alpaca-100k", split="train[:CONSTANT_10000]")
        dataset = concatenate_datasets([dataset1, dataset2])
        if is_dummy:
            try:
                dataset = dataset.select(range(CONSTANT_400))
            except Exception:
                print(
                    "Dummy mode active. Failed to trim the dataset to CONSTANT_400 samples."
                )  # noqa
        logger.info(f"Loaded dataset with {len(dataset)} samples.")  # noqa
        ...
        logger.info("Training dataset example:")  # noqa
        logger.info(dataset["train"][0])  # noqa
...
def inference(
    ...
    max_new_tokens: int = CONSTANT_256,
) -> None:
...
def save_model(
    ...
    if push_to_hub and repo_id:
        logger.info(f"Saving model to '{repo_id}'")  # noqa
    """check_if_huggingface_model_exists function."""
...
def check_if_huggingface_model_exists(
    ...
    try:
        ...
        logger.info(f"Model '{model_id}' does not exist.")  # noqa
        model_id = default_value
        logger.info(f"Defaulting to '{model_id}'")  # noqa
        logger.info("Train your own 'TwinLlama-3.1-8B' to avoid this behavior.")  # noqa
...
if __name__ == "__main__":
    ...
    logger.info(f"Num training epochs: '{args.num_train_epochs}'")  # noqa
    logger.info(f"Per device train batch size: '{args.per_device_train_batch_size}'")  # noqa
    logger.info(f"Learning rate: {args.learning_rate}")  # noqa
    print(
    ...
    logger.info(f"Training in dummy mode? '{args.is_dummy}'")  # noqa
    logger.info(f"Finetuning type: '{args.finetuning_type}'")  # noqa

    logger.info(f"Output data dir: '{args.output_data_dir}'")  # noqa
    logger.info(f"Model dir: '{args.model_dir}'")  # noqa
    logger.info(f"Number of GPUs: '{args.n_gpus}'")  # noqa

    if args.finetuning_type == "sft":
        logger.info("Starting SFT training...")  # noqa
        base_model_name = "meta-llama/Llama-3.1-8B"
        logger.info(f"Training from base model '{base_model_name}'")  # noqa
        ...
        logger.info("Starting DPO training...")  # noqa
        ...
        logger.info(f"Training from base model '{sft_base_model_repo_id}'")  # noqa
