"""
Utilities File Operations Message 6

This module provides functionality for utilities file operations message 6.
...
async def unzip(bot, update):
    ...
    if (
        ...
        try:
            ...
            extract_dir_path = Config.DOWNLOAD_LOCATION + "/" + str(update.from_user.id) + "zipped" + "/"
            ...
            try:
                ...
                for current_file in zip_file_contents:
                    ...
                    inline_keyboard.append(
                        [pyrogram.InlineKeyboardButton(current_file, callback_data=cb_string.encode("UTF-8"))]
                    )
                    i = i + 1
                cb_string = "ZIP:{}:ZIP".format("ALL")
                inline_keyboard.append(
                    [pyrogram.InlineKeyboardButton("Upload All Files", callback_data=cb_string.encode("UTF-8"))]
                )
                cb_string = "ZIP:{}:ZIP".format("NONE")
                inline_keyboard.append(
                    [pyrogram.InlineKeyboardButton("Cancel", callback_data=cb_string.encode("UTF-8"))]
                )
