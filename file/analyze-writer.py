...
class SimpleQualityMonitor:
        ...
        if self.analyzer_path.exists():
            try:
                ...
                if result.returncode == 0:
                    ...
                    metrics.maintainability_score = summary.get("average_maintainability_score", 0)
                    metrics.performance_potential = summary.get("average_performance_potential", 0)
                    metrics.domains_detected = summary.get("domains_detected", [])
                    metrics.patterns_detected = summary.get("patterns_detected", [])
                    metrics.anti_patterns_detected = summary.get("anti_patterns_detected", [])
                    metrics.improvement_opportunities = summary.get("total_improvements_generated", 0)
    ...
    def _generate_report(self, metrics: QualityMetrics) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(f"- **Min Docstring Coverage:** {thresholds['min_docstring_coverage']}%\n")
            f.write(f"- **Min Type Hint Coverage:** {thresholds['min_type_hint_coverage']}%\n")
            f.write(f"- **Min Error Handling Coverage:** {thresholds['min_error_handling_coverage']}%\n")
            f.write(f"- **Min Logging Coverage:** {thresholds['min_logging_coverage']}%\n")
