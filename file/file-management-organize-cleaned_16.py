...
__author__ = "Marwan CONSTANT_007 : @mrwn.007"
__license__ = "GPLv3"
__version__ = "0.1"
...
from libs.utils import (CheckPublicIP, GetInput, IsProxyWorking, LoadProxies,
                        LoadUsers, PrintBanner, PrintChoices, PrintError,
                        PrintFatalError, PrintStatus, PrintSuccess)
...
def MultiThread(username, userid, loginuser, loginpass, proxy, reasonid):
    ...
    if proxy != None:
        ...
        client = InstaClient(loginuser, loginpass, proxy["ip"], proxy["port"])
    else:
        PrintStatus("[" + loginuser + "]", "Logging into the Account!")
        client = InstaClient(loginuser, loginpass, None, None)
...
def NoMultiThread():
    for user in USERS:
        ...
        if useproxy:
            ...
            client = InstaClient(
                user["user"], user["password"], proxy["ip"], proxy["port"]
            )
        else:
            proxy = choice(PROXIES)
            PrintStatus("[" + user["user"] + "]", "Logging into the Account!")
            client = InstaClient(user["user"], user["password"], None, None)
