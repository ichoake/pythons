...
async def scrape_feed(page, feed_url, max_scrolls, max_pages=3):
    ...
    if "Log In" in await page.title() or "Sign In" in await page.content()[:CONSTANT_800]:
        logger.info(f"[{feed_url}] If you're not logged in, please log in manually and then press Enter here.")
        input()
    ...
    for page_num in range(1, max_pages + 1):
        ...
        try:
            ...
            logger.info(f"[{feed_url}] Warning: job tiles might not have appeared on page {page_num}.")

        cards = await page.query_selector_all(JOB_TILE_SELECTOR)
        logger.info(f"[{feed_url}] [page {page_num}] Found {len(cards)} potential job cards.")
        ...
        for card in cards:
            try:
                ...
                location = ""
                loc_el = await card.query_selector('[data-test="client-location"]') or await card.query_selector(
                    ".up-client-info"
                )
                if loc_el:
                    location = (await loc_el.inner_text()).strip()

                desc_el = await card.query_selector('[data-test="job-description"]') or await card.query_selector(
                    ".up-line-clamp"
                )
        ...
        for sel in selectors_to_try:
            try:
                ...
                if nxt:
                    ...
                    next_clicked = True
                    logger.info(f"[{feed_url}] Clicked pagination control '{sel}' on page {page_num}.")
                    break
            except Exception:
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default=Path("/Users/steven/upwork"), help="Output directory for CSVs")
