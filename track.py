...
class Track:
    def try_with_key_error(
        self, name: str, getter: Callable, default: str = ""
    ) -> None:
    ...
    def __init__(self, spotify_data, track_type=Type.TRACK) -> None:
        ...
        try:
            ...
            try:
                ...
                self.album_name = (
                    "Unknown Show" if track_type == Type.EPISODE else "Unknown Album"
                )
        ...
        self.try_with_key_error(
            "album_track_count", lambda: spotify_data["album"]["total_tracks"]
        )

        self.try_with_key_error(
            "release_date", lambda: spotify_data["album"]["release_date"]
        )
...
def _spotify_artist_names(artist_data) -> list:
    """_spotify_artist_names function."""
