...
".join(transcript_with_timestamps)
    ...
    def generate_quiz_from_transcript(
        self, transcript: str, num_questions: int = 5
    ) -> str:
...
def main():
    ...
    parser.add_argument(
        "--convert", action="store_true", help="Convert MP4 to MP3 first"
    )
    parser.add_argument(
        "--split", type=int, help="Split audio into segments of specified length"
    )
    parser.add_argument(
        "--quiz", type=int, help="Generate quiz with specified number of questions"
    )
