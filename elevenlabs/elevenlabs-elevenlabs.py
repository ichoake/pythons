...
class AIAgent:
    ...
    def __init__(self):
        ...
        self.base_path = Path(
            Path("/Users/steven/Documents/python/LLM_APPLICATIONS/content_generation")
        )
        ...
        try:
            ...
            for step in workflow_plan["steps"]:
                ...
                try:
                    ...
                    # Quality check
                    if step["step_id"] in [
                        qg["step"] for qg in workflow_plan["quality_gates"]
                    ]:
                        ...
                        min_score = next(
                            qg["min_score"]
                            for qg in workflow_plan["quality_gates"]
                            if qg["step"] == step["step_id"]
                        )

                        if quality_score < min_score:
                            # Trigger improvement or fallback
                            improved_result = self._improve_content(
                                step["step_id"], result, quality_score
                            )
                            execution_log["results"][step["step_id"]] = improved_result
                            execution_log["quality_scores"][step["step_id"]] = (
                                self._assess_quality(step["step_id"], improved_result)
                            )
    ...
    def _execute_step(
        self, step: Dict, request_data: Dict, execution_log: Dict
    ) -> Dict:
    ...
    def _execute_image_generation_step(self, request_data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                "images": [f"generated_image_{i}.png" for i in range(2)],
                "service_used": "stability_ai",
            }
    ...
    def _improve_content(
        self, step_id: str, result: Dict, current_score: float
    ) -> Dict:
    ...
    def _execute_fallback(
        self, step: Dict, request_data: Dict, error: str
    ) -> Optional[Dict]:
            ...
            self.agent_knowledge["performance_history"].append(
                {
                    "timestamp": datetime.now().isoformat(),
                    "workflow_id": workflow_plan["workflow_id"],
                    "duration": execution_log.get("end_time", "")
                    - execution_log.get("start_time", ""),
        ...
        return jsonify(
            {
                ...
                "performance_summary": {
                    ...
                    "average_quality": (
                        sum(
                            exec.get("quality_scores", {}).get("content_generation", 0)
                            for exec in performance_data
                        )
                    ...
                    "error_rate": (
                        sum(1 for exec in performance_data if exec.get("errors"))
                        / len(performance_data)
                        if performance_data
                        else 0
                    ),
    ...
    return jsonify(
        {
            ...
            "knowledge_base_size": len(
                ai_agent.agent_knowledge.get("performance_history", [])
            ),
        }
    )
