...
def generate_requirements_file(
    packages: List[str], output_path: Path, category_name: str = None
):
...
def analyze_category_directories():
    ...
    report = {
        ...
        "categories": {
            name: {"packages": packages, "count": len(packages)}
            for name, packages in category_requirements.items()
        },
    }
    ...
    for i, (pkg, count) in enumerate(
        sorted(package_freq.items(), key=lambda x: x[1], reverse=True)[:15], 1
    ):
...
Examples:
  ...
  python3 generate_requirements.py --dir my_project
    ...
    parser.add_argument(
        "--dir",
        type=str,
        help="Specific directory to analyze (default: all categories)",
    )
