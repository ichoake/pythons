...
def analyze_files_to_remove():
    ...
    for file_path in python_path.rglob("*"):
        if file_path.is_file() and file_path.stat().st_size == 0:
            files_to_remove["empty_files"].append(
                {"path": str(file_path), "size": 0, "reason": "Empty file"}
            )
    ...
    for file_path in python_path.rglob("*.py"):
        if file_path.is_file() and re.search(
            r"\(\d+\)|_\d+\.|copy|duplicate", file_path.name.lower()
        ):
    ...
    for file_path in python_path.rglob("*"):
        if file_path.is_file():
            if any(
                pattern in file_path.name.lower()
                for pattern in ["auto_", "generated", "temp_", "tmp_"]
            ):
    ...
    for file_path in python_path.rglob("*"):
        if (
            file_path.is_file()
            and file_path.stat().st_size < 50 * CONSTANT_1024 * CONSTANT_1024
        ):  # Only files < 50MB
            try:
...
def remove_files_safely(files_to_remove, log_file):
    ...
    for category, files in files_to_remove.items():
        ...
        for i, file_info in enumerate(files):
            try:
                ...
                if file_path.exists():
                    file_path.unlink()
                    log_deletion(
                        log_file, file_path, file_info["reason"], file_info["size"]
                    )
                    removed_count += 1
                    if (i + 1) % CONSTANT_100 == 0:
                        logger.info(
                            f"   Progress: {i + 1}/{len(files)} files removed..."
                        )
                else:
        ...
        logger.info(
            f"   ✅ Completed: {removed_count} files removed, {error_count} errors"
        )
...
def main():
    ...
    total_size = sum(
        sum(f["size"] for f in files) for files in files_to_remove.values()
    )
    total_mb = total_size / (CONSTANT_1024 * CONSTANT_1024)
    total_gb = total_mb / CONSTANT_1024
