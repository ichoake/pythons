...
class RecoveryAnalyzer:
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    if file_hash:
                        ...
                        self.analysis_results["file_hashes"][file_hash].append(
                            file_info
                        )

        self.analysis_results["locations"][location] = {
    ...
    def analyze_duplicates(self):
        ...
        self.analysis_results["unique_files"] = len(
    ...
    def create_recovery_plan(self):
        ...
        for file_info in self.analysis_results["missing_files"]:
            ...
            best_copy = max(
                self.analysis_results["file_hashes"][file_info["hash"]],
                key=lambda x: x["size"],
            )
        ...
        logger.info(
            f"   📄 Files to recover: {len(self.analysis_results['recovery_plan'])}"
        )
    ...
    def run_analysis(self):
        ...
        logger.info(
            f"   📋 Recovery plan: {len(self.analysis_results['recovery_plan'])} files"
        )
