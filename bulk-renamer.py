...
def main():
    ...
    parser.add_argument(
        "--pattern", required=True, help='Glob pattern (e.g., "script_*.py")'
    )
    parser.add_argument("--output", default="rename_plan.csv", help="Output CSV file")
    parser.add_argument(
        "--dry-run", action="store_true", help="Generate plan without applying"
    )
        ...
        results.append(
            {
                ...
                "docstring": (
                    analysis.get("docstring", "")[:CONSTANT_100]
                    if analysis.get("docstring")
                    else ""
                ),
                "comment": (
                    analysis.get("first_comment", "")[:CONSTANT_100]
                    if analysis.get("first_comment")
                    else ""
                ),
    ...
    logger.info(
        f"   Suggestions generated: {suggestions} ({suggestions/len(results)*CONSTANT_100:.1f}%)"
    )
