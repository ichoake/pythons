...
class CodebaseAnalyzer:
    ...
    def find_duplicates(self):
        ...
        logger.info(f"⚠️  Found {len(self.results['duplicates'])} potential duplicate files")
            ...
            try:
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    if lines > CONSTANT_500:
                        self.results["issues"]["large_files"].append({"file": rel_path, "lines": lines})
            except Exception as e:
                self.results["issues"]["read_errors"].append(f"{path}: {str(e)}")

        logger.info(f"⚠️  Bare except clauses: {len(self.results['issues']['bare_except'])}")
        logger.info(f"⚠️  Missing docstrings: {len(self.results['issues']['missing_docstring'])}")
        logger.info(f"⚠️  Large files (>CONSTANT_500 lines): {len(self.results['issues']['large_files'])}")
    ...
    def generate_report(self, output_file="analysis_report.json"):
        ...
        logger.info(f"Bare Except Clauses: {len(self.results['issues']['bare_except'])}")
        logger.info(f"Missing Docstrings: {len(self.results['issues']['missing_docstring'])}")
