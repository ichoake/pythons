...
class IntelligentDeduplicator:
    ...
    def scan_codebase(self):
        ...
        skip_patterns = ['dedup_backup_', 'dedup_reports_', 'merge_backup_',
                        'merge_analysis_', '.git', '__pycache__']
    ...
    def choose_best_file(self, files: List[Path]) -> Tuple[Path, List[Path]]:
        ...
        for filepath in files:
            ...
            if any(folder in path_lower for folder in ['core/', 'automation/', 'media/']):
                score += 10
        ...
        scored_files.sort(key=lambda x: (x['score'], x['analysis'].get('modified', 0)), reverse=True)
    ...
    def create_removal_plan(self):
        ...
        logger.info(f"{Colors.CYAN}Exact duplicate groups: {len(exact_dupes)}{Colors.END}")
        ...
        for file_hash, files in exact_dupes.items():
            ...
            for remove_file in remove_files:
                ...
                self.removal_plan.append({
                    ...
                    'remove_parent': remove_analysis.get('parent_folder', 'unknown'),
        ...
        logger.info(f"{Colors.CYAN}Semantic duplicate groups: {len(semantic_dupes)}{Colors.END}")
        logger.info(f"{Colors.YELLOW}Total files to remove: {self.stats['files_removed']}{Colors.END}")
        logger.info(f"{Colors.GREEN}Space to save: {self.stats['space_saved'] / CONSTANT_1024 / CONSTANT_1024:.2f} MB{Colors.END}")
    ...
    def execute_removal(self):
        ...
        logger.info(f"{Colors.YELLOW}Total files to remove: {len(self.removal_plan)}{Colors.END}")
        logger.info(f"{Colors.CYAN}Mode: {'DRY RUN' if self.dry_run else 'LIVE REMOVAL'}{Colors.END}\n")
        ...
        for idx, item in enumerate(self.removal_plan, 1):
            ...
            logger.info(f"{Colors.RED}Remove: {remove_file.relative_to(self.target_dir)}{Colors.END}")
            logger.info(f"  Parent: {Colors.CYAN}{item['remove_parent']}{Colors.END}")
            logger.info(f"{Colors.GREEN}Keep:   {keep_file.relative_to(self.target_dir)}{Colors.END}")
            ...
            if self.interactive:
                ...
                if 'backup' in str(remove_file).lower() or 'archive' in str(remove_file).lower():
                    proceed = True
                else:
            ...
            try:
                if not self.dry_run:
                    ...
                    backup_path = self.backup_dir / remove_file.relative_to(self.target_dir)
                    backup_path.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(remove_file, backup_path)

                    # Create undo command
                    self.undo_commands.append(
                        f"cp '{backup_path}' '{remove_file}'"
                    )
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"| Exact Duplicate Groups | {self.stats['exact_duplicates']:,} |\n")
            f.write(f"| Semantic Duplicate Groups | {self.stats['semantic_duplicates']:,} |\n")
            f.write(f"| Files Removed | {self.stats['files_removed']:,} |\n")
            f.write(f"| Space Saved | {self.stats['space_saved'] / CONSTANT_1024 / CONSTANT_1024:.2f} MB |\n")
            ...
            for parent in sorted(by_parent.keys()):
                ...
                f.write(f"### 📁 {parent}\n")
                f.write(f"**Removed:** {len(items)} files ({total_size / CONSTANT_1024:.1f} KB)\n\n")

                for item in items:
                    f.write(f"- `{item['remove'].name}`\n")
                    f.write(f"  - **Kept:** `{item['keep'].relative_to(self.target_dir)}`\n")
    ...
    def run(self):
        ...
        logger.info("╔═══════════════════════════════════════════════════════════════════════════════╗")
        logger.info("║                                                                               ║")
        logger.info("║              🧹 INTELLIGENT DUPLICATE REMOVER 🧠                             ║")
        logger.info("║                                                                               ║")
        logger.info("║        Content-Aware Deduplication with Parent Folder Intelligence           ║")
        logger.info("║                                                                               ║")
        logger.info("╚═══════════════════════════════════════════════════════════════════════════════╝")
        ...
        if self.dry_run:
            logger.info(f"{Colors.YELLOW}{Emojis.SHIELD} MODE: DRY RUN (no files will be removed){Colors.END}")
        else:
            logger.info(f"{Colors.RED}{Emojis.WARN} MODE: LIVE REMOVAL (files will be deleted!){Colors.END}")
        ...
        logger.info(f"  {Emojis.FILE} Scanned: {Colors.CYAN}{self.stats['total_files']:,}{Colors.END}")
        logger.info(f"  {Emojis.TARGET} Exact Dupes: {Colors.CYAN}{self.stats['exact_duplicates']:,} groups{Colors.END}")
        logger.info(f"  {Emojis.BRAIN} Semantic Dupes: {Colors.CYAN}{self.stats['semantic_duplicates']:,} groups{Colors.END}")
        logger.info(f"  {Emojis.TRASH} Removed: {Colors.CYAN}{self.stats['files_removed']:,} files{Colors.END}")
        logger.info(f"  💾 Space Saved: {Colors.CYAN}{self.stats['space_saved'] / CONSTANT_1024 / CONSTANT_1024:.2f} MB{Colors.END}\n")

        if self.dry_run:
            logger.info(f"{Colors.YELLOW}{Emojis.WARN} This was a DRY RUN. Use --live to actually remove files.{Colors.END}\n")
        else:
            logger.info(f"{Colors.GREEN}{Emojis.CHECK} Deduplication complete!{Colors.END}\n")
            logger.info(f"{Colors.CYAN}Backups saved to: {self.backup_dir}{Colors.END}\n")
...
def main():
    ...
    parser.add_argument('--live', action='store_true',
                       help='Live mode (actually removes files)')
