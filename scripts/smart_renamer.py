...
class SmartRenamer:
    ...
    REDUNDANT_WORDS = {
        'enhanced_', 'simple_', 'basic_', 'new_', 'old_',
        'fixed_', 'temp_', 'tmp_', 'test_', 'demo_',
    }
    ...
    def generate_smart_name(self, filepath: Path) -> Optional[str]:
        ...
        if (len(original_stem) < 50 and
            not re.search(r'[\(\)\[\]]', original_stem) and
            not re.search(r'\s\d+$', original_stem) and
            'copy' not in original_stem.lower() and
            original_stem.count('_') < 5 and
            not original_stem.endswith(('--', '__'))):
            return None
    ...
    def scan_files(self):
        ...
        skip_patterns = ['backup', '.git', '__pycache__', 'dedup_backup',
                        'rename_backup', 'merge_backup', 'bare_except_backup']
        files_to_check = [f for f in files_to_check
                         if not any(skip in str(f) for skip in skip_patterns)]

        logger.info(f"{Colors.GREEN}Checking {len(files_to_check)} files...{Colors.END}\n")

        for idx, filepath in enumerate(files_to_check, 1):
            if idx % CONSTANT_200 == 0:
                logger.info(f"{Colors.YELLOW}Progress: {idx}/{len(files_to_check)}...{Colors.END}", end='\r')
                ...
                if not self.dry_run:
                    try:
                        ...
                        if new_path.exists() and new_path != item['old_path']:
                            ...
                            while (item['old_path'].parent / f"{base}-v{counter}{ext_name}").exists():
                                counter += 1
                            new_path = item['old_path'].parent / f"{base}-v{counter}{ext_name}"
    ...
    def generate_report(self):
        ...
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Old Name', 'New Name', 'Extension', 'Parent Folder', 'Path'])
    ...
    def run(self):
        ...
        logger.info("╔═══════════════════════════════════════════════════════════════════════════════╗")
        logger.info("║                                                                               ║")
        logger.info("║              🏷️ SMART CONTENT-AWARE RENAMER V2 🧠                           ║")
        logger.info("║                                                                               ║")
        logger.info("║         Intelligent Naming with Style Consistency                            ║")
        logger.info("║                                                                               ║")
        logger.info("╚═══════════════════════════════════════════════════════════════════════════════╝")
