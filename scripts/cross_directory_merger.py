...
class CrossDirectoryMerger:
    ...
    def scan_directories(self):
        ...
        for source_dir in self.source_dirs:
            ...
            if not source_dir.exists():
                logger.info(f"  {Colors.YELLOW}⚠️  Directory not found, skipping{Colors.END}\n")
                continue
    ...
    def create_merge_plan(self):
            ...
            if len(instances) > 1:
                ...
                if len(sources) > 1:
                    ...
                    if not has_master:
                        ...
                        self.stats['files_skipped'] += len([i for i in instances if i['source'] != 'MASTER'])
            else:
                # Unique file
                inst = instances[0]
        ...
        logger.info(f"  Cross-directory duplicates: {Colors.YELLOW}{duplicates_across_dirs}{Colors.END}")
        logger.info(f"  Files to merge: {Colors.CYAN}{self.stats['files_to_merge']}{Colors.END}")
        logger.info(f"  Files to skip (already in master): {Colors.YELLOW}{self.stats['files_skipped']}{Colors.END}")
    ...
    def execute_merge(self):
        ...
        for idx, item in enumerate(self.merge_plan[:20], 1):  # Show first 20
            ...
            logger.info(f"  From: {Colors.CYAN}{source_file.relative_to(source_file.parent.parent.parent)}{Colors.END}")
            logger.info(f"  To:   {Colors.GREEN}{target_path.relative_to(self.master_dir)}{Colors.END}")

            if item['duplicates']:
                logger.info(f"  Duplicates: {len(item['duplicates'])} (will be skipped)")
    ...
    def run(self):
        ...
        logger.info("╔═══════════════════════════════════════════════════════════════════════════════╗")
        logger.info("║                                                                               ║")
        logger.info("║          🔄 CROSS-DIRECTORY INTELLIGENT MERGER 🧠                            ║")
        logger.info("║                                                                               ║")
        logger.info("║    Merge Multiple Directories with Duplicate Detection                       ║")
        logger.info("║                                                                               ║")
        logger.info("╚═══════════════════════════════════════════════════════════════════════════════╝")
        ...
        logger.info(f"  Master files: {Colors.CYAN}{self.stats['master_files']:,}{Colors.END}")
        logger.info(f"  Source files: {Colors.CYAN}{self.stats['source_files']:,}{Colors.END}")
        logger.info(f"  To merge: {Colors.CYAN}{self.stats['files_to_merge']:,}{Colors.END}")
        logger.info(f"  Duplicates found: {Colors.YELLOW}{self.stats['duplicates_found']:,}{Colors.END}")
        logger.info(f"  Skipped (already in master): {Colors.YELLOW}{self.stats['files_skipped']:,}{Colors.END}\n")
...
def main():
    ...
    parser.add_argument('--user-scripts-only', action='store_true', default=True,
