...
class ReorganizationPlanner:
    ...
    def analyze_structure(self):
        ...
        logger.info(f"{Colors.GREEN}Analyzed {self.stats['total_folders']} folders{Colors.END}")
        logger.info(f"{Colors.YELLOW}Found {self.stats['deep_folders']} folders deeper than level {self.max_depth}{Colors.END}\n")
    ...
    def create_flattening_plan(self, folders_by_depth: Dict):
        ...
        logger.info(f"{Colors.GREEN}Created plan for {self.stats['folders_to_move']} folders{Colors.END}")
        logger.info(f"{Colors.YELLOW}Affects {self.stats['files_affected']} Python files{Colors.END}")
    ...
    def generate_reports(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"| Deep Folders (>{self.max_depth}) | {self.stats['deep_folders']:,} |\n")
            ...
            f.write(f"**Goal:** Reduce folder depth from 10 → {self.max_depth} levels\n\n")
            f.write("**Method:**\n")
            ...
            if self.move_plan:
                ...
                for move in self.move_plan[:10]:
                    ...
                    f.write(f"**From:** `{move['old_path'].relative_to(self.target_dir)}` (Level {move['old_depth']})\n")
                    f.write(f"**To:** `{move['new_path'].relative_to(self.target_dir)}` (Level {move['new_depth']})\n")
            ...
            for idx, move in enumerate(self.move_plan, 1):
                f.write(f"{idx}. `{move['old_path'].relative_to(self.target_dir)}` → `{move['new_path'].relative_to(self.target_dir)}`\n")
        ...
        with open(script_file, 'w') as f:
            ...
            for idx, move in enumerate(self.move_plan, 1):
                ...
                f.write(f"echo '[{idx}/{len(self.move_plan)}] Moving {move['files']} files...'\n")
                f.write(f"mkdir -p '{move['new_path']}'\n")
                f.write(f"mv '{move['old_path']}'/* '{move['new_path']}/' 2>/dev/null\n")
    ...
    def run(self):
        ...
        logger.info("╔═══════════════════════════════════════════════════════════════════════════════╗")
        logger.info("║                                                                               ║")
        logger.info("║             🗂️ INTELLIGENT REORGANIZATION PLANNER 🧠                         ║")
        logger.info("║                                                                               ║")
        logger.info("║          Smart Folder Flattening with Content Awareness                      ║")
        logger.info("║                                                                               ║")
        logger.info("╚═══════════════════════════════════════════════════════════════════════════════╝")
        ...
        logger.info(f"  Total Folders: {Colors.CYAN}{self.stats['total_folders']:,}{Colors.END}")
        logger.info(f"  Deep Folders (>{self.max_depth}): {Colors.YELLOW}{self.stats['deep_folders']:,}{Colors.END}")
        logger.info(f"  To Move: {Colors.CYAN}{self.stats['folders_to_move']:,}{Colors.END}")
        logger.info(f"  Files Affected: {Colors.CYAN}{self.stats['files_affected']:,}{Colors.END}\n")
...
def main():
    ...
    parser.add_argument('--max-depth', type=int, default=6, help='Maximum folder depth (default: 6)')
