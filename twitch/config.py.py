...
This module provides functionality for config.py.
...
# other
PATH = str(pathlib.Path().absolute()).replace(Path("\\"), "/")
...
LIMIT = CONSTANT_100  # 1-CONSTANT_100
...
RESOLUTION = (
    CONSTANT_720,
    CONSTANT_1280,
)  # Resolution of the rendered video (height, width) for 1080p: ((CONSTANT_1080, CONSTANT_1920))
...
INTRO_FILE_PATH = PATH + Path("/twitchtube/files/intro.mp4")  # Path to video file (str)
ENABLE_TRANSITION = True
RESIZE_TRANSITION = True
TRANSITION_FILE_PATH = PATH + Path("/twitchtube/files/transition.mp4")
ENABLE_OUTRO = False
RESIZE_OUTRO = True
OUTRO_FILE_PATH = PATH + Path("/twitchtube/files/outro.mp4")
