...
This module provides functionality for ffmpegrobot.
        ...
        if len(commands) == 3:
            ...
            if o is not None:
                await bot.edit_message_text(
                    chat_id=update.chat.id, text=Translation.UPLOAD_START, message_id=a.message_id
                )
                c_time = time.time()
            ...
            await bot.edit_message_text(
                chat_id=update.chat.id, text=Translation.FF_MPEG_RO_BOT_RE_SURRECT_ED, message_id=a.message_id
            )
    else:
        # reply help message
        await bot.send_message(
...
async def clear_media(bot, update):
    ...
    await bot.send_message(
        chat_id=update.chat.id, text=Translation.FF_MPEG_DEL_ETED_CUSTOM_MEDIA, reply_to_message_id=update.message_id
    )
...
async def download_media(bot, update):
    ...
    if not os.path.exists(saved_file_path):
        ...
        try:
            ...
            await bot.edit_message_text(chat_id=update.chat.id, text=str(e), message_id=a.message_id)
        else:
            await bot.edit_message_text(
                chat_id=update.chat.id, text=Translation.SAVED_RECVD_DOC_FILE, message_id=a.message_id
            )
    else:
        await bot.send_message(
            chat_id=update.chat.id,
