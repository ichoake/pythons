...
if len(sys.argv) > 1:
    ...
    base_dir = (
        input("Enter the base directory containing your media files (leave blank to use current directory): ").strip()
        or os.getcwd()
    )
...
def transcribe_audio(file_path):
    ...
    if os.path.getsize(file_path) > MAX_SIZE:
        logging.info(f"File {file_path} exceeds {MAX_SIZE} bytes. Converting to lower bitrate.")
        filename_no_ext = os.path.splitext(os.path.basename(file_path))[0]
        converted_file = os.path.join(os.path.dirname(file_path), f"{filename_no_ext}_converted.mp4")
        try:
            subprocess.run(
                [
                    "ffmpeg",
    ...
    with open(file_path, "rb") as audio_file:
        transcript_data = openai.Audio.transcribe("whisper-1", audio_file, response_format="verbose_json")
    transcript_lines = []
