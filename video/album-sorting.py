...
PROJECT_ROOT = Path("/Users/steven/Movies/PROJECt2025-DoMinIon/Media")
DIRECTORIES = {
...
def analyze_organization(dry_run=False):
    ...
    for idx, base in enumerate(bases, start=1):
        ...
        logger.info(f"[{idx}/{total_bases}] Analyzing base: '{base}' → folder: '{base}/'")
    ...
    logger.info("────────────────────────────────────────")
    print(f"✅ Analysis completed. Files moved: {moved_files}, skipped/missing: {skipped_files}")
    if dry_run:
