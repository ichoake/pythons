...
class AISystemMonitor:
    ...
    def test_removebg(self) -> Dict[str, Any]:
        ...
        try:
            ...
            data = response.json() if response.status_code == CONSTANT_200 else {}
            credits = data.get("data", {}).get("attributes", {}).get("credits", {}).get("total", "N/A")
...
"""
    ...
    def check_health(self):
        ...
        critical_down = [
            name
            for name, result in self.results.items()
            if result.get("status") == "❌" and name in ["OpenAI", "Deepgram", "ElevenLabs"]
        ]

        if critical_down:
            logger.info(f"\n🚨 CRITICAL: {', '.join(critical_down)} is down!")
            self.send_telegram_alert(f"🚨 **CRITICAL ALERT**\n\n{', '.join(critical_down)} is down!\n\n{report}")
