...
def create_pulse_animation(
    ...
    if object_data is None:
        if object_type == "emoji":
            object_data = {"emoji": "❤️", "size": CONSTANT_100}
        elif object_type == "circle":
            object_data = {
                "radius": 50,
                "color": (CONSTANT_255, CONSTANT_100, CONSTANT_100),
            }
    ...
    for i in range(num_frames):
        ...
        if pulse_type == "smooth":
            ...
            if phase < 0.15:
                ...
                # First release
                scale = interpolate(
                    max_scale, min_scale, (phase - 0.15) / 0.10, "ease_in"
                )
            elif phase < 0.35:
                # Second pump (smaller)
                scale = interpolate(
            ...
            if phase < 0.2:
                ...
                scale = interpolate(
                    max_scale, min_scale, (phase - 0.2) / 0.8, "ease_in"
                )
            ...
            if phase < 0.3:
                ...
                scale = interpolate(
                    min_scale, max_scale * 1.1, phase / 0.3, "elastic_out"
                )
            else:
                # Settle back
                scale = interpolate(
                    max_scale * 1.1, min_scale, (phase - 0.3) / 0.7, "ease_out"
                )
        ...
        if object_type == "emoji":
            ...
            draw_emoji_enhanced(
                ...
                position=(
                    center_pos[0] - current_size // 2,
                    center_pos[1] - current_size // 2,
                ),
                size=current_size,
            ...
            draw_text_with_outline(
                ...
                text_color=object_data.get(
                    "text_color", (CONSTANT_255, CONSTANT_100, CONSTANT_100)
                ),
...
def create_breathing_animation(
    ...
    frame_width: int = CONSTANT_480,
    frame_height: int = CONSTANT_480,
    bg_color: tuple[int, int, int] = (CONSTANT_240, CONSTANT_248, CONSTANT_255),
) -> list[Image.Image]:
    """
...
if __name__ == "__main__":
    logger.info("Creating pulse animations...")
    ...
    builder.save("pulse_smooth.gif", num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save("pulse_heartbeat.gif", num_colors=CONSTANT_128)
    ...
    logger.info("Created pulse animations!")
