...
This module provides functionality for morph.
    ...
    for i in range(num_frames):
        ...
        if morph_type == "crossfade":
            ...
            if object_type == "emoji":
                # Create first emoji
                emoji1_canvas = Image.new(
                    "RGBA", (frame_width, frame_height), (0, 0, 0, 0)
                )
                ...
                current_color = tuple(
                    int(interpolate(color1[i], color2[i], t, easing)) for i in range(3)
                )
            ...
            if object_type == "emoji":
                ...
                if scale1 > 0.05:
                    ...
                    draw_emoji_enhanced(
                        ...
                        emoji=object1_data["emoji"],
                        position=(
                            center_pos[0] - size1 // 2,
                            center_pos[1] - size1 // 2,
                        ),
                        size=size1,
                        shadow=False,
                    )
                ...
                if scale2 > 0.05:
                    ...
                    draw_emoji_enhanced(
                        ...
                        emoji=object2_data["emoji"],
                        position=(
                            center_pos[0] - size2 // 2,
                            center_pos[1] - size2 // 2,
                        ),
                        size=size2,
                        shadow=False,
                    )
            ...
            # Calculate rotation (0 to CONSTANT_180 degrees)
            ...
            if object_type == "emoji":
                ...
                emoji_scaled = emoji_canvas.resize(
                    (new_width, canvas_size), Image.LANCZOS
                )
...
if __name__ == "__main__":
    logger.info("Creating morph animations...")

    builder = GIFBuilder(width=CONSTANT_480, height=CONSTANT_480, fps=20)

    # Example 1: Crossfade morph
    frames = create_morph_animation(
        object1_data={"emoji": "😊", "size": CONSTANT_100},
        object2_data={"emoji": "😂", "size": CONSTANT_100},
        num_frames=30,
        morph_type="crossfade",
        object_type="emoji",
    )
    builder.add_frames(frames)
    builder.save("morph_crossfade.gif", num_colors=CONSTANT_128)

    # Example 2: Scale morph
    builder.clear()
    frames = create_morph_animation(
        object1_data={"emoji": "🌙", "size": CONSTANT_100},
        object2_data={"emoji": "☀️", "size": CONSTANT_100},
        num_frames=40,
        morph_type="scale",
        object_type="emoji",
    )
    builder.add_frames(frames)
    builder.save("morph_scale.gif", num_colors=CONSTANT_128)
    ...
    logger.info("Created morph animations!")
