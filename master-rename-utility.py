...
class MasterRenamer:
    ...
    def should_exclude(self, filepath: Path) -> bool:
        ...
        try:
            ...
            if (
                ...
                or size > self.config["max_file_size"]
            ):
                return True
        except OSError:
            return True
    ...
    def analyze_file_content(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            analysis["main_nouns"] = self._extract_nouns(
                content, analysis["main_imports"]
            )
    ...
    def _extract_keywords(self, content: str) -> List[str]:
        ...
        key_terms = [
            word.lower() for word in key_terms if word.lower() not in common_words
        ]
    ...
    def suggest_better_name(self, filepath: Path, analysis: Dict) -> Optional[str]:
        ...
        if analysis.get("api_services") and not any(
            api in name_parts for api in ["yt", "ig"]
        ):
    ...
    def apply_renames(self, suggestions: List[Dict], auto_approve: bool = False) -> int:
        ...
        for suggestion in suggestions:
            ...
            if not auto_approve:
                response = input(
                    f"Rename {current.name} → {suggestion['suggested_name']}? [y/N]: "
                )
            ...
            try:
                ...
                if is_tracked:
                    ...
                    logger.info(
                        f"✅ Renamed (git): {current.name} → {suggestion['suggested_name']}"
                    )
                else:
                    current.rename(new_path)
                    logger.info(
                        f"✅ Renamed: {current.name} → {suggestion['suggested_name']}"
                    )
    ...
    def export_report(self, suggestions: List[Dict], output_path: Path):
        ...
        with open(summary_path, "w") as f:
            ...
            for i, suggestion in enumerate(suggestions, 1):
                f.write(
                    f"{i:3d}. {suggestion['current_name']} → {suggestion['suggested_name']}\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Master Rename Utility - Advanced Python File Renaming System"
    )
    parser.add_argument(
        "path",
        type=Path,
