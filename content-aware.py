...
class ContentAwareDeduplicator:
    ...
    def load_data(self):
        ...
        self.df["Base_Name"] = self.df["Filename"].str.replace(
            r"\.[^.]+$", "", regex=True
        )
            ...
            for i, name1 in enumerate(base_names):
                for j, name2 in enumerate(base_names[i + 1 :], i + 1):
                    ...
                    if similarity > 0.7:  # 70% similarity threshold
                        ...
                        self.similar_files[key].extend(
                            [file1.to_dict(), file2.to_dict()]
                        )
    ...
    def analyze_file_patterns(self):
        ...
        self.analysis_results["common_duplicates"] = common_duplicates.head(
            20
        ).to_dict()
    ...
    def generate_recommendations(self):
        ...
        for filename, files in self.duplicates.items():
            if len(files) > 1:
                ...
                files_sorted = sorted(
                    files, key=lambda x: x["Creation Date"], reverse=True
                )
                        ...
                        recommendations["similar_files"].append(
                            {
                                "group": group_name,
                                "files": size_files,
                                "potential_duplicates": len(size_files),
                            }
                        )

        # Large files
        large_files = self.df[self.df["File Size"] > CONSTANT_100]  # > 100MB
        recommendations["large_files"] = large_files.nlargest(20, "File Size").to_dict(
            "records"
        )

        # Old files (assuming older than 1 year)
        self.df["Creation Date"] = pd.to_datetime(
            self.df["Creation Date"], errors="coerce"
        )
        old_files = self.df[self.df["Creation Date"] < "2024-01-01"]
        recommendations["old_files"] = old_files.nlargest(20, "File Size").to_dict(
            "records"
        )
...
"""
    ...
    def run_analysis(self):
        ...
        logger.info(
            f"Potential space savings: {total_space_saved / (CONSTANT_1024*CONSTANT_1024):.2f} MB"
        )
