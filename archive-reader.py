...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("comprehensive_merge.log"), logging.StreamHandler()],
)
...
class ComprehensiveMerger:
    ...
    def analyze_file_content(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            category = self._determine_category(
                content, file_path.name, file_path.suffix
            )
    ...
    def _determine_category(self, content: str, filename: str, extension: str) -> str:
        ...
        # Configuration files
        if (
            extension in [".txt", ".md", ".log"]
            or "requirements" in filename_lower
        ...
        if extension == ".py":
            ...
            if any(
                ...
                for keyword in ["analyze", "analysis", "analyzer"]
            ):
    ...
    def collect_all_files(self) -> Dict[str, List[Path]]:
        ...
        logger.info(
            f"Collected {self.merge_stats['total_files_found']} files from all sources"
        )
        return all_files

    def find_and_merge_duplicates(
        self, all_files: Dict[str, List[Path]]
    ) -> Dict[str, Path]:
        ...
        for file_hash, file_list in hash_to_files.items():
            if len(file_list) == 1:
                ...
                file_analyses.sort(
                    key=lambda x: (x["quality_score"], x["size"], x["modified"]),
                    reverse=True,
                )
                ...
                logger.info(
                    f"Selected best from {len(file_list)} duplicates: {best_file.name}"
                )

        self.merge_stats["unique_files_kept"] = len(best_files)
        logger.info(
            f"Selected {len(best_files)} unique files from {self.merge_stats['total_files_found']} total files"
        )
    ...
    def organize_files(self, best_files: Dict[str, Path]):
        ...
        for file_hash, file_path in best_files.items():
            try:
                ...
                logger.info(
                    f"Organized {file_path.name} -> {category}/{target_path.name}"
                )
    ...
    def generate_merge_report(self, duplicate_groups: Dict[str, List[Dict]]):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- **Unique files kept:** {self.merge_stats['unique_files_kept']}\n"
            )
            f.write(
                f"- **Duplicates merged:** {self.merge_stats['duplicates_merged']}\n"
            )
            f.write(f"- **Files organized:** {self.merge_stats['files_organized']}\n")
            f.write(
                f"- **Categories created:** {self.merge_stats['categories_created']}\n\n"
            )
            ...
            for file_hash, file_list in duplicate_groups.items():
                ...
                for i, file_analysis in enumerate(file_list):
                    ...
                    f.write(
                        f"- {status} {file_analysis['file_path']} (Quality: {file_analysis['quality_score']})\n"
                    )
                f.write(Path("\n"))
        ...
        analysis_data = {
            ...
            "duplicate_groups": {
                h: [
                    {"path": str(f["file_path"]), "quality": f["quality_score"]}
                    for f in files
                ]
                for h, files in duplicate_groups.items()
            },
            "categories": {
                cat: len(list((self.final_dir / cat).glob("*"))) - 1
    ...
    def run_comprehensive_merge(self):
        ...
        try:
            ...
            logger.info(
                f"🗂️  Unique files kept: {self.merge_stats['unique_files_kept']}"
            )
            logger.info(
                f"🔄 Duplicates merged: {self.merge_stats['duplicates_merged']}"
            )
