...
class GitHubUploadManager:
    ...
    def prepare_repository_for_upload(
        self, repo_path: str, github_username: str = "ichoake"
    ) -> Dict[str, Any]:
    ...
    def _add_remote_origin(self, repo_path: Path, repo_url: str):
        ...
        subprocess.run(
            ["git", "remote", "add", "origin", repo_url], cwd=repo_path, check=True
        )
...
"""
            ...
            with open(readme_path, "r", encoding="utf-8") as f:
                ...
                for line in lines[:10]:  # Check first 10 lines
                    if (
                        ...
                        return line.strip()[
                            :CONSTANT_100
                        ]  # Limit to CONSTANT_100 characters
    ...
    def _detect_project_type(self, repo_path: Path) -> str:
        ...
        elif (repo_path / "requirements.txt").exists() or (
    ...
    def prepare_all_repositories(
        self, github_username: str = "ichoake"
    ) -> Dict[str, Any]:
        ...
        for repo_path in self.github_dir.iterdir():
            if repo_path.is_dir() and not repo_path.name.startswith("."):
                ...
                try:
                    result = self.prepare_repository_for_upload(
                        str(repo_path), github_username
                    )
                    ...
                    results["failed"] += 1
                    results["repositories"][repo_path.name] = {
                        "success": False,
                        "error": str(e),
                    }

        logger.info(
            f"✅ Preparation complete: {results['prepared']}/{results['total_repos']} repositories prepared"
        )
        return results
