...
class AIConfigManager:
                ...
                with open(self.config_file, "r") as f:
                    ...
                    for key, service_data in config.get("services", {}).items():
                        if key in self.services:
                            self.services[key].enabled = service_data.get(
                                "enabled", False
                            )
    ...
    def save_config(self):
        ...
        config = {
            "services": {
                key: {"enabled": service.enabled, "api_key": service.api_key}
    ...
    def get_services_by_category(self, category: str) -> List[tuple]:
        ...
        if category == "essential":
            ...
            return [
                (k, v) for k, v in self.services.items() if v.category == "audio_video"
            ]
        elif category == "automation":
            return [
                (k, v) for k, v in self.services.items() if v.category == "automation"
            ]
        elif category == "data_search":
            return [
                (k, v) for k, v in self.services.items() if v.category == "data_search"
            ]
        elif category == "cloud":
    ...
    def display_essential_tab(self):
        ...
        for i, (key, service) in enumerate(services, 1):
            ...
            if service.api_key:
                masked_key = (
                    service.api_key[:8] + "..." + service.api_key[-4:]
                    if len(service.api_key) > 12
                    else "***"
                )
    ...
    def toggle_service(self, category: str, index: int):
        ...
        if 0 <= index < len(services):
            ...
            logger.info(
                f"✅ {service.name} {'enabled' if service.enabled else 'disabled'}"
            )
        else:
