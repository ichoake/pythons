...
class ConversationArchiver:
    ...
    def extract_cursor_conversations(self):
        ...
        for chat_dir in chat_dirs:
            try:
                ...
                # Get metadata
                meta_data = cursor.execute("SELECT value FROM meta WHERE key = '0'").fetchone()
                if meta_data:
                    import base64
                    ...
                    conv_date = datetime.fromtimestamp(created_at / CONSTANT_1000).strftime("%Y-%m-%d")
    ...
    def create_conversation_from_prompts(self, prompts):
        ...
        for date, day_prompts in daily_prompts.items():
            ...
            preview = day_prompts[0][:CONSTANT_100] + "..." if len(day_prompts[0]) > CONSTANT_100 else day_prompts[0]
            ...
            conversations.append(
                {
                    ...
                    "tags": tags,
                    "messages": [
                        {"role": "user", "content": p, "time": "N/A"} for p in day_prompts[:10]
    ...
    def generate_html_archive(self):
        ...
        html_content = html_content.replace("const conversations = [];", f"const conversations = {conversations_js};")
        html_content = html_content.replace(
            "const currentMessages = [];", f"const currentMessages = {current_messages_js};"
        )
...
"""
        ...
        for msg in current_messages:
            markdown += f"**{msg['role'].upper()} ({msg['time']}):** {msg['content']}\n\n"
        ...
        # Save markdown file
        md_path = self.archive_dir / f"conversation-archive-{datetime.now().strftime('%Y-%m-%d')}.md"
...
def main():
    ...
    logger.info("The system will automatically capture and organize your conversations.")
