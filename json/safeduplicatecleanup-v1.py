...
class SafeDuplicateCleanup:
                ...
                if group["original"] and group["original"].exists():
                    ...
                    for duplicate in group["duplicates"]:
                        if duplicate.exists():
                            ...
                            logger.info(f"   🗑️  Removing: {duplicate.name} ({size} bytes)")
                            duplicate.unlink()
    ...
    def save_log(self, log_file):
        ...
        log_data = {
            ...
            "space_saved_mb": round(self.space_saved / (CONSTANT_1024 * CONSTANT_1024), 2),
    ...
    def run_cleanup(self):
        ...
        logger.info(f"💾 Space saved: {round(self.space_saved / (CONSTANT_1024 * CONSTANT_1024), 2)} MB")
...
def main():
    ...
    logger.info("⚠️  WARNING: This will remove numbered duplicate files from the backups directory")
