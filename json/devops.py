...
class DevOpsEngineerAgent:
    ...
    def _detect_infrastructure(self, project_path: Path) -> Dict:
        ...
        infra = {"docker": [], "kubernetes": [], "terraform": [], "ansible": [], "other": []}
                ...
                if file_path.is_file():
                    ...
                    if any(p in filename for p in patterns):
                        if pattern_type in infra:
                            infra[pattern_type].append(str(file_path.relative_to(project_path)))
    ...
    def _analyze_ci_cd(self, project_path: Path) -> Dict:
        ...
        if github_workflows.exists():
            ...
            ci_cd["workflows"] = [f.name for f in github_workflows.glob("*.yml") if f.is_file()]
    ...
    def _analyze_containers(self, project_path: Path) -> Dict:
        ...
        containers = {"dockerfile_present": False, "docker_compose_present": False, "issues": [], "best_practices": []}
    ...
    def _analyze_dockerfile(self, content: str) -> tuple:
        ...
        if not any(line.strip().startswith("FROM") for line in lines):
            issues.append({"severity": "critical", "message": "No FROM instruction found"})

        # Check for latest tag
        if "FROM" in content and ":latest" in content:
            issues.append({"severity": "warning", "message": "Using :latest tag - pin to specific versions"})

        # Check for COPY instead of ADD
        if "ADD " in content and "http" not in content.lower():
            issues.append({"severity": "suggestion", "message": "Prefer COPY over ADD for local files"})
        ...
        if "USER" in content and "USER root" not in content:
            ...
            issues.append({"severity": "warning", "message": "No USER instruction - container runs as root"})
    ...
    def _analyze_configuration(self, project_path: Path) -> Dict:
        ...
        config["env_files"] = [str(f.relative_to(project_path)) for f in env_files if f.is_file()]

        # Check for config files
        config_patterns = ["config.yaml", "config.json", "settings.yaml", "settings.json"]
        for pattern in config_patterns:
            files = list(project_path.glob(f"**/{pattern}"))
            config["config_files"].extend([str(f.relative_to(project_path)) for f in files if f.is_file()])

        # Check for secrets management
        if any(p.name in ["vault", "secrets", ".secrets"] for p in project_path.rglob("*")):
    ...
    def _check_security(self, project_path: Path) -> Dict:
        ...
        security = {"gitignore_present": False, "secrets_exposed": [], "dependency_security": False, "issues": []}
            ...
            try:
                ...
                if missing:
                    security["issues"].append(
                        {"severity": "warning", "message": f'Gitignore missing patterns: {", ".join(missing)}'}
                    )
            except (IndexError, KeyError):
                pass
        else:
            security["issues"].append({"severity": "critical", "message": "No .gitignore file found"})

        # Check for dependency security tools
        if (project_path / "Pipfile").exists() or (project_path / "requirements.txt").exists():
            ...
            for file in [".github/workflows/*.yml", ".gitlab-ci.yml"]:
                ...
                for wf in workflow_files:
                    try:
                        ...
                        if any(tool in content for tool in ["safety", "bandit", "snyk"]):
    ...
    def format_analysis(self, analysis: Dict) -> str:
        ...
        output.append(f"  • .gitignore: {'Present ✅' if security['gitignore_present'] else 'Missing ❌'}")
        output.append(
