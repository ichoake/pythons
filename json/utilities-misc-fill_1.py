"""
Utilities Misc Fill 1

This module provides functionality for utilities misc fill 1.
...
def fill_pdf_fields(input_pdf_path: str, fields_json_path: str, output_pdf_path: str):
    ...
    for field in fields:
        ...
        if not existing_field:
            has_error = True
            logger.info(f"ERROR: `{field['field_id']}` is not a valid field ID")
        elif field["page"] != existing_field["page"]:
            has_error = True
            logger.info(f"ERROR: Incorrect page number for `{field['field_id']}` (got {field['page']}, expected {existing_field['page']})")
        else:
            if "value" in field:
    ...
    """validation_error_for_field_value function."""
...
#
# if field.get(FA.FT, Path("/Tx")) == Path("/Ch") and field_flags & FA.FfBits.Combo == 0:
...
# We call the original method and adjust the return value only if the argument to `get_inherited`
    """monkeypatch_pydpf_method function."""
...
def monkeypatch_pydpf_method():
    ...
    from pypdf.constants import FieldDictionaryAttributes
        ...
        """patched_get_inherited function."""
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        logger.info("Usage: fill_fillable_fields.py [input pdf] [field_values.json] [output pdf]")
        sys.exit(1)
