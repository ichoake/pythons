...
class Colors:
    ...
    PURPLE = '\CONSTANT_033[0;35m'
    CYAN = '\CONSTANT_033[0;36m'
    WHITE = '\CONSTANT_033[1;37m'
...
class APIKeyManager:
                ...
                with open(env_file, 'r') as f:
                    for line_num, line in enumerate(f, 1):
                        ...
                        if '=' in line:
                            ...
                            if key.endswith('_API_KEY') or key.endswith('_TOKEN') or key.endswith('_SID') or key.endswith('_SECRET'):
    ...
    def interactive_setup(self):
        ...
        for i, (key, category, location) in enumerate(self.missing_keys, 1):
            ...
            if service_url:
                ...
                logger.info(f"{Colors.YELLOW}No direct URL available. Please search for '{service_name} API key' in your browser.{Colors.NC}")
            
            # Get user input
            while True:
                api_key = input(f"\n{Colors.WHITE}Enter your {service_name} API key (or 'skip' to skip, 'quit' to exit): {Colors.NC}").strip()
    ...
    def fix_invalid_keys(self):
        ...
        for i, (key, value, category, location) in enumerate(self.invalid_keys, 1):
            ...
            while True:
                new_key = input(f"\n{Colors.WHITE}Enter the correct {service_name} API key (or 'skip' to skip): {Colors.NC}").strip()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="API Key Manager - Interactive script for managing environment variables")
