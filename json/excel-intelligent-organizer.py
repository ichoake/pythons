...
class ExcelIntelligentOrganizer:
    ...
    def __init__(self, source_dir: str, target_base: str = None):
        ...
        self.target_base = Path(target_base) if target_base else self.source_dir.parent / "organized_excel"
        self.analysis_data = {}
    ...
    def analyze_excel_files(self) -> Dict[str, Any]:
        ...
        for file_path in excel_files:
            ...
            try:
                ...
                duplicate_info = self._check_duplicates(file_path, analysis_results["file_details"])
                if duplicate_info:
    ...
    def _analyze_content(self, df: pd.DataFrame, filename: str) -> Dict[str, Any]:
        ...
        analysis["null_percentage"] = (null_cells / total_cells) * CONSTANT_100 if total_cells > 0 else 0
        ...
        if patterns:
            analysis["content_summary"] = f"Content appears to be {', '.join(patterns[:3])}"
        else:
    ...
    def _categorize_file(self, df: pd.DataFrame, filename: str) -> str:
        ...
        if any(keyword in filename_lower for keyword in ["trivia", "quiz", "question", "answer"]):
            return "trivia"
        
        # Educational content
        if any(keyword in filename_lower for keyword in ["flashcard", "learn", "study", "education"]):
            return "educational"
        
        # Entertainment content
        if any(keyword in filename_lower for keyword in ["horror", "movie", "film", "entertainment"]):
            return "entertainment"
        
        # Music content
        if any(keyword in filename_lower for keyword in ["discography", "music", "song", "album"]):
            return "music"
        
        # Avatar/character content
        if any(keyword in filename_lower for keyword in ["avatar", "character", "profile"]):
            return "avatars"
        
        # Templates and layouts
        if any(keyword in filename_lower for keyword in ["template", "layout", "example", "format"]):
            return "templates"
        
        # Analysis and data
        if any(keyword in filename_lower for keyword in ["analysis", "data", "report", "stats"]):
            return "analysis"
    ...
    def _calculate_quality_score(self, df: pd.DataFrame) -> float:
        ...
        null_percentage = (df.isnull().sum().sum() / (df.shape[0] * df.shape[1])) * CONSTANT_100
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.analysis_data["duplicates"]:
            recommendations.append(f"Found {len(self.analysis_data['duplicates'])} potential duplicate files - consider consolidating")
        
        # Quality improvements
        low_quality_files = [
            name for name, info in self.analysis_data["file_details"].items()
            if "quality_score" in info and info["quality_score"] < 0.5
        ]
        if low_quality_files:
            recommendations.append(f"Found {len(low_quality_files)} low-quality files - consider data cleaning")
        
        # Category distribution
        category_counts = {cat: len(files) for cat, files in self.analysis_data["categories"].items()}
        if len(category_counts) > 1:
            recommendations.append(f"Files are distributed across {len(category_counts)} categories - good organization potential")
        
        # File naming
        poorly_named = [
            name for name in self.analysis_data["file_details"].keys()
            if " " in name or not name.replace(".xlsx", "").replace(".xls", "").replace(" ", "").isalnum()
        ]
        if poorly_named:
            recommendations.append(f"Consider renaming {len(poorly_named)} files for better organization")
    ...
    def generate_report(self) -> str:
        ...
        report_path = self.target_base / "reports" / f"organization_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        with open(report_path, 'w') as f:
            ...
            # Summary
            f.write("## Summary\n\n")
            f.write(f"- **Total Files Analyzed:** {self.analysis_data['total_files']}\n")
            f.write(f"- **Files Successfully Analyzed:** {self.analysis_data['files_analyzed']}\n")
            f.write(f"- **Categories Found:** {len(self.analysis_data['categories'])}\n")
            f.write(f"- **Potential Duplicates:** {len(self.analysis_data['duplicates'])}\n\n")
            ...
            if quality_scores:
                ...
                f.write(f"- **High Quality Files (>0.7):** {len([s for s in quality_scores if s > 0.7])}\n")
                f.write(f"- **Medium Quality Files (0.4-0.7):** {len([s for s in quality_scores if 0.4 <= s <= 0.7])}\n")
                f.write(f"- **Low Quality Files (<0.4):** {len([s for s in quality_scores if s < 0.4])}\n\n")
            
            # Duplicates
            if self.analysis_data["duplicates"]:
                f.write("## Duplicate Files\n\n")
                for dup_info in self.analysis_data["duplicates"]:
                    f.write(f"- **{dup_info['file']}** duplicates: {', '.join(dup_info['duplicates'])}\n")
                f.write(Path("\n"))
            
            # Recommendations
            f.write("## Recommendations\n\n")
            for i, rec in enumerate(self.organization_plan.get("recommendations", []), 1):
                f.write(f"{i}. {rec}\n")
            f.write(Path("\n"))
            ...
            for filename, info in self.analysis_data["file_details"].items():
                ...
                if "content_analysis" in info:
                    f.write(f"- **Content Summary:** {info['content_analysis'].get('content_summary', 'N/A')}\n")
                    f.write(f"- **Patterns:** {', '.join(info['content_analysis'].get('patterns', []))}\n")
                f.write(Path("\n"))
