...
def run_exporter(transcript_path):
    ...
    try:
        ...
        result = subprocess.run(
            ["python3", str(EXPORTER_PATH)], input=hook_input, capture_output=True, text=True, timeout=30
        )
        ...
        if should_save(user_message):
            if transcript_path:
                ...
                if success:
                    ...
                    response = {
                        "action": "allow",
                        "notification": {"title": "⚠️ Save Failed", "message": "Check exporter configuration"},
                    }
                    logger.info(json.dumps(response))
                    sys.exit(0)
