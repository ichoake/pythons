...
This module provides functionality for utils 11.
...
def get_start_end_time(timespan: str) -> dict:
    ...
    if timespan == "hour":
        ...
        started_at = ended_at - timedelta(hours=1)
    return dict(
        started_at=get_date_string(started_at), ended_at=get_date_string(ended_at)
    )
    """get_valid_file_name function."""
...
def get_game_path(folder: str, game: str, output_path: str) -> str:
    return os.path.join(
        output_path, "media", get_valid_game_name(game), FOLDER_TIMESTAMP, folder
    """get_previous_path function."""
...
def clean_directory(game: str, output_path: str):
    ...
    shutil.rmtree(
        get_game_path(
            config.DIRECTORIES["raw_clips_dir"], get_valid_game_name(game), output_path
        )
    )
