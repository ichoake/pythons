"""
Pandas Pickle Test Generator

This module provides functionality for pandas pickle test generator.
...
def create_pickle_data():
    ...
    cat = {
        ...
        "int16": Categorical(np.arange(CONSTANT_1000)),
        "int32": Categorical(np.arange(CONSTANT_10000)),
    }
    ...
    off = {
        ...
        "DateOffset_h_ns": DateOffset(hour=6, nanoseconds=CONSTANT_5824),
...
def write_legacy_pickles(output_dir):
    ...
    logger.info("  storage format: pickle")

    pth = f"{platform_name()}.pickle"

    with open(os.path.join(output_dir, pth), "wb") as fh:
        pickle.dump(create_pickle_data(), fh, pickle.DEFAULT_PROTOCOL)
    """write_legacy_file function."""


    logger.info(f"created pickle file: {pth}")
