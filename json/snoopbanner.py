...
""",
    ...
    try:
        ...
        logger.info("\CONSTANT_033[31;1mНе удалось открыть браузер\CONSTANT_033[0m")
...
<span style="color:#009a7c"><big>├──</big>ЮMoney::</span> <a href="https://yoomoney.ru/to/4100111364257544" target="_blank">CONSTANT_4100111364257544</a><br />\
<span style="color:#009a7c"><big>├──</big>Сбер_карта:: </span><strong>CONSTANT_2202208013277075</strong><br />\
<span style="color:#009a7c"><big>├──</big>Raiffeisen_card:: </span><strong>CONSTANT_2200300512321074</strong><br />\
<span style="color:#009a7c"><big>├──</big>По номеру телефона <em>(СБП: банк Юмани)</em>:: </span><strong>+CONSTANT_79004753581</strong><br />\
...
\
<ul>\
	<li><span style="font-size:18px"><span style="color:#007500">&nbsp;CONSTANT_4700+ Websites;</span></span></li>\
...
## Лого.
def logo(text, color="\CONSTANT_033[31;1m", exit=True):
...
""")
    ...
    if exit:
        logger.info("\CONSTANT_033[31;1m\n\nВыход")
        sys.exit()


# snoop.py Справка Модули 'if mod == 'help'.
def help_module_1():
    print("""\CONSTANT_033[32;1m└──[Справка]\CONSTANT_033[0m

\CONSTANT_033[32;1m========================
| Плагин GEO_IP/domain |
========================\CONSTANT_033[0m \CONSTANT_033[32m\n
...
============================\CONSTANT_033[0m\n
\CONSTANT_033[32mОбратный impresionante-геокодер от Snoop Project для визуализации координат на карте OSM и статистическим анализом в html/csv/txt форматах.
...
Предназначение плагина — CTF/Образование.\CONSTANT_033[0m

\CONSTANT_033[32;1m========================
| Плагин Yandex_parser |
========================\CONSTANT_033[0m\n
\CONSTANT_033[32mПлагин позволяет получить информацию о пользователях Яндекс-сервисов:
...
Подробнее о плагинах см. 'Общее руководство Snoop Project.pdf'.\CONSTANT_033[0m""")
...
[*] Метод '\CONSTANT_033[32;1mПростой\CONSTANT_033[0m\CONSTANT_033[32m':: На карте OSM (урезанный HTML-отчет) расставляются маркеры по координатам.
Все маркеры подписаны геометками.
Для данного метода доступны сокращенные отчёты с геометками в html/txt форматах.

[*] Метод '\CONSTANT_033[32;1mПодробный\CONSTANT_033[0m\CONSTANT_033[32m':: На карте OSM (HTML-отчет) расставляются маркеры по координатам.
...
\"\"\"\CONSTANT_033[36m
🌎 Координаты: 55.CONSTANT_342595 42.CONSTANT_230801
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Страна: RU
Регион: Nizhny Novgorod Oblast
Округ: Ozero Razodeyskoye\CONSTANT_033[0m\CONSTANT_033[32m
\"\"\"

Метод работает на основе — 'Евклидово дерево'.

\CONSTANT_033[32;1mПлагин Reverse Vgeocoder\CONSTANT_033[0m \CONSTANT_033[32m- работает в оффлайн режиме и укомплектован специально разработанной гео-БД (некоторые БД предоставляются под свободной лицензией от download.geonames.org/export/dump/).
...
\"\"\"\CONSTANT_033[36m
51.CONSTANT_352,   -CONSTANT_108.CONSTANT_625
55.CONSTANT_466,64.CONSTANT_776
52.CONSTANT_40662,66.CONSTANT_77631
53.CONSTANT_028 -CONSTANT_104.CONSTANT_680
54.CONSTANT_505/73.CONSTANT_773
Москва55.75, 37.62 Калининград54.71, 20.51 Ростов-на-Дону47.23, 39.72
случайная_строка1, которая_будет обработана Казань 55.CONSTANT_7734/49.CONSTANT_1436
случайная строка2, которая не будет обработана\CONSTANT_033[0m\CONSTANT_033[32m
\"\"\"
...
def help_yandex_parser():
    print("""\CONSTANT_033[32;1m└──[Справка]

Однопользовательский режим\CONSTANT_033[0m
\CONSTANT_033[32m[*] Логин — левая часть до символа '@', например, bobbimonov@ya.ru, логин
'\CONSTANT_033[36mbobbimonov\CONSTANT_033[0m\CONSTANT_033[32m'.
...
'\CONSTANT_033[36mhttps://yadi.sk/d/7C6Z9q_Ds1wXkw\CONSTANT_033[0m\CONSTANT_033[32m' или '\CONSTANT_033[36mhttps://disk.yandex.ru/d/7C6Z9q_Ds1wXkw\CONSTANT_033[0m\CONSTANT_033[32m'.
[*] Идентификатор — хэш, который указан в url на странице пользователя, например, в сервисе Я.Район: 'https://local.yandex.ru/users/tr6r2c8ea4tvdt3xmpy5atuwg0/' идентификатор — '\CONSTANT_033[36mtr6r2c8ea4tvdt3xmpy5atuwg0\CONSTANT_033[0m\CONSTANT_033[32m'.
...
\CONSTANT_033[32;1mМногопользовательский режим\CONSTANT_033[0m
\CONSTANT_033[32m[*] Файл с именами пользователей — файл (в кодировке UTF-8 с расширением .txt или без него), в котором записаны логины.
Каждый логин в файле должен быть записан с новой строки, например:

\"\"\"
\CONSTANT_033[36mbobbimonov
username
username2
username3
случайная строка
bobbimonov@ya.ru
bobbimonov@ya.ru
bobbimonov@ya.ru\CONSTANT_033[0m
\CONSTANT_033[32m\"\"\"
...
Все результаты сохраняются в '\CONSTANT_033[36m~/snoop/results/plugins/Yandex_parser/*\CONSTANT_033[0m\CONSTANT_033[32m'\CONSTANT_033[0m
    \CONSTANT_033[31;1mВ конце ноября CONSTANT_2022 года Яндекс закрыл публичный api, и возможно, данный плагин больше не заработает...\CONSTANT_033[0m""")


# snoopplugins.py Справка Модуль GEO_IP/domain 'elif dipbaza'.
def geo_ip_domain():
    logger.info("\CONSTANT_033[32;1m└──Справка\CONSTANT_033[0m\n")
    logger.info("""\CONSTANT_033[32m[*] Режим '\CONSTANT_033[32;1mOnline поиск\CONSTANT_033[0m\CONSTANT_033[32m'. Модуль GEO_IP/domain от Snoop Project использует публичный api и создает статистическую и визуализированную информацию по ip/url/domain цели (массиву данных).
...
случайная строка\CONSTANT_033[0m
\CONSTANT_033[32m\"\"\"\CONSTANT_033[0m

\CONSTANT_033[32m[*] Режим '\CONSTANT_033[32;1mOffline поиск\CONSTANT_033[0m\CONSTANT_033[32m'. Модуль GEO_IP/domain от Snoop Project использует специальные базы данных и создает статистическую и визуализированную информацию по ip цели (массиву данных т.е. по ip-адресам).
...
[*] Режим '\CONSTANT_033[32;1mOffline_тихий поиск\CONSTANT_033[0m\CONSTANT_033[32m'. Тот же режим, что и режим 'Offline', но не выводит на печать в CLI промежуточные таблицы с данными. Режим даёт прирост производительности в несколько раз.
    Пример файла с данными (список.txt):

\"\"\"
\CONSTANT_033[36m8.8.8.8
93.CONSTANT_184.CONSTANT_216.34
CONSTANT_2606:CONSTANT_2800:CONSTANT_220:1:CONSTANT_248:CONSTANT_1893:25c8:CONSTANT_1946
случайная строка\CONSTANT_033[0m
\CONSTANT_033[32m\"\"\"
...
В конечном итоге использовать плагин в образовательных целях или из естественного любопытства (проверить любые ip-адреса и их принадлежность к провайдеру и местности).\CONSTANT_033[0m""")
