...
def find_duplicates(directory):
    ...
    for filepath in jpg_files:
        ...
        if file_hash:
            hash_groups[file_hash].append({"path": filepath, "filename": filename, "size": file_size})

        # Group by base name (without (1), (2) suffixes)
        base_name = extract_base_name(filename)
        name_groups[base_name].append({"path": filepath, "filename": filename, "size": file_size})
    ...
    logger.info(f"  • Potential duplicates (similar names): {len(name_duplicates)} groups")
    ...
    logger.info(f"  • Space that could be saved: {total_duplicate_size / (CONSTANT_1024*CONSTANT_1024):.2f} MB")
