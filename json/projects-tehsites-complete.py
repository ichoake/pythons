...
class CompleteFileProcessor:
    ...
    def generate_report(self):
        ...
        for group_name, files in self.file_groups.items():
            ...
            file_infos = [f for f in file_infos if f and f.get("exists")]  # Remove None values and non-existent files
            ...
            if file_infos:  # Only add groups with existing files
                report["file_groups"][group_name] = {
                    ...
                    "avg_size": sum(f["size"] for f in file_infos) / len(file_infos) if file_infos else 0,
                }
