                    ...
                    stats["recent_files"].append(
                        {
                            "name": file,
                            "path": file_path,
                            "modified": datetime.datetime.fromtimestamp(mod_time).strftime("%Y-%m-%d"),
                            "size_mb": round(file_size / (CONSTANT_1024 * CONSTANT_1024), 2),
                        }
                    )
                ...
                file_index.append(
                    {
                        ...
                        "size": file_size,
                        "size_mb": round(file_size / (CONSTANT_1024 * CONSTANT_1024), 2),
                        "modified": datetime.datetime.fromtimestamp(mod_time).strftime("%Y-%m-%d %H:%M:%S"),
                        "category": rel_path.split(os.sep)[0] if os.sep in rel_path else "root",
                    }
                )
            except (OSError, IOError):
...
"""
        ...
        with open(os.path.join(categories_dir, f"{category}.html"), "w", encoding="utf-8") as f:
...
def main():
    ...
    logger.info(f"💾 Total size: {round(stats['total_size'] / (CONSTANT_1024 * CONSTANT_1024 * CONSTANT_1024), 2)}GB")

    logger.info("\n📝 Generating file index...")
    file_index = generate_file_index(downloads_dir, os.path.join(downloads_dir, "file_index.json"))
