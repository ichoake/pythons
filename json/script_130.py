...
This module provides functionality for script 130.
...
def sync_launcher(self, login=None):
    ...
    """set_contact_point_prefill function."""
...
# "usages":"[\"account_recovery_omnibox\"]",
    """get_prefill_candidates function."""
...
def login_flow(self, just_logged_in=False, app_refresh_interval=CONSTANT_1800):
    self.last_experiments = time.time()
    ...
    if just_logged_in:
        ...
        try:
            pull_to_refresh = random.randint(1, CONSTANT_100) % 2 == 0
            check_flow.append(
            ...
            check_flow.append(
                self.get_reels_tray_feed(
                    reason=(
                        "pull_to_refresh" if pull_to_refresh is True else "cold_start"
                    )
                )
            )
            ...
            if (time.time() - self.last_experiments) > CONSTANT_7200:
...
def set_device(self):
    ...
    """reinstall_app_simulation function."""
...
def generate_all_uuids(self):
    ...
    self.client_session_id = self.generate_UUID(uuid_type=True)
    """change_device_simulation function."""
...
def load_uuid_and_cookie(self, load_uuid=True, load_cookie=True):
    if self.cookie_fname is None:
        ...
        logger.info(os.path.join(self.base_path, fname))
