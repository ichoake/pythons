...
CHROME = [
    ...
    "{8237E44A-CONSTANT_0054-442C-B6B6-EA0509993955}",
...
def randomize(_option_, _length_):
    ...
    if _length_ > 0:
        ...
        if _option_ == "-d":
            ...
            _generated_info_ = random.randint(CONSTANT_1950, CONSTANT_2000)
        elif _option_ == "-m":
        ...
        logger.info("randomize: No valid length specified...")
        ext()
...
def check_proxy(agent, proxy, proxy_type):
    try:
        ...
        logger.info(bcolors.FAIL + f"Error while checking proxy" + bcolors.ENDC)
        status = "error"
...
def click_whatever(driver):
    ...
    try:
        driver.find_element_by_xpath(
            "//div[contains(text(), 'Express personalization (1 step)')]"
        ).click()
    except (IndexError, KeyError):
        pass
...
def check_username_taken(driver, uName, rand_name):
    ...
    logger.info(bcolors.OKCYAN + f"Check if username is available!!" + bcolors.ENDC)
    try:
        time.sleep(1)
        driver.find_element_by_xpath("//div[contains(text(), 'That username is taken')]").click()
        logger.info(bcolors.FAIL + f"That username is taken. Try another!" + bcolors.ENDC)
        time.sleep(1)
    ...
    try:
        ...
        print(
            bcolors.FAIL
            + f"Sorry, your username must be between 6 and 30 characters long."
            + bcolors.ENDC
        )
        time.sleep(1)
        username = driver.find_element_by_id("username")
            ...
            with closing(connection.cursor()) as cursor:
                try:
                    cursor.execute(
                        ...
                        (username, password, phone, status, today),
                    )
                except Exception:
                    logger.info(bcolors.FAIL + f"DB ERROR...insert won't work" + bcolors.ENDC)
...
def start(proxy):
    ...
    if category == "rp":
        num = 0
        oldnum = 0
    logger.info(f"################################## START ID:{counter} #########################")
    ...
    if status == CONSTANT_200:
        try:
            ...
            logger.info("Password:", password + Path("\n"))
            ...
            for e in driver.get_log("browser"):
                if re.search(errconclose0, str(e)):
                    logger.info(bcolors.FAIL + errconclose0 + bcolors.ENDC)
                    driver.quit()
                    start(proxy_list[num])
                elif re.search(errconclose1, str(e)):
                    logger.info(bcolors.FAIL + errconclose1 + bcolors.ENDC)
                    driver.quit()
                    start(proxy_list[num])
            ##################################################################################
            ...
            if not du_Url in current_Url:
                logger.info(bcolors.FAIL + f"Restarting...redirected to wrong site" + bcolors.ENDC)
                driver.quit()
                start(proxy_list[num])
            ##################################################################################
            ...
            if du_Url in current_Url:
                logger.info(bcolors.FAIL + f"Restarting...robot check active" + bcolors.ENDC)
                driver.quit()
                start(proxy_list[num])
            ##################################################################################
            ...
            if not uName == NewuName:
                uName = NewuName
                logger.info("New Username: ", uName)
            ##################################################################################
            ...
            try:
                ...
                logger.info(bcolors.FAIL + f"Site not correct loaded..." + bcolors.ENDC)
                logger.info(Path("\n"))
                driver.quit()
                start(proxy_list[num])

            logger.info(bcolors.OKGREEN + f"Verify Your Phone number!!" + bcolors.ENDC)
            time.sleep(1)
            ...
            a = num_numbers["go_0"]
            if a == "0":
                logger.info(bcolors.FAIL + f"No Numbers available" + bcolors.ENDC)
                driver.quit()
                start(proxy_list[num])
            else:
                logger.info("Available phone numbers: ", a)
                ...
                logger.info("buy TEXT: ", order_number.text)
                ...
                if order_number.text == "NO_NUMBERS":
                    ...
                    logger.info(bcolors.FAIL + f"No Balance...Please TopUp" + bcolors.ENDC)
                    driver.quit()
                    sys.exit()
                ...
                try:
                    ...
                    ch_activation_status = requests.get(
                        "https://sms-activate.ru/stubs/handler_api.php?api_key="
                        + api_key
                    ...
                    logger.info(bcolors.FAIL + f"sorry this number has some issues" + bcolors.ENDC)
                    driver.quit()
                    start(proxy_list[num])
                ...
                if ch_activation_status.text in access_ready:
                    logger.info("number readiness confirmed\n")
                    ...
                    while (
                        status_wait in code
                        or status_ok in code
                        or status_cancel in code
                        or status_wait_resend in code
                        or status_wait_retry in code
                    ):
                        if waitcounter == 6:
                            ch_activation_status = requests.get(
                                "https://sms-activate.ru/stubs/handler_api.php?api_key="
                            ...
                            logger.info("Cancel the activation")
                            print(
                                bcolors.FAIL + f"sorry this number has some issues" + bcolors.ENDC
                            )
                            driver.quit()
                            start(proxy_list[num])

                        if code in status_wait:
                            logger.info("wait sometime for SMS")
                            waitcounter += 1
                            time.sleep(20)
                            get_sms = requests.get(
                                "https://sms-activate.ru/stubs/handler_api.php?api_key="
                            ...
                            logger.info("Your SMS code: ", m_code)
                            time.sleep(2)
            ...
            while veryfi_url in a or phone_url in a or main_url in a:
                if main_url in a:
                    ...
                    time.sleep(2)
                    logger.info("This is not correct page\nplz wait some time")
                    a = driver.current_url
            ...
            if du_Url in current_Url:
                ...
                if fail_url in cur_url:
                    logger.info("This account take some time")
                    logger.info("Plz Cut this browser yourself\n")
                    time.sleep(10)
                    click_whatever(driver)
                    time.sleep(5)
                    click_whatever(driver)
            ...
            logger.info(bcolors.OKGREEN + f"Now, this account is completed.\n" + bcolors.ENDC)
            driver.quit()
        except (ValueError, TypeError):
            driver.quit()
            logger.info(bcolors.FAIL + f"Proxie not working ID:" + str(oldnum) + bcolors.ENDC)
            logger.info(Path("\n"))
            start(proxy_list[num])
    else:
        logger.info(bcolors.FAIL + f"Proxie not working ID:" + str(oldnum) + bcolors.ENDC)
...
logger.info(bcolors.OKGREEN + f"Total proxies : {total_proxies}" + bcolors.ENDC)
