...
This module provides functionality for script 112.
...
class ContentAwareAnalyzer:
    ...
    def analyze_file_relationships(self, files):
        ...
        for file_info in files:
            ...
            for other_file in files:
                ...
                shared_projects = set(file_info.get("projects", [])) & set(other_file.get("projects", []))
                shared_topics = set(file_info.get("categories", [])) & set(other_file.get("categories", []))
    ...
    def generate_analysis_summary(self):
        ...
        self.analysis_results["file_relationships"] = self.analyze_file_relationships(self.all_files)
    ...
    def create_organized_structure(self):
        ...
        for project in self.analysis_results["project_identifications"].keys():
            project_dir = self.target_dir / "01_Projects" / self.sanitize_filename(project, 50)
        ...
        for project, files in self.analysis_results["project_identifications"].items():
            ...
            with open(report_path, "w", encoding="utf-8") as f:
                ...
                for file_info in sorted(files, key=lambda x: x["modified"], reverse=True):
                    ...
                    f.write(f"- **Categories:** {', '.join(file_info.get('categories', []))}\\n")
                    f.write(f"- **Code Blocks:** {len(file_info.get('code_blocks', []))}\\n")
                    f.write(f"- **Tool Calls:** {len(file_info.get('tool_calls', []))}\\n")
                    f.write(f"- **Modified:** {file_info['modified'].strftime('%Y-%m-%d %H:%M:%S')}\\n\\n")

    def generate_category_reports(self):
        """Generate category reports"""
        for category, files in self.analysis_results["content_categories"].items():
            report_path = self.target_dir / "02_Content_Categories" / category / f"{category}_ANALYSIS.md"
            ...
            with open(report_path, "w", encoding="utf-8") as f:
                ...
                for file_info in sorted(files, key=lambda x: x["modified"], reverse=True):
                    f.write(f"### {file_info['name']}\\n")
                    f.write(f"- **Projects:** {', '.join(file_info.get('projects', []))}\\n")
                    f.write(f"- **Chat ID:** {file_info.get('chat_id', 'unknown')}\\n")
                    f.write(f"- **Modified:** {file_info['modified'].strftime('%Y-%m-%d %H:%M:%S')}\\n\\n")
        ...
        for file_info in self.all_files:
            for code_block in file_info.get("code_blocks", []):
                ...
                if code_type not in code_stats:
                    code_stats[code_type] = {"count": 0, "total_size": 0, "languages": set(), "files": set()}

                code_stats[code_type]["count"] += 1
                code_stats[code_type]["total_size"] += code_block.get("size", 0)
                code_stats[code_type]["languages"].add(code_block.get("language", "unknown"))
                code_stats[code_type]["files"].add(file_info["name"])

        for code_type, stats in code_stats.items():
            report_path = self.target_dir / "03_Code_Extractions" / code_type / f"{code_type}_ANALYSIS.md"
        ...
        for file_info in self.all_files:
            for tool_call in file_info.get("tool_calls", []):
                ...
                if tool_type not in tool_stats:
                    tool_stats[tool_type] = {"count": 0, "total_size": 0, "files": set()}
    ...
    def generate_relationship_reports(self):
        ...
        relationship_report_path = self.target_dir / "05_File_Relationships" / "RELATIONSHIPS.md"
        ...
        with open(relationship_report_path, "w", encoding="utf-8") as f:
            ...
            for file_id, relationships in self.analysis_results["file_relationships"].items():
                f.write(f"### {file_id}\\n")
                f.write(f"- **Related Files:** {len(relationships['related_files'])}\\n")
