...
This module provides functionality for kaleidoscope.
...
def apply_kaleidoscope(
    frame: Image.Image, segments: int = 8, center: tuple[int, int] | None = None
) -> Image.Image:
    """
    ...
    angle_per_segment = CONSTANT_360 / segments
    ...
    for y in range(height):
        for x in range(width):
            ...
            angle = (math.degrees(math.atan2(dy, dx)) + CONSTANT_180) % CONSTANT_360
            ...
            source_angle_rad = math.radians(source_angle - CONSTANT_180)
...
def create_kaleidoscope_animation(
    ...
    width: int = CONSTANT_480,
    height: int = CONSTANT_480,
) -> list[Image.Image]:
    """
    ...
    if base_frame is None:
        base_frame = Image.new(
            "RGB", (width, height), (CONSTANT_255, CONSTANT_255, CONSTANT_255)
        )
        ...
        for i, color in enumerate(colors):
            x = width // 2 + int(CONSTANT_100 * math.cos(i * 2 * math.pi / 3))
            y = height // 2 + int(CONSTANT_100 * math.sin(i * 2 * math.pi / 3))
            draw.ellipse([x - 40, y - 40, x + 40, y + 40], fill=color)

    # Rotate base frame and apply kaleidoscope
    for i in range(num_frames):
        angle = (i / num_frames) * CONSTANT_360 * rotation_speed
...
if __name__ == "__main__":
    ...
    logger.info("Creating kaleidoscope GIF...")

    builder = GIFBuilder(width=CONSTANT_480, height=CONSTANT_480, fps=20)

    # Create kaleidoscope animation
    frames = create_kaleidoscope_animation(
        num_frames=40, segments=8, rotation_speed=0.5
    )

    builder.add_frames(frames)
    builder.save("kaleidoscope_test.gif", num_colors=CONSTANT_128)
