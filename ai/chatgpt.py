...
This module provides functionality for chatgpt.
...
class ChatGPTAgent:
    ...
    def chat(self, message: str, temperature: float = 0.7, max_tokens: int = CONSTANT_1000) -> str:
        """
        ...
        try:
            ...
            if "gpt-4" in self.model or "gpt-5" in self.model:
                ...
                response = self.client.chat.completions.create(
                    model=self.model, messages=self.conversation_history, temperature=temperature, max_tokens=max_tokens
                )
...
def main():
    ...
    if not api_key:
        ...
        logger.info("1. Set environment variable: export OPENAI_API_KEY='your-key-here'")
