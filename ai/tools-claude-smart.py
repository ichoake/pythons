...
class SmartOrganizationPlanner:
    ...
    def analyze_file_for_organization(self, file_info):
        ...
        project_context = self._identify_project_context(file_name, current_location, relative_path)
        ...
        restore_info = self._generate_restore_info(full_path, relative_path, current_location)
    ...
    def _identify_project_context(self, file_name, current_location, relative_path):
        ...
        elif any(word in file_lower for word in ["instagram", "tiktok", "social", "media"]):
            return "social_media"
        elif any(word in file_lower for word in ["ai", "gpt", "claude", "automation", "bot"]):
            return "ai_content"
        elif any(word in file_lower for word in ["html", "css", "js", "web", "website"]):
            return "web_development"
        elif any(word in file_lower for word in ["csv", "json", "data", "analysis", "stats"]):
            return "data_analysis"
    ...
    def _identify_content_type(self, file_name):
        ...
        if any(word in file_lower for word in ["template", "example", "sample", "demo"]):
            return "template"
        elif any(word in file_lower for word in ["config", "settings", "setup", "env"]):
            return "configuration"
        elif any(word in file_lower for word in ["report", "analysis", "summary", "results"]):
            return "report"
    ...
    def _generate_organization_suggestions(
        ...
        reasons = []
        if file_size_mb > 50:
            reasons.append(f"Large file ({file_size_mb:.1f}MB) - high priority for organization")
        if project_context != "general_files":
            reasons.append(f"Project-specific file - belongs in {project_context.replace('_', ' ').title()}")
    ...
    def _calculate_priority(self, file_info, project_context, content_type):
        ...
        high_priority_contexts = ["portfolio_work", "claude_projects", "as_man_thinketh_project"]
    ...
    def _generate_restore_info(self, full_path, relative_path, current_location):
        ...
        if "python" in current_location.lower() and "python" not in relative_path.lower():
            instructions.append("Originally in Python directory - may be part of a Python project")
        ...
        if "claude" in current_location.lower():
            instructions.append("Claude course material - belongs with educational content")

        if "notion" in current_location.lower():
            instructions.append("Notion export - may be part of a larger workspace export")
    ...
    def generate_organization_plan(self):
        ...
        for file_info in self.files_data:
            try:
                ...
                logger.info(f"⚠️  Error processing {file_info.get('file_name', 'unknown')}: {e}")
                continue

        # Sort by priority (highest first)
        self.organization_plan.sort(key=lambda x: x["priority"], reverse=True)

        logger.info(f"✅ Generated organization plan for {len(self.organization_plan)} files")
    ...
    def save_organization_plan(self):
        ...
        csv_filename = f"/Users/steven/Documents/python/smart_organization_plan_{timestamp}.csv"
    ...
    def generate_summary_report(self):
        ...
        for entry in self.organization_plan:
            priority_level = "High" if entry["priority"] >= 15 else "Medium" if entry["priority"] >= 10 else "Low"
        ...
        immediate_actions = sum(1 for entry in self.organization_plan if entry["immediate_action"])
        backup_recommended = sum(1 for entry in self.organization_plan if entry["backup_recommended"])
    ...
    def print_summary(self):
        ...
        for i, entry in enumerate(self.organization_plan[:10], 1):
            logger.info(f"   {i:2d}. {entry['file_name']} ({entry['file_size_mb']:.1f} MB)")
