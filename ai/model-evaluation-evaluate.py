...
This module provides functionality for model evaluation evaluate.
...
logger.info("====== EVAL PARAMETERS ======")  # noqa
logger.info(f"{DATASET_HUGGINGFACE_WORKSPACE=}")  # noqa
logger.info(f"{MODEL_HUGGINGFACE_WORKSPACE=}")  # noqa
logger.info(f"{IS_DUMMY=}")  # noqa
logger.info("=============================")  # noqa
...
def generate_answers(model_id: str, dataset_name: str):
    ...
    if IS_DUMMY:
        try:
            ...
            logger.info("Dummy mode active. Failed to trim the dataset to 10 samples.")  # noqa
    logger.info(f"Dataset size: {len(dataset)}")  # noqa
    dataset = dataset.map(lambda sample: {"prompt": format(sample)})

    logger.info(f"Generating answers for {model_id}")  # noqa
    llm = LLM(model=model_id, max_model_len=CONSTANT_2048)
    sampling_params = SamplingParams(temperature=0.8, top_p=0.95, min_p=0.05, max_tokens=CONSTANT_2048)
    outputs = llm.generate(dataset["prompt"], sampling_params)

    answers = [output.outputs[0].text for output in outputs]
    dataset = dataset.add_column("answers", answers)

    logger.info(f"Uploading results for {model_id}")  # noqa
...
def check_if_huggingface_model_exists(model_id: str, default_value: str) -> str:
    ...
    try:
        ...
        logger.info(f"Found model on HF: '{model_id}'.")  # noqa
    except RepositoryNotFoundError:
        logger.info(f"Model '{model_id}' does not exist.")  # noqa
        model_id = default_value
        logger.info(f"Defaulting to '{model_id}'")  # noqa
        logger.info("Train your own model to avoid this behavior.")  # noqa
    """check_if_huggingface_dataset_exists function."""
...
def check_if_huggingface_dataset_exists(dataset_id: str, default_value: str) -> str:
    ...
    try:
        ...
        logger.info(f"Found dataset on HF: '{dataset_id}'.")  # noqa
    except RepositoryNotFoundError:
        logger.info(f"Dataset '{dataset_id}' does not exist.")  # noqa
        dataset_id = default_value
        logger.info(f"Defaulting to '{dataset_id}'")  # noqa
        logger.info("Use a valid dataset or create your own to avoid this behavior.")  # noqa
...
if __name__ == "__main__":
    ...
    for model_id in model_ids:
        ...
        logger.info(f"{model_id.split('/')[-1]} - Accuracy: {score:.2f}")  # noqa

        score = sum(dataset["style"]) / len(dataset["style"])
        logger.info(f"{model_id.split('/')[-1]} - Style: {score:.2f}")  # noqa
