...
def get_openai_batch_titles(script_contents):
    ...
    try:
        ...
        if len(titles) < len(script_contents):
            titles.extend([f"Script {i+1}: Untitled" for i in range(len(titles), len(script_contents))])
...
if __name__ == "__main__":
    ...
    output_csv = Path("/Users/steven/Documents/python/output.csv")  # Output CSV file path
