...
class AIDeepAnalyzer:
    ...
    def detect_content_type(self, content: str) -> str:
        ...
        elif "<" in content and ">" in content and not content.count("<") > CONSTANT_100:
            return "structured_markup"
        elif "```" in content:
            return "code_documentation"
        elif any(keyword in content_lower for keyword in ["story", "narrative", "character"]):
            return "narrative_content"
        elif any(keyword in content_lower for keyword in ["api", "endpoint", "function"]):
    ...
    def extract_key_insights(self, content: str, content_type: str) -> List[str]:
        ...
        if content_type == "prompt_template":
            ...
            if variables:
                insights.append(f"Uses {len(variables)} template variables for dynamic content")
    ...
    def generate_recommendations(self, content: str, content_type: str, structure: Dict[str, Any]) -> List[str]:
        """Generate recommendations for improvement"""
        recommendations = []

        if structure["lines"] > CONSTANT_1000:
            recommendations.append("Consider breaking into smaller, more manageable sections")

        if structure["avg_chars_per_line"] > CONSTANT_100:
            recommendations.append("Lines are quite long - consider adding line breaks for readability")
        ...
        if content_type == "prompt_template":
            if structure["template_variables"] == 0:
                recommendations.append("Consider adding template variables for dynamic content")

            if not re.search(r"#+\s+", content):
                recommendations.append("Add clear section headers to organize the prompt")
        ...
        if structure["word_count"] > CONSTANT_5000:
            recommendations.append("Content is quite extensive - consider adding a table of contents")
    ...
    def analyze_technical_details(self, content: str) -> Dict[str, Any]:
        ...
        if "&#" in content:
            technical["encoding_issues"].append("Contains HTML entities that may need decoding")
        ...
        if not re.search(r"alt\s*=|aria-", content, re.IGNORECASE):
            technical["accessibility_concerns"].append("No accessibility attributes found")

        if len(content) > CONSTANT_100000:
            technical["performance_considerations"].append("Large file size may impact loading performance")

        return technical

    def run_ai_analysis(self, content: str, ai_service: str = "claude") -> Dict[str, Any]:
        ...
        if len(content) > max_length:
            content_sample = content[:max_length] + "\n\n[Content truncated for analysis...]"
        else:
        ...
        try:
            if ai_service == "claude":
                ...
                if result.returncode == 0:
                    return {"success": True, "analysis": result.stdout, "service": "claude"}
                else:
                    return {"success": False, "error": f"Claude analysis failed: {result.stderr}", "service": "claude"}
            elif ai_service == "xai":
                # Use xAI CLI
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    return {"success": True, "analysis": result.stdout, "service": "xai"}
                else:
                    return {"success": False, "error": f"xAI analysis failed: {result.stderr}", "service": "xai"}
            else:
                return {"success": False, "error": f"Unsupported AI service: {ai_service}", "service": ai_service}
            ...
            return {"success": False, "error": f"Error running AI analysis: {e}", "service": ai_service}

    def generate_summary(self, content_type: str, structure: Dict[str, Any], insights: List[str]) -> str:
        """Generate a comprehensive summary"""
        summary_parts = []

        summary_parts.append(f"This is a {content_type.replace('_', ' ')} document")
        summary_parts.append(f"containing {structure['word_count']:,} words across {structure['lines']} lines")

        if structure["template_variables"] > 0:
            summary_parts.append(f"with {structure['template_variables']} template variables")
    ...
    def analyze_file(self, file_path: str, ai_service: str = "claude") -> AIAnalysisResult:
        ...
        recommendations = self.generate_recommendations(content, content_type, structure)
    ...
    def print_analysis(self, result: AIAnalysisResult, file_path: str):
        ...
        logger.info(f"   Template Variables: {result.structure['template_variables']:,}")
        logger.info(f"   Code Blocks: {result.structure['code_blocks']:,}")
        logger.info(f"   Average Words/Line: {result.structure['avg_words_per_line']:.1f}")
        ...
        logger.info(f"\n⚙️  TECHNICAL DETAILS:")
        logger.info(f"   Formatting Consistent: {'✅' if result.technical_details['formatting_consistency'] else '❌'}")
        logger.info(f"   Template Complexity: {result.technical_details['template_complexity']}")
...
def main():
    ...
    if ai_service not in ["claude", "xai"]:
        logger.info(f"Error: Unsupported AI service '{ai_service}'. Use 'claude' or 'xai'")
        sys.exit(1)

    analyzer = AIDeepAnalyzer()

    try:
        logger.info(f"🔍 Analyzing {os.path.basename(file_path)} with {ai_service.upper()}...")
