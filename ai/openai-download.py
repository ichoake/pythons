...
This module provides functionality for script 103.
...
#!/usr/bin/env python3
"""
Simple TTS Generator for "As a Man Thinketh"
Generates emotional audiobook using OpenAI TTS
"""
...
def generate_audiobook():
    """Generate the complete audiobook using OpenAI TTS"""
    ...
    audio_dir = Path("simple_tts_generator")
    audio_dir.mkdir(exist_ok=True)

    logger.info("🎙️ Generating 'As a Man Thinketh' Audiobook with OpenAI TTS")
    logger.info("=" * 70)
...
JAMES ALLEN. BROAD PARK AVENUE, ILFRACOMBE, ENGLAND"""
    ...
    try:
        response = client.audio.speech.create(
            model="tts-1-hd", voice="alloy", input=foreword_text, response_format="mp3"  # Warm, professional narrator
        )

        with open(audio_dir / "01-Foreword.mp3", "wb") as f:
...
In this direction, as in no other, is the law absolute that "He that seeketh findeth; and to him that knocketh it shall be opened;" for only by patience, practice, and ceaseless importunity can a man enter the Door of the Temple of Knowledge."""
    ...
    try:
        response = client.audio.speech.create(
            model="tts-1-hd",
            voice="nova",  # Deep, philosophical wisdom
            input=thought_character_text,
            response_format="mp3",
        )

        with open(audio_dir / "02-Thought-and-Character.mp3", "wb") as f:
            f.write(response.content)
...
"So You will be what you will to be; Let failure find its false content In that poor word, 'environment,' But spirit scorns it, and is free. It masters time, it conquers space; It cowes that boastful trickster, Chance, And bids the tyrant Circumstance Uncrown, and fill a servant's place. The human Will, that force unseen, The offspring of a deathless Soul, Can hew a way to any goal, Though walls of granite intervene. Be not impatient in delays But wait as one who understands; When spirit rises and commands The gods are ready to obey." """
    ...
    try:
        response = client.audio.speech.create(
            model="tts-1-hd",
            voice="shimmer",  # Uplifting, motivational
            input=effect_thought_text,
            response_format="mp3",
        )

        with open(audio_dir / "03-Effect-of-Thought-on-Circumstances.mp3", "wb") as f:
            f.write(response.content)
    ...
    try:
        response = client.audio.speech.create(
            model="tts-1-hd", voice="alloy", input=complete_text, response_format="mp3"  # Consistent narrator voice
        )

        with open(audio_dir / "04-Complete-Book.mp3", "wb") as f:
            f.write(response.content)
    ...
    metadata = {
        ...
        "chapters": [
            {"file": "01-Foreword.mp3", "title": "Foreword", "voice": "alloy"},
            {"file": "02-Thought-and-Character.mp3", "title": "Thought and Character", "voice": "nova"},
            {
                "file": "03-Effect-of-Thought-on-Circumstances.mp3",
                "title": "Effect of Thought on Circumstances",
                "voice": "shimmer",
            },
            {"file": "04-Complete-Book.mp3", "title": "Complete Book", "voice": "alloy"},
        ],
    }
...
def main():
    """Main function"""
    logger.info("🎙️ Simple TTS Generator for 'As a Man Thinketh'")
    logger.info("Using OpenAI TTS with emotional delivery")
    logger.info("=" * 70)
    ...
    if success:
        ...
        logger.info("\n❌ FAILED! Check your OpenAI API key and try again.")
