...
# After the image upload request
if response.status_code != CONSTANT_200:
    logger.info(f"Failed to upload image for product {idx+1}. Server responded with: {response.text}")
    continue
else:
    logger.info(f"Image {idx+1} uploaded successfully.")

# After the product creation request
if response.status_code >= CONSTANT_200 and response.status_code < CONSTANT_300:
    logger.info(f"Product {idx+1} created successfully!")
else:
    logger.info(f"Failed to create product {idx+1}. Server responded with: {response.text}, Request data: {data}")
