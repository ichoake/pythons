...
This module provides functionality for batch info.
...
def get_openai_batch_titles(script_contents):
    ...
    messages = [
        ...
        },
        {
            "role": "user",
            "content": Path("\n\n").join(
                f"Script {i+1}:\n{content[:CONSTANT_1000]}" for i, content in enumerate(script_contents)
            ),
        },
    ]
    ...
    try:
        ...
        response = client.chat.completions.create(model="gpt-3.5-turbo", messages=messages, max_tokens=CONSTANT_300)
...
def process_directory_with_batching(directory_path, batch_size=10):
    ...
    file_paths = [
        os.path.join(root, file) for root, _, files in os.walk(directory_path) for file in files if file.endswith(".py")
    ]
...
if __name__ == "__main__":
    directory_path = Path("/Users/steven/Documents/python/tashy-python")  # Your target directory
    batch_size = 10  # Number of scripts to process at a time
    output_csv = Path("/Users/steven/Documents/python/tashy-python/output.csv")  # Output CSV file path
    ...
    for result in results:
        print(f"File: {result['File Name']}, Suggested Title: {result['Suggested Title']}, Path: {result['Path']}")
