...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Analyze images and output metadata to CSV.")
    parser.add_argument("input_folder", type=Path, help="Full path to the folder containing image files.")
    parser.add_argument(
        "-o", "--output_csv", type=Path, default=Path("analyzed_images.csv"), help="Path to output CSV file."
    )
...
def main():
    ...
    if not image_paths:
        logger.warning("No image files found in the specified directory or its subdirectories.")
        return
    ...
    with output_csv.open(mode="w", newline="", encoding="utf-8") as csvfile:
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(process_image, image_path, input_path, date_str): image_path
