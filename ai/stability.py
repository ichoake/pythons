...
CONSTANT_512 = 512
CONSTANT_1024 = 1024
CONSTANT_123463446 = 123463446
CONSTANT_1000000000 = 1000000000


os.environ['STABILITY_HOST'] = 'grpc.stability.ai:443'

seed = random.randint(0, CONSTANT_1000000000)

# Set up our connection to the API.
stability_api = client.StabilityInference(
    key=os.environ['STABILITY_KEY'], # API Key reference.
    verbose=True, # Print debug messages.
    engine="stable-diffusion-xl-CONSTANT_1024-v1-0", # Set the engine to use for generation.
...
def text_to_image(prompt):
    ...
    answers = stability_api.generate(
        ...
        width=CONSTANT_1024, # Generation width, defaults to CONSTANT_1024 if not included.
        height=CONSTANT_1024, # Generation height, defaults to CONSTANT_1024 if not included.
...
def edit_image(input_image_path, prompt, output_image_name):
    ...
    answers = stability_api.generate(
        ...
        seed=CONSTANT_123463446, # If attempting to transform an image that was previously generated with our API,
                        # initial images benefit from having their own distinct seed rather than using the seed of the original image generation.
        steps=50, # Amount of inference steps performed on image generation. Defaults to 30.
        cfg_scale=8.0, # Influences how strongly your generation is guided to match your prompt.
                    # Setting this value higher increases the strength in which it tries to match your prompt.
                    # Defaults to 7.0 if not specified.
        width=CONSTANT_512, # Generation width, defaults to CONSTANT_512 if not included.
        height=CONSTANT_512, # Generation height, defaults to CONSTANT_512 if not included.
        sampler=generation.SAMPLER_K_DPMPP_2M # Choose which sampler we want to denoise our generation with.
                                                    # Defaults to k_dpmpp_2m if not specified. Clip Guidance only supports ancestral samplers.
                                                    # (Available Samplers: ddim, plms, k_euler, k_euler_ancestral, k_heun, k_dpm_2, k_dpm_2_ancestral, k_dpmpp_2s_ancestral, k_lms, k_dpmpp_2m, k_dpmpp_sde)
    )
