"""
Utilities Cli Tools Run 8

This module provides functionality for utilities cli tools run 8.
    ...
    def run_renamer(self, renamer_name: str, target_path: Path, **kwargs):
        ...
        try:
            ...
            if renamer_name == "master":
                ...
                renamer.analyze_and_rename(target_path, apply=kwargs.get("apply", False))
                return True
                ...
                if not OCR_AVAILABLE:
                    ...
                    logger.info("Install with: pip install opencv-python pytesseract pillow openai python-dotenv")
                    return False

                renamer = renamer_class()
                rows = renamer.walk_and_process(
                    target_path, apply=kwargs.get("apply", False), confirm=not kwargs.get("yes", False)
                )
...
Examples:
  ...
  python run_renamers.py --renamer ocr /path/to/media --apply
    ...
    parser.add_argument("path", type=Path, nargs="?", help="Target directory to process")

    parser.add_argument("--list", action="store_true", help="List all available renamers")

    parser.add_argument("--demo", action="store_true", help="Demo all renamers (dry run)")
    ...
    if not args.path:
        logger.info("❌ Error: Path required for demo, all, or specific renamer operations")
    ...
    # Run specific renamer
    if args.renamer:
        success = runner.run_renamer(args.renamer, args.path, apply=args.apply, yes=args.yes)
