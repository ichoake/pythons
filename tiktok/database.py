...
This module provides functionality for database.
...
def initDatabase():
    ...
    cursor.execute("SET sql_notes = 0;")
    cursor.execute("set global max_allowed_packet=CONSTANT_67108864;")
    cursor.execute(
        "create table IF NOT EXISTS clip_bin (clip_num int NOT NULL AUTO_INCREMENT, PRIMARY KEY (clip_num), clip_id varchar(CONSTANT_100), date varchar(40), status varchar(CONSTANT_100),  clipwrapper BLOB, filter_name varchar(70));"
    )
...
def getFoundClips(filter, limit):
    ...
    query = (
        "select * FROM clip_bin WHERE filter_name = %s and status = 'FOUND' LIMIT %s;"
    )
    args = (filter, limit)
