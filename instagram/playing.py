...
This module provides functionality for playing.
...
with smart_run(session):
    ...
    session.set_quota_supervisor(
        ...
        notify_me=True,
        peak_likes=(57, CONSTANT_585),
        peak_follows=(48, None),
        peak_unfollows=(35, CONSTANT_402),
        peak_server_calls=(CONSTANT_500, None),
    )
    session.set_relationship_bounds(
        enabled=True,
        potency_ratio=-1.3,
        delimit_by_numbers=True,
        max_followers=CONSTANT_10000,
        max_following=CONSTANT_15000,
        min_followers=75,
        min_following=75,
    )
    ...
    session.set_delimit_liking(enabled=True, max=CONSTANT_1005, min=10)
    ...
    session.unfollow_users(
        amount=random.randint(30, CONSTANT_100),
        InstapyFollowed=(True, "all"),
        style="FIFO",
        unfollow_after=90 * 60 * 60,
        sleep_delay=CONSTANT_501,
    )
