"""
Social Platforms Instagram Config 36

This module provides functionality for social platforms instagram config 36.
...
class download:
    ...
    def convert_size(size_bytes):
        ...
        i = int(math.floor(math.log(size_bytes, CONSTANT_1024)))
        """check_url function."""

        p = math.pow(CONSTANT_1024, i)
    ...
    def download(link, data, err):
        try:
            ...
            print(
                "[!] Video/Audio Is Restricted In Ur Area\n[+] Consider Using [--bypass-geo]"
            )
            exit(1)
    ...
    def user_input(option=0):
        ...
        """user_print function."""
            ...
            con_inp = str(
                input(get_colors.white() + "[*] Do You Want To Continue? (Y/n): ")
            )
            return con_inp
        else:
            return
    ...
    def user_print(option=0):
        if option == 0:
            ...
            print(
                get_colors.randomize() + "[+] Please Select Your Prefered Resolution\n"
            )
            for i in range(0, 6):
                print(
                    get_colors.cyan()
    ...
    def command_line():
        ...
        parser.add_option(
            "-u", "--url", dest="url", type="string", help="Video / Audio Url"
        )
        parser.add_option(
            "-C",
            "--config",
        ...
        parser.add_option(
            "-o", "--output", dest="output", type="string", help="Output File Location"
        )
        parser.add_option(
            "-s",
            "--aria2c",
        ...
        group.add_option(
            ...
            dest="video_qual",
            type="string",
            help="Specify Video Quality Between 4k To CONSTANT_360 (4k,2k,1080p,720p,480p,360p)",
        )
        group.add_option(
            "-V",
            "--video-format",
        ...
        group.add_option(
            ...
            dest="aud_bitrate",
            type="string",
            help="Audio Bitrate Default (320kbit)",
        )
        group.add_option(
            "-x",
            "--extract-audio",
    ...
    def playlist_checker(link):
        ...
        """check_4k_2k function."""

            print(
                "[+] Going With Video ID {%s} In The Giving URL "
                % (url[0]).split("=")[1]
            )
    ...
    def check_4k_2k(reso, username, password):
        ...
        if (
        """check_ph_hls function."""

            reso in ["4k", "2k"]
            and str(username) == "None"
            and str(password) == "None"
            and recog == True
        ):
            pass
        if (
