"""
Instagram Image Uploadr

This module provides functionality for instagram image uploadr.
...
services:
  web:
    command: ["./wait-for-selenium.sh", "http://selenium:CONSTANT_4444/wd/hub", "--",
    "python", "start.py"]
    environment:
      - PYTHONUNBUFFERED=0
    build:
...
if data["friends_interaction"]:
    ...
    friends.set_selenium_remote_session(selenium_url="http://selenium:CONSTANT_4444/wd/hub")
    with smart_run(friends):
        ...
        friends.set_do_like(True, percentage=CONSTANT_100)
...
bot.set_selenium_remote_session(selenium_url="http://selenium:CONSTANT_4444/wd/hub")
with smart_run(bot):
    """
    ...
    bot.set_quota_supervisor(
        ...
        notify_me=True,
        peak_likes_hourly=CONSTANT_106,
        peak_likes_daily=CONSTANT_585,
        peak_follows_hourly=48,
        peak_follows_daily=None,
        peak_unfollows_hourly=35,
        peak_unfollows_daily=CONSTANT_403,
        peak_server_calls_hourly=None,
        peak_server_calls_daily=CONSTANT_4700,
    )
    """
    ...
    bot.set_relationship_bounds(
        ...
        max_followers=CONSTANT_99999999,
        max_following=CONSTANT_5000,
        min_followers=CONSTANT_2000,
        min_following=10,
    )
    """
    Filters
    ...
    """
    bot.set_do_like(enabled=True, percentage=CONSTANT_100)
    bot.set_delimit_liking(enabled=True, min_likes=40)
    if data["do_comments"]:
    ...
    if data["do_unfollow"]:
        ...
        bot.unfollow_users(
            amount=random.randint(75, CONSTANT_100),
