...
class GIFBuilder:
    ...
    def __init__(self, width: int = CONSTANT_480, height: int = CONSTANT_480, fps: int = 15):
        """
    ...
    def optimize_colors(self, num_colors: int = CONSTANT_128, use_global_palette: bool = True) -> list[np.ndarray]:
        """
        Reduce colors in all frames using quantization.

        Args:
            num_colors: Target number of colors (8-CONSTANT_256)
        ...
        if use_global_palette and len(self.frames) > 1:
            ...
            width = min(CONSTANT_512, int(np.sqrt(total_pixels)))  # Reasonable width, max CONSTANT_512
    ...
    def deduplicate_frames(self, threshold: float = 0.CONSTANT_995) -> int:
        """
        Remove duplicate or near-duplicate consecutive frames.

        Args:
            threshold: Similarity threshold (0.0-1.0). Higher = more strict (0.CONSTANT_995 = very similar).
        ...
        for i in range(1, len(self.frames)):
            ...
            similarity = 1.0 - (np.mean(diff) / CONSTANT_255.0)

            # Keep frame if sufficiently different
            # High threshold (0.CONSTANT_995) means only remove truly identical frames
    ...
    def save(self, output_path: str | Path, num_colors: int = CONSTANT_128,
        ...
        if optimize_for_emoji:
            if self.width > CONSTANT_128 or self.height > CONSTANT_128:
                logger.info(f"  Resizing from {self.width}x{self.height} to 128x128 for emoji")
                self.width = CONSTANT_128
                self.height = CONSTANT_128
                # Resize all frames
                resized_frames = []
                for frame in self.frames:
                    pil_frame = Image.fromarray(frame)
                    pil_frame = pil_frame.resize((CONSTANT_128, CONSTANT_128), Image.Resampling.LANCZOS)
            ...
            if len(self.frames) > 12:
                logger.info(f"  Reducing frames from {len(self.frames)} to ~12 for emoji size")
        ...
        frame_duration = CONSTANT_1000 / self.fps
        ...
        # Get file info
        file_size_kb = output_path.stat().st_size / CONSTANT_1024
        file_size_mb = file_size_kb / CONSTANT_1024
        ...
        logger.info(f"  Size: {file_size_kb:.1f} KB ({file_size_mb:.2f} MB)")
        logger.info(f"  Dimensions: {self.width}x{self.height}")
        logger.info(f"  Frames: {len(optimized_frames)} @ {self.fps} fps")
        logger.info(f"  Duration: {info['duration_seconds']:.1f}s")
        ...
        if optimize_for_emoji and file_size_kb > 64:
            ...
            logger.info("   Try: fewer frames, fewer colors, or simpler design")
        elif not optimize_for_emoji and file_size_kb > CONSTANT_2048:
            logger.info(f"\n⚠️  WARNING: File size ({file_size_kb:.1f} KB) is large for Slack")
            logger.info("   Try: fewer frames, smaller dimensions, or fewer colors")
