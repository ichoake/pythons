...
class InstaClient:
    ...
    def Connect(self):
        if self.isproxyok != True:
            PrintError(
                "Proxy does not work! (Proxy:", self.user, self.ip, ":", self.port, ")"
            )
            return

        if self.ip != None and self.port != None:
            PrintSuccess(
                "Proxy working! (Proxy:", self.user, self.ip, ":", self.port, ")"
            )
    ...
    def Spam(self, userid, username, reasonid):
        ...
        res = self.PostAndUpdate(
            reportURL, {"source_name": "profile", "reason": reasonid}
        )
        ...
        try:
            ...
            if "description" in obj and "status" in obj:
                if (
                    obj["status"] == "ok"
                    and obj["description"]
                    == "Your reports help keep our community free of spam."
                ):
