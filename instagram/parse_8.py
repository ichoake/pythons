...
try:
    for pic in pics:
        ...
        bot.logger.info(
            "Uploading pic `{pic}` with caption: `{caption}`".format(pic=pic, caption=caption)
        )
        ...
        if not bot.upload_photo(
            os.path.dirname(os.path.realpath(__file__)) + Path("/media/") + pic,
            caption=caption,
    ...
    bot.logger.error("\CONSTANT_033[41mERROR...\CONSTANT_033[0m")
