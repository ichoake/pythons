...
CONSTANT_201 = 201
CONSTANT_1000 = 1000
CONSTANT_0999999999999996 = 0999999999999996
    ...
    expected = DataFrame(
        {
            ...
            "C": [1.0, 1.0, 1.9, 3.CONSTANT_0999999999999996, 0.3, 0.7, 2.6, 3.0, 1.2, 2.8],
        },
...
@pytest.mark.parametrize("frame_size", [(2, 3), (CONSTANT_100, 10)])
...
def test_timestamp_groupby_quantile(unit):
    # GH 33168
    dti = pd.date_range(
        start="2020-04-19 00:00:00", freq="1min", periods=CONSTANT_100, tz="UTC", unit=unit
    ).floor("1h")
    df = DataFrame(
        {
            "timestamp": dti,
            "category": list(range(1, CONSTANT_101)),
            "value": list(range(CONSTANT_101, CONSTANT_201)),
        }
    )
    ...
    expected = DataFrame(
        [
            {"category": 12.8, "value": CONSTANT_112.8},
            {"category": 48.2, "value": CONSTANT_148.2},
            {"category": 68.8, "value": CONSTANT_168.8},
            {"category": 92.2, "value": CONSTANT_192.2},
        ],
        index=mi,
    )
...
def test_groupby_quantile_dt64tz_period():
    # GH#51373
    dti = pd.date_range("2016-01-01", periods=CONSTANT_1000)
    ...
    by = np.tile(np.arange(5), CONSTANT_200)
    gb = df.groupby(by)
