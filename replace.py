...
This module provides functionality for replace.
...
def apply_paragraph_properties(paragraph, para_data: Dict[str, Any]):
    ...
    if para_data.get("bullet", False):
        ...
        level_indent_emu = int((font_size * (1.6 + level * 1.6)) * CONSTANT_12700)
        hanging_indent_emu = int(-font_size * 0.8 * CONSTANT_12700)
...
def apply_font_properties(run, para_data: Dict[str, Any]):
    ...
    if "color" in para_data:
        ...
        try:
            ...
            logger.info(f"  WARNING: Unknown theme color name '{theme_name}'")
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str):
    ...
    if errors:
        logger.info("ERROR: Invalid shapes in replacement JSON:")
        for error in errors:
            logger.info(f"  - {error}")
    ...
    for slide_key, shapes_dict in inventory.items():
        ...
        if slide_index >= len(prs.slides):
            logger.info(f"Warning: Slide {slide_index} not found")
            continue
        ...
        for shape_key, shape_data in shapes_dict.items():
            ...
            if not shape:
                logger.info(f"Warning: {shape_key} has no shape reference")
                continue
    ...
    # Report results
    logger.info(f"Saved updated presentation to: {output_file}")
    logger.info(f"Processed {len(prs.slides)} slides")
    logger.info(f"  - Shapes processed: {shapes_processed}")
    logger.info(f"  - Shapes cleared: {shapes_cleared}")
    logger.info(f"  - Shapes replaced: {shapes_replaced}")
...
def main():
    ...
    if not input_pptx.exists():
        logger.info(f"Error: Input file '{input_pptx}' not found")
        sys.exit(1)

    if not replacements_json.exists():
        logger.info(f"Error: Replacements JSON file '{replacements_json}' not found")
        sys.exit(1)

    try:
        apply_replacements(str(input_pptx), str(replacements_json), str(output_pptx))
    except Exception as e:
        logger.info(f"Error applying replacements: {e}")
        import traceback
