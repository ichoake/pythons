...
class ContextDetector:
    ...
    def analyze_complexity(content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "imports": len(
                re.findall(r"^(import|from|require|use)", content, re.MULTILINE)
            ),
...
class DynamicInsights:
    ...
    def generate_insights(
        filepath: Path, content: str, context: Dict, analysis: Dict
    ) -> List[Dict]:
        ...
        # Complexity insights
        insights.extend(
            DynamicInsights._complexity_insights(context.get("complexity", {}))
        )
    ...
    def _security_insights(content: str, context: Dict) -> List[Dict]:
        ...
        if re.search(
            r'(password|passwd|pwd)\s*=\s*["\'][^"\']+["\']', content, re.IGNORECASE
        ):
            insights.append(
                {
                    "type": "security",
    ...
    def _best_practices(content: str, context: Dict) -> List[Dict]:
        ...
        if context.get("language") in ["python", "javascript"]:
            magic_numbers = re.findall(
                r"\b(CONSTANT_100|CONSTANT_1000|CONSTANT_256|CONSTANT_512|CONSTANT_1024)\b",
                content,
            )
...
class AdaptiveAnalyzer:
    ...
    def _analyze_directory(self, dirpath: Path, thoroughness: str) -> Dict:
        ...
        files = [
            f for f in dirpath.rglob("*") if f.is_file() and not self._should_skip(f)
        ]
    ...
    def _generate_summary(self) -> Dict:
        ...
        return {
            ...
            "by_priority": {
                "critical": len(
                    [i for i in all_insights if i.get("priority") == "critical"]
                ),
                "high": len([i for i in all_insights if i.get("priority") == "high"]),
                "medium": len(
                    [i for i in all_insights if i.get("priority") == "medium"]
                ),
            ...
            "languages_detected": list(
                set(
                    [
                        ctx.get("language")
                        for ctx in self.results["contexts"].values()
                        if ctx.get("language")
                    ]
                )
            ),
    ...
    def print_report(self):
        ...
        critical = [
            i
            for i in self.results["insights_generated"]
