...
class FormatOptimizedAutomation:
    ...
    def _sort_and_categorize_data(self, analysis):
        ...
        for file_type, files in analysis["file_categories"].items():
            analysis["file_categories"][file_type] = sorted(
        ...
        for project_type, projects in analysis["sorted_by_type"].items():
            analysis["sorted_by_type"][project_type] = sorted(
                projects, key=lambda x: x["complexity"], reverse=True
            )
    ...
    def generate_article_content(self):
        ...
        article = {
            ...
            "introduction": self._generate_formatted_introduction(
                primary_type, project_name
            ),
            "sections": self._generate_formatted_sections(primary_type),
            "conclusion": self._generate_formatted_conclusion(
                primary_type, project_name
            ),
    ...
    def _generate_formatted_metrics(self):
        ...
        return {
            ...
            "avg_file_size_mb": round(
                self.analysis_data["avg_file_size"] / (CONSTANT_1024 * CONSTANT_1024), 2
            ),
        }
