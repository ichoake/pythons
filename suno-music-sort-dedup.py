...
class AdvancedSortAndDedupe:
    ...
    def process_duplicates(self, duplicates: Dict[str, List[Path]]):
        ...
        for file_hash, files in duplicates.items():
            if len(files) > 1:
                ...
                file_analyses.sort(
                    key=lambda x: (x["quality_score"], x["size"]), reverse=True
                )
        ...
        for group_name, files in similar_groups.items():
            ...
            for similar in similar_files:
                ...
                while archive_path.exists():
                    archive_path = (
                        self.duplicates_dir
                        / f"{group_name}_{similar.stem}_{counter}{similar.suffix}"
                    )
                    counter += 1
    ...
    def generate_reports(
        self, duplicates: Dict[str, List[Path]], similar_groups: Dict[str, List[Path]]
    ):
        ...
        with open(summary_report, "w") as f:
            ...
            f.write("## Overview\n\n")
            f.write(
                f"- **Total Python files processed:** {len(list(self.base_dir.rglob('*.py')))}\n"
            )
            f.write(
                f"- **Exact duplicates found:** {sum(len(files) - 1 for files in duplicates.values())}\n"
            )
            f.write(f"- **Similar file groups:** {len(similar_groups)}\n")
            f.write(
                f"- **Files archived:** {len(list(self.duplicates_dir.rglob('*.py')))}\n"
            )
            f.write(
                f"- **Files organized:** {len(list(self.clean_dir.rglob('*.py')))}\n\n"
            )
        ...
        analysis_data = {
            ...
            "similar_groups": {
                k: [str(f) for f in files] for k, files in similar_groups.items()
            },
            "categories": {
                cat: len(list((self.clean_dir / cat).glob("*.py")))
