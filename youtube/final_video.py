...
def createBackground(
    ...
    logger.info("Creating background for the video...Gathering Audio and Video files")
    ...
    video = VideoFileClip(
        "{}/{}".format(videoPath, random.choice(os.listdir(f"{videoPath}"))),
        audio=False,
    )
    durationVideo = video.duration
    logger.info("Editing the videos...")
...
def finalize(
    videoPath, audio_path, title, exportPath="./"
):  # the path to video FILES, the path to the mp3 file, the title of the video, export path
    ...
    print(
        "Successfully saved the video {} with subtitle".format(
            str(title) + "-with-subtitle.mp4"
        )
    )
