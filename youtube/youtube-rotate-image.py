"""
Youtube Rotate Image By Orientation

This module provides functionality for youtube rotate image by orientation.
...
def rotate_image_by_orientation(image):
    ...
    try:
        ...
        if exif and EXIF_TAG_MAP["Orientation"] in exif:
            ...
            if orientation == 3:
                rotation_angle = CONSTANT_180
            elif orientation == 6:
                rotation_angle = CONSTANT_270
            elif orientation == 8:
                rotation_angle = 90
            else:
                rotation_angle = 0
        ...
        if exif:
            if EXIF_TAG_MAP["DateTimeOriginal"] in exif:
                image_date = parse_exif_datetime(
                    exif[EXIF_TAG_MAP["DateTimeOriginal"]])
...
def get_metadata(image, thumbnail_path, public_path):
    ...
    :return:
    """
    # Paths should be relative to the public folder, because they will
    # directly be used in the HTML
    image_data = dict(
