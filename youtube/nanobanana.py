...
class NanoBananaPromptAnalyzer:
    ...
    def __init__(self, documents_root: Path):
        ...
        self.prompt_patterns = {
            ...
            "sora": [r"sora", r"video.*generation", r"txt.*to.*video", r"openai.*video"],
            "suno": [r"suno", r"music.*generation", r"audio.*generation", r"song.*creation"],
    ...
    def analyze_documents(self, max_depth: int = 6) -> Dict:
        ...
        results["content_analysis"] = self._generate_content_analysis(results["prompt_collections"])

        # Generate API integration recommendations
        results["api_integrations"] = self._generate_api_integrations(results["prompt_collections"])
        ...
        for pattern in prompt_patterns:
            ...
            for match in matches:
                if len(match.strip()) > 10:  # Filter out very short matches
                    prompts.append({"text": match.strip(), "length": len(match.strip()), "type": "extracted"})
    ...
    def _extract_technical_specs(self, content: str) -> Dict[str, str]:
        ...
        # Quality patterns
        quality_match = re.search(r"(4[kK]|8[kK]|HD|UHD|ultra-hd)", content, re.IGNORECASE)
        if quality_match:
                        ...
                        examples.append(
                            {
                                ...
                                "nanobanana_optimized": self._optimize_for_nanobanana(prompt["text"]),
                                "type": prompt_type,
    ...
    def _generate_recommendations(self, results: Dict) -> List[str]:
        ...
        if "nanobanana" in results["content_analysis"]["api_usage"]:
            recommendations.append("Integrate NanoBanana API for advanced video generation")

        if results["content_analysis"]["prompt_types"].get("image_prompts", 0) > 50:
            recommendations.append("Implement batch image generation with NanoBanana API")
    ...
    def generate_nanobanana_prompts(self, analysis_results: Dict) -> Dict:
        ...
        nanobanana_prompts = {"image_generation": [], "video_generation": [], "image_editing": [], "video_editing": []}
    ...
    def _create_nanobanana_prompt(self, prompt: Dict, file_data: Dict) -> Optional[Dict]:
        ...
        if "video" in original_text.lower() or "motion" in original_text.lower():
            ...
            category = "image_editing" if "image" in original_text.lower() else "video_editing"
        else:
    ...
    def _print_analysis_summary(self, results: Dict):
        ...
        logger.info(f"🎨 Total prompts found: {results['content_analysis']['total_prompts']}")
