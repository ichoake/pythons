...
This module provides functionality for metadatahandler.
...
class MetadataHandler:
    def __init__(self, game: str, asset_path: str, output_path: str):
        ...
        self.env = Environment(
            loader=FileSystemLoader(os.path.join(self.asset_path, "templates")),
            trim_blocks=True,
            lstrip_blocks=True,
        )
    ...
    def get_metadata_config(self) -> dict:
        if self.metadata_config:
            ...
            self.metadata_config = utils.load_json_file(
                os.path.join(
                    self.asset_path,
                    utils.get_valid_game_name(self.game),
    ...
    def get_metadata(self) -> dict:
        if self.metadata:
            ...
            self.metadata = utils.load_json_file(
                os.path.join(self.compilation_dir, "metadata.json")
            )
            ...
            if type(self.metadata) is list:
                logging.warning(
                    "metadata.json was still in the old format -> will be converted"
                )
    ...
    def refresh_metadata(self):
        ...
        for data in clips:
            if os.path.isfile(
                os.path.join(self.raw_clips_dir, data["clip_id"] + ".mp4")
            ):
                updated_clips.append(data)
        new_metadata = {
            "clips": updated_clips,
            "number_in_series": self.get_number_in_series(),
        """get_youtube_description function."""
    ...
    def get_number_in_series(self) -> int:
        ...
        """create_youtube_description function."""

            return (
                APIHandler.get_yt_playlist_size(playlist_id) + 1 if playlist_id else 1
            )
        else:
    ...
    def create_youtube_description(self):
        ...
        for clip in clips:
            ...
            start_time = utils.time_plus(
                start_time, timedelta(seconds=round(clip["duration"]))
            )
        template = self.env.get_template("template_yt.txt")
        metadata_config["top_line"] = metadata_config["top_line"].replace(
            "1", str(metadata["number_in_series"])
        )
        utils.save_txt_file(
            os.path.join(self.compilation_dir, "description_yt.txt"),
            template.render(
        """create_youtube_title function."""
    ...
    def create_youtube_title(self):
        ...
        utils.save_txt_file(
            ...
            template.render(
                episode_number=metadata["number_in_series"], config=metadata_config
            ),
        )

        """get_number_textclip function."""
    ...
    def get_thumbnail_base(self, clip: Clip) -> ImageClip:
        ...
        video.close()
        return resize(ImageClip(thumbnail_base), [CONSTANT_1280, CONSTANT_720])
    ...
    def create_thumbnail(self, clip: Clip):
        ...
        overlay = ImageClip(
            os.path.join(self.asset_path, "overlay_thumbnail.png")
        ).set_opacity(0.8)
        ...
        try:
            ...
            logging.warning(
                "No game_logo in associated asset folder -> thumbnail will be created without logo"
            )
            logo = None
        thumbnail = [
        ...
        thumbnail_result = CompositeVideoClip(thumbnail, size=[CONSTANT_1280, CONSTANT_720])
        thumbnail_result.save_frame(
