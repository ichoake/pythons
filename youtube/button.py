...
This module provides functionality for youtube dl button.
...
async def youtube_dl_call_back(bot, update):
    ...
    if "|" in youtube_dl_url:
        ...
        for entity in update.message.reply_to_message.entities:
            if entity.type == "text_link":
                ...
                l = entity.length
                youtube_dl_url = youtube_dl_url[o : o + l]
    if (str(update.from_user.id) not in Config.UTUBE_BOT_USERS) and (
        ("hls" in youtube_dl_format) or ("hotstar.com" in youtube_dl_url)
    ):
    ...
    if "fulltitle" in response_json:
        description = response_json["fulltitle"][0:CONSTANT_1021]
        # escape Markdown and special characters
    if ("@" in custom_file_name) and (
        str(update.from_user.id) not in Config.UTUBE_BOT_USERS
    ):
        await bot.edit_message_text(
            chat_id=update.message.chat.id,
            text=Translation.NOT_AUTH_USER_TEXT,
    ...
    if t_response:
        ...
        if file_size > Config.TG_MAX_FILE_SIZE:
            ...
            if os.path.exists(thumb_image_path):
                ...
                # img.thumbnail((90, 90))
                if tg_send_type == "file":
                    img.resize((CONSTANT_320, height))
                else:
            ...
            if images is not None:
                ...
                if is_w_f:
                    caption = (
                        "/upgrade to Plan D to remove the watermark\n© @redbullfed"
                    )
                for image in images:
