...
This module provides functionality for clipeditor.
...
class Slide:
    ...
    def __init__(
        ...
        self,
        text: str = "Test",
        size: tuple = (CONSTANT_1920, CONSTANT_1080),
        bgColor: tuple = (0, 0, 0),
        txtColor: tuple = (CONSTANT_255, CONSTANT_255, CONSTANT_255),
        fileName: str = f"slide{random.randrange(CONSTANT_1000)}",
        fontName: str = "font",
        textRatio: float = 0.5,
        customBg=False,
        bgName=None,
...
def create_text_slide(slide: Slide) -> None:
    ...
    drawImage.text(
        ((slide.size[0] - textWidth) / 2, (slide.size[1] - textHeight) / 2),
        text=slide.text,
        fill=slide.txtColor,
        font=font,
    )
...
def create_intro(
    size: tuple, video_fps: int, numberOfClips: int, channel: str, time: str
) -> ImageSequenceClip:
...
def create_video(save_path: str = ".", channel: str = None, time: str = "7d") -> None:
    ...
    if get_intro_slide():
        clipList.append(
            create_intro(video_size, video_fps, numberOfClips, channel, time)
        )
