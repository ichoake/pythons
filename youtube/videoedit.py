...
This module provides functionality for videoedit 2.
...
class VideoEditor:
    def __init__(self, num_replies, video_name):
        ...
        logger.info(self.num_replies)

    def create_dir(self):
        """Creates the dir to hold the edited video if it doesnt already exist"""
        try:
            os.makedirs(self.save_path)
            logger.info(f"directory: {self.save_path} created")
    ...
    def create_movie(self):
        ...
        final_vid.write_videofile(
            f"{self.save_path}{self.video_name}.mp4",
            fps=10,
            codec="libx264",
            audio_codec="aac",
