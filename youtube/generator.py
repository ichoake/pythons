...
@app.route(Path("/backgrounds"), methods=["GET"])
def backgrounds():
    return render_template("backgrounds.html", file="backgrounds.json")


@app.route(Path("/background/add"), methods=["POST"])
def background_add():
    # Get form values
...
@app.route(Path("/background/delete"), methods=["POST"])
...
@app.route(Path("/settings"), methods=["GET", "POST"])
def settings():
