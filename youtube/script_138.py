...
def compatible_aspect_ratio(size):
    ...
    logger.info("FOUND: w:{w} h:{h} r:{r}".format(w=width, h=height, r=ratio))
    return min_ratio <= ratio <= max_ratio


def configure_photo(
    self, upload_id, photo, caption="", user_tags=None, is_sidecar=False
):
...
def upload_photo(
    ...
    for attempt in range(4):
        ...
        elif self.configure_photo(
            upload_id, photo, caption, usertags, is_sidecar=False
        ):
...
def upload_album(
    ...
    for photo in photos:
        ...
        if not result:
            self.logger.error(
                "Could not upload photo {photo} for the album!".format(photo=photo)
            )
            return False
...
def resize_image(fname):
    ...
    ratio = w * 1.0 / h * 1.0
    logger.info("FOUND w:{w}, h:{h}, ratio={r}".format(w=w, h=h, r=ratio))
    if w > h:
        ...
        if w > CONSTANT_1080:
            ...
            nw = CONSTANT_1080
            nh = int(ceil(CONSTANT_1080.0 * h / w))
        ...
        if h > CONSTANT_1080:
            logger.info("Resizing image")
            nw = int(ceil(CONSTANT_1080.0 * w / h))
            nh = CONSTANT_1080
            img = img.resize((nw, nh), Image.ANTIALIAS)
    else:
        ...
        if w > CONSTANT_1080:
            logger.info("Resizing image")
            img = img.resize((CONSTANT_1080, CONSTANT_1080), Image.ANTIALIAS)
    (w, h) = img.size
    new_fname = "{fname}.CONVERTED.jpg".format(fname=fname)
    logger.info("Saving new image w:{w} h:{h} to `{f}`".format(w=w, h=h, f=new_fname))
...
def stories_shaper(fname):
    ...
    CONSTANT_1920 pixels. Substrate formation: Crop the image under 1080x1920 pixels
    ...
    try:
        ...
        return False
    img = Image.open(fname)
    if (img.size[0], img.size[1]) == (CONSTANT_1080, CONSTANT_1920):
        ...
        img_bg = img.crop(
            (
                int((img.size[0] - CONSTANT_1080) / 2),
                int((img.size[1] - CONSTANT_1920) / 2),
                int(CONSTANT_1080 + ((img.size[0] - CONSTANT_1080) / 2)),
                int(CONSTANT_1920 + ((img.size[1] - CONSTANT_1920) / 2)),
            )
        ).filter(ImageFilter.GaussianBlur(CONSTANT_100))
        if img.size[1] > img.size[0]:
            ...
            if img.size[0] > CONSTANT_1080:
                ...
                img_bg.paste(
                    img, (int(CONSTANT_540 - img.size[0] / 2), int(CONSTANT_960 - img.size[1] / 2))
                )
            else:
