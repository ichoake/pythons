"""
Media Processing Image Youtube Remove Background

This module provides functionality for media processing image youtube remove background.
...
REMOVE_BG_API_URL = "https://api.vanceai.com/web_api/v1/removebg"
PROGRESS_API_URL = "https://api.vanceai.com/web_api/v1/progress"
...
def remove_background(input_path, output_path):
    ...
    with open(input_path, "rb") as file:
        response = requests.post(REMOVE_BG_API_URL, files={"image_file": file}, headers={"api_key": API_KEY})
...
def download_result(trans_id, output_path):
    while True:
        response = requests.get(f"{PROGRESS_API_URL}?trans_id={trans_id}&api_token={API_KEY}")
