...
CONSTANT_999999 = 999999
CONSTANT_111111111111111 = 111111111111111
...
def test_precise_conversion(c_parser_only, num):
    ...
    def error(val: float, actual_val: Decimal) -> Decimal:
        return abs(Decimal(f"{val:.CONSTANT_100}") - actual_val)
...
def test_custom_lineterminator(c_parser_only):
    ...
    expected = parser.read_csv(StringIO(data.replace("~", Path("\n"))))
...
def test_tokenize_CR_with_quoting(c_parser_only):
    ...
    expected = parser.read_csv(StringIO(data.replace(Path("\r"), Path("\n"))), header=None)
    tm.assert_frame_equal(result, expected)
...
def test_parse_trim_buffers(c_parser_only, encoding):
    ...
    # approx 1.5KiB).
    record_ = (
        """CONSTANT_9999-9,99:99,,,,ZZ,ZZ,,,ZZZ-ZZZZ,.Z-ZZZZ,-9.99,,,9.99,Z"""
        ...
        """9,9,99,99,CONSTANT_999,CONSTANT_999,ZZZZZ,ZZZ-ZZZZZ,ZZZ-ZZZZ,9,ZZ-ZZZZ,9."""
        """99,ZZ-ZZZZ,ZZ-ZZZZ,,,,ZZZZ,,,ZZ,ZZ,,,,,,,,,,,,,9,,,CONSTANT_999."""
        """99,CONSTANT_999.99,,,ZZZZZ,,,Z9,,,,,,,ZZZ,ZZZ,,,,,,,,,,,ZZZZZ,ZZ"""
        """ZZZ,ZZZ-ZZZZZZ,ZZZ-ZZZZZZ,ZZ-ZZZZ,ZZ-ZZZZ,ZZ-ZZZZ,ZZ-ZZ"""
        """ZZ,,,CONSTANT_999999,CONSTANT_999999,ZZZ,ZZZ,,,ZZZ,ZZZ,CONSTANT_999.99,CONSTANT_999.99,,,,Z"""
        """ZZ-ZZZ,ZZZ-ZZZ,-9.99,-9.99,9,9,,99,,9.99,9.99,9,9,9.99,"""
        """9.99,,,,9.99,9.99,,99,,99,9.99,9.99,,,ZZZ,ZZZ,,CONSTANT_999.99,,"""
        """CONSTANT_999.99,ZZZ,ZZZ-ZZZZ,ZZZ-ZZZZ,,,ZZZZZ,ZZZZZ,ZZZ,ZZZ,9,9,"""
        """,,,,,ZZZ-ZZZZ,ZZZ999Z,,,CONSTANT_999.99,,CONSTANT_999.99,ZZZ-ZZZZ,,,9.999"""
        """,9.CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,-9.CONSTANT_999,-9.CONSTANT_999,-9.CONSTANT_999,-9.CONSTANT_999,9.CONSTANT_999,9."""
        """CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,9.CONSTANT_999,CONSTANT_99999,ZZZ-ZZZZ,"""
        """,9.99,ZZZ,,,,,,,,ZZZ,,,,,9,,,,9,,,,,,,,,,ZZZ-ZZZZ,ZZZ-Z"""
        """ZZZ,,ZZZZZ,ZZZZZ,ZZZZZ,ZZZZZ,,,9.99,,ZZ-ZZZZ,ZZ-ZZZZ,ZZ"""
        """,CONSTANT_999,,,,ZZ-ZZZZ,ZZZ,ZZZ,ZZZ-ZZZZ,ZZZ-ZZZZ,,,99.99,99.99"""
        """,,,9.99,9.99,9.99,9.99,ZZZ-ZZZZ,,,ZZZ-ZZZZZ,,,,,-9.99,-"""
        """9.99,-9.99,-9.99,,,,,,,,,ZZZ-ZZZZ,,9,9.99,9.99,99ZZ,,-9"""
        """.99,-9.99,ZZZ-ZZZZ,,,,,,,ZZZ-ZZZZ,9.99,9.99,CONSTANT_9999,,,,,,,"""
        """,,,-9.9,Z/Z-ZZZZ,CONSTANT_999.99,9.99,,CONSTANT_999.99,ZZ-ZZZZ,ZZ-ZZZZ,9."""
        """99,9.99,9.99,9.99,9.99,9.99,,ZZZ-ZZZZZ,ZZZ-ZZZZZ,ZZZ-ZZ"""
        """ZZZ,ZZZ-ZZZZZ,ZZZ-ZZZZZ,ZZZ,ZZZ,ZZZ,ZZZ,9.99,,,-9.99,ZZ"""
        """-ZZZZ,-CONSTANT_999.99,,-CONSTANT_9999,,CONSTANT_999.99,,,,CONSTANT_999.99,99.99,,,ZZ-ZZZZZ"""
        """ZZZ,ZZ-ZZZZ-ZZZZZZZ,,,,ZZ-ZZ-ZZZZZZZZ,ZZZZZZZZ,ZZZ-ZZZZ"""
        """,CONSTANT_9999,CONSTANT_999.99,ZZZ-ZZZZ,-9.99,-9.99,ZZZ-ZZZZ,99:99:99,,99"""
    ...
    # within the parser.
    chunksize, n_lines = CONSTANT_128, 2 * CONSTANT_128 + 15
    csv_data = Path("\n").join([record_] * n_lines) + Path("\n")
...
def test_read_nrows_large(c_parser_only):
    ...
    parser = c_parser_only
    header_narrow = Path("\t").join(["COL_HEADER_" + str(i) for i in range(10)]) + Path("\n")
    data_narrow = Path("\t").join(["somedatasomedatasomedata1" for _ in range(10)]) + Path("\n")
    header_wide = Path("\t").join(["COL_HEADER_" + str(i) for i in range(15)]) + Path("\n")
    data_wide = Path("\t").join(["somedatasomedatasomedata2" for _ in range(15)]) + Path("\n")
    test_input = header_narrow + data_narrow * CONSTANT_1050 + header_wide + data_wide * 2

    df = parser.read_csv(StringIO(test_input), sep=Path("\t"), nrows=CONSTANT_1010)

    assert df.size == CONSTANT_1010 * 10
...
def test_large_difference_in_columns(c_parser_only):
    ...
    count = CONSTANT_10000
    large_row = ("X," * count)[:-1] + Path("\n")
    normal_row = "XXXXXX XXXXXX,CONSTANT_111111111111111\n"
    test_input = (large_row + normal_row * 6)[:-1]

    result = parser.read_csv(StringIO(test_input), header=None, usecols=[0])
    rows = test_input.split(Path("\n"))
...
def test_chunk_whitespace_on_boundary(c_parser_only):
    ...
    chunk1 = "a" * (CONSTANT_1024 * CONSTANT_256 - 2) + Path("\na")
    chunk2 = "\n a"
    result = parser.read_csv(StringIO(chunk1 + chunk2), header=None)

    expected = DataFrame(["a" * (CONSTANT_1024 * CONSTANT_256 - 2), "a", " a"])
    tm.assert_frame_equal(result, expected)
