...
This module provides functionality for filtercreator.
...
class Filter:
    def __init__(
        ...
        self, searchType, inputText, likeCount, shareCount, playCount, commentCount
    ):
...
class FilterCreationWindow(QMainWindow):
    ...
    def changeCategory(self):
        ...
        if isHashTag:
            self.inputTextLabel.setText(
                "Enter comma separated Hashtags (no #) e.g. memes, lol, funny"
            )

        if isAuthor:
            self.inputTextLabel.setText(
                "Enter comma separated author names  e.g. charlidamelio, addisonre"
    ...
    def attemptCreateFilter(self):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                "Make sure that the filter inputs are all numbers!",
                QMessageBox.Ok,
            )
            return
        ...
        if isHashTag or isAuthor:
            if (
                self.inputText.text() == ""
                or self.inputText.text().replace(" ", "") == ""
            ):
                QMessageBox.information(
                    self,
                    "%s type specified but no input text!" % filterType,
        ...
        if filterName in self.savedFilters:
            QMessageBox.information(
                ...
                "Please use a different name for this filter! This name (%s) is already registered."
                % filterName,
                QMessageBox.Ok,
            )
            return
        ...
        filter = Filter(
            filterType, inputText, likeAmount, shareAmount, playAmount, commentAmount
        )
