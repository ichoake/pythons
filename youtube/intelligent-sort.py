...
class FinalIntelligentSorter:
    ...
    def analyze_python_file(self, file_path: Path):
        ...
        # AI/ML detection
        ai_keywords = ["openai", "anthropic", "claude", "gpt", "llm", "ai", "agent", "chat", "model"]
        if any(kw in content_lower for kw in ai_keywords):
            return "AI_CONTENT"

        # Bot/Automation detection
        bot_keywords = ["selenium", "playwright", "bot", "automation", "scrape", "instagram", "reddit", "twitter"]
    ...
    def execute(self, dry_run=True):
        ...
        for main_cat, items in sorted(by_category.items()):
            ...
            for move in items:
                ...
                rel_source = source.relative_to(self.base_dir) if source.is_relative_to(self.base_dir) else source.name
