"""
Youtube Create Zoom Animation

This module provides functionality for youtube create zoom animation.
...
def create_zoom_animation(
    ...
    if object_data is None:
        if object_type == "emoji":
            object_data = {"emoji": "🔍", "size": CONSTANT_100}

    base_size = (
        object_data.get("size", CONSTANT_100)
        if object_type == "emoji"
        else object_data.get("font_size", 60)
    )
    ...
    for i in range(num_frames):
        ...
        if zoom_type == "in":
            ...
            if t < 0.3:
                ...
                scale = interpolate(
                    end_scale * 1.2, end_scale, (t - 0.3) / 0.7, "elastic_out"
                )
        else:
            scale = interpolate(start_scale, end_scale, t, easing)
        ...
        if object_type == "emoji":
            ...
            if add_motion_blur and abs(scale - 1.0) > 0.5:
                ...
                emoji_canvas = emoji_canvas.filter(
            ...
            emoji_cropped = emoji_canvas.crop(
            ...
            current_size = int(base_size * scale)
            current_size = max(10, min(current_size, CONSTANT_500))
            ...
            draw_text_with_outline(
                ...
                outline_color=object_data.get(
                    "outline_color", (CONSTANT_255, CONSTANT_255, CONSTANT_255)
                ),
...
def create_explosion_zoom(
    ...
    for i in range(num_frames):
        ...
        angle = t * CONSTANT_360 * 2

        frame = create_blank_frame(frame_width, frame_height, bg_color)

        current_size = int(CONSTANT_100 * scale)
        ...
        emoji_canvas = emoji_canvas.rotate(
            angle, center=(canvas_size // 2, canvas_size // 2), resample=Image.BICUBIC
        )
...
if __name__ == "__main__":
    logger.info("Creating zoom animations...")
    ...
    builder.save("zoom_in.gif", num_colors=CONSTANT_128)

    # Example 2: Explosion zoom
    builder.clear()
    frames = create_explosion_zoom(emoji="💥", num_frames=20)
    builder.add_frames(frames)
    builder.save("zoom_explosion.gif", num_colors=CONSTANT_128)

    # Example 3: Mind blown
    builder.clear()
    frames = create_mind_blown_zoom(emoji="🤯", num_frames=30)
    builder.add_frames(frames)
    builder.save("zoom_mind_blown.gif", num_colors=CONSTANT_128)

    logger.info("Created zoom animations!")
