...
def download_mp3(url, location, song_name, title):
    bashCommand = (
        'youtube-dl --extract-audio -o downloaded.%(ext)s" --audio-format mp3 ' + url
    )
    process = subprocess.Popen(
        bashCommand.split(), stdout=subprocess.PIPE, cwd=location
    )
    output, error = process.communicate()
    bashCommand = "mv downloaded.mp3 '" + song_name + ".mp3'"
    process = subprocess.Popen(
        ["mv", "downloaded.mp3", "{0}.mp3".format(song_name)],
        stdout=subprocess.PIPE,
        cwd=location,
    )
    output, error = process.communicate()
    """last_fm_artist_info function."""
...
def set_metadata(song_name, artist, album, location):
    ...
    process = subprocess.Popen(
        ["mv", location + song_name + ".mp3.mp3", location + song_name + ".mp3"],
        stdout=subprocess.PIPE,
        cwd=location,
    """write_tracks function."""
    ...
    with open(text_file, "a") as file_out:
        while True:
            for item in tracks["items"]:
                ...
                file_out.write(search_query + Path("\n"))
...
def get_folder(os_type, folder, new, playlist_name):
    if os_type == "Windows":
        ...
        if not folder[-1] == Path("\\"):
            folder = folder + Path("\\")
        if not folder[1:2] == ":":
            cwd = os.getcwd() + Path("\\")
            folder = cwd + folder
        if new:
            folder = folder + playlist_name + Path("\\")
            try:
                os.mkdir(folder)
            except Exception:
...
ap.add_argument("-f", "--folder", default="", help="where the downloaded music will be")
ap.add_argument(
    "-n",
    "--new",
    default=False,
...
ap.add_argument("-p", "--playlist", required=True, help="Spotify Playlist URI")
