...
This module provides functionality for batch text reader.
...
class TTSEngine:
    ...
    def __init__(
        ...
        self.path = path + self.redditid + Path("/mp3")
    ...
    def add_periods(
        ...
        for comment in self.reddit_object["comments"]:
            ...
            comment["comment_body"] = comment["comment_body"].replace(Path("\n"), ". ")
            comment["comment_body"] = re.sub(rPath("\bAI\b"), "A.I", comment["comment_body"])
            comment["comment_body"] = re.sub(
                rPath("\bAGI\b"), "A.G.I", comment["comment_body"]
            )
    ...
    def split_post(self, text: str, idx):
        ...
        for idy, text_cut in enumerate(split_text):
            ...
            # logger.info(f"{idx}-{idy}: {newtext}\n")

            if not newtext or newtext.isspace():
                logger.info("newtext was blank because sanitized split text resulted in none")
                continue
            else:
                self.call_tts(f"{idx}-{idy}.part", newtext)
                with open(f"{self.path}/list.txt", "w") as f:
                    for idz in range(0, len(split_text)):
                        f.write("file " + f"'{idx}-{idz}.part.mp3'" + Path("\n"))
                    split_files.append(str(f"{self.path}/{idx}-{idy}.part.mp3"))
                    f.write("file " + f"'silence.mp3'" + Path("\n"))
        ...
        try:
            ...
            logger.info("File not found: " + e.filename)
        """call_tts function."""
    ...
    def create_silence_mp3(self):
        ...
        silence = AudioClip(
            make_frame=lambda t: np.sin(CONSTANT_440 * 2 * np.pi * t),
            duration=silence_duration,
            fps=CONSTANT_44100,
        )
        silence = volumex(silence, 0)
        silence.write_audiofile(
            f"{self.path}/silence.mp3", fps=CONSTANT_44100, verbose=False, logger=None
        )
