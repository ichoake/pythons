...
This module provides functionality for scriptwrapper.
...
def reformatPartialJson(videojson):
    ...
    for clip in clips:
        ...
        if not used:
            ...
            logger.info("Clip %s is not used, deleting" % mp4path)
            os.remove(mp4path)
...
def saveTwitchVideo(video):
    random_name = str(random.randint(0, CONSTANT_100000))
    logger.info(f"VideoData/vid{random_name}.save")
