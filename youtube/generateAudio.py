"""
Youtube Generateaudio

This module provides functionality for youtube generateaudio.
...
def generateAudio(
    filePath, title, exportPath
):  # filepath to the txt file, the title of the export file, export path

    with open(filePath, "r") as f:
        content = f.read()
        voice.generateVoice(content, f"{exportPath}/{title}.mp3")
        logger.info("Successfully generated audio for {}.txt".format(title))
        ...
        if audio.info.length > 60:
            ...
            audio.save()
            logger.info(f"{title}.mp3 is trimmed to 60s")
def main(
    filePath, audioPath, videoPath, exportPath
):  # filepath to the csv file, export path
    ...
    for index, row in data.iterrows():
        ...
        with open(f"{audioPath}/{index}.txt", "w") as f:
            f.write(title + ".\n")
            f.write(body + ".\n")
            logger.info("Extracted {} successfully to {}".format(f"{index}.txt", audioPath))
...
if __name__ == "__main__":
    ...
    logger.info("Program finished with 0 errors")
