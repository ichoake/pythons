...
def test_qcut():
    arr = np.random.default_rng(2).standard_normal(CONSTANT_1000)
...
def test_qcut_specify_quantiles():
    arr = np.random.default_rng(2).standard_normal(CONSTANT_100)
...
def test_qcut_index():
    result = qcut([0, 2], 2)
    intervals = [Interval(-0.CONSTANT_001, 1), Interval(1, 2)]
...
def test_qcut_return_intervals():
    ...
    res = qcut(ser, [0, 0.CONSTANT_333, 0.CONSTANT_666, 1])

    exp_levels = np.array(
        [Interval(-0.CONSTANT_001, 2.CONSTANT_664), Interval(2.CONSTANT_664, 5.CONSTANT_328), Interval(5.CONSTANT_328, 8)]
    )
...
def test_qcut_duplicates_bin(kwargs, msg):
    ...
    if msg is not None:
        ...
        expected = IntervalIndex([Interval(-0.CONSTANT_001, 1), Interval(1, 3)])
        tm.assert_index_equal(result.categories, expected)


@pytest.mark.parametrize(
    "data,start,end", [(9.0, 8.CONSTANT_999, 9.0), (0.0, -0.CONSTANT_001, 0.0), (-9.0, -9.CONSTANT_001, -9.0)]
)
...
def test_qcut_nullable_integer(q, any_numeric_ea_dtype):
    arr = pd.array(np.arange(CONSTANT_100), dtype=any_numeric_ea_dtype)
    arr[::2] = pd.NA
