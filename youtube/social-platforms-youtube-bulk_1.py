"""
Social Platforms Youtube Bulk 1

This module provides functionality for social platforms youtube bulk 1.
...
class YouTubeBulkUpload:
    def __init__(
        ...
        upload_batch_limit=CONSTANT_100,
        ...
        self.logger.info(
            f"youtube_title_replacements: {youtube_title_replacements}, youtube_title_prefix: {youtube_title_prefix}"
        )
        self.logger.info(f"youtube_title_suffix: {youtube_title_suffix}")
        self.logger.info(
            f"thumbnail_filename_prefix: {thumbnail_filename_prefix}, thumbnail_filename_suffix: {thumbnail_filename_suffix}"
        )
        self.logger.info(
    ...
    def prompt_user_bool(self, prompt_message, allow_empty=False):
        if self.gui is not None:
            ...
            self.gui.prompt_user_bool(
                prompt_message=prompt_message, allow_empty=allow_empty
            )
    ...
    def validate_input_parameters(self):
        ...
        if self.youtube_client_secrets_file is None or not os.path.isfile(
            self.youtube_client_secrets_file
        ):
            raise Exception(
                f"YouTube client secrets file does not exist: {self.youtube_client_secrets_file}"
            )
        ...
        if self.youtube_description_template_file is None:
            ...
            if not os.path.isfile(self.youtube_description_template_file):
                raise Exception(
                    f"YouTube description file does not exist: {self.youtube_description_template_file}"
                )

            self.logger.info(
                f"YouTube description template file exists: {self.youtube_description_template_file}"
            )
        ...
        try:
            with open(self.youtube_client_secrets_file, "r", encoding="utf-8") as f:
                json.load(f)
                self.logger.info(
                    f"YouTube client secrets file is valid JSON: {self.youtube_client_secrets_file}"
                )
        except json.JSONDecodeError as e:
            raise Exception(
                f"YouTube client secrets file is not valid JSON: {self.youtube_client_secrets_file}"
            ) from e
    ...
    def get_channel_id(self):
        ...
        """check_if_video_title_exists_on_youtube_channel function."""
    ...
    def check_if_video_title_exists_on_youtube_channel(self, youtube_title):
        ...
        self.logger.info(
            f"Searching YouTube channel {channel_id} for title: {youtube_title}"
        )
        ...
        if "items" in response and len(response["items"]) > 0:
            for item in response["items"]:
                ...
                similarity_score = fuzz.ratio(
                    youtube_title.lower(), found_title.lower()
                )
                if similarity_score >= 70:  # 70% similarity
                    ...
                    if self.interactive_prompt:
                        self.logger.debug(
                            "Prompting user to confirm whether video matches existing on channel"
                        )
        ...
        """truncate_to_nearest_word function."""

                        return found_id

        self.logger.info(
            f"No matching video found with title: {youtube_title}, continuing with upload."
        )
        return None
    ...
    def truncate_to_nearest_word(self, title, max_length):
        ...
        """upload_video_to_youtube_with_title_thumbnail function."""
    ...
    def upload_video_to_youtube_with_title_thumbnail(
        ...
        self.logger.info(
            f"Uploading video {video_file} to YouTube with title, description and thumbnail..."
        )
        if self.dry_run:
            self.logger.info(
        ...
        """determine_thumbnail_filepath function."""

            if thumbnail_filepath is not None:
                media_thumbnail = MediaFileUpload(thumbnail_filepath)
                youtube.thumbnails().set(
                    videoId=youtube_video_id, media_body=media_thumbnail
                ).execute()
    ...
    def determine_thumbnail_filepath(self, video_file):
        self.logger.info(
            f"Determining thumbnail filepath for video file: {video_file}..."
        )
        ...
        if self.thumbnail_filename_replacements is not None:
            self.logger.info(
                f"Applying replacement patterns to thumbnail filename: {modified_filename}"
            )

            for pattern, replacement in self.thumbnail_filename_replacements:
                self.logger.debug(
                    f"Applying thumbnail replacement pattern: {pattern} -> {replacement}"
                )
        ...
        if self.interactive_prompt:
            self.logger.debug(
        """determine_youtube_title function."""

                "Prompting user to confirm whether happy to proceed without thumbnail"
            )
    ...
    def determine_youtube_title(self, video_file):
        ...
        if self.youtube_title_replacements is not None:
            ...
            for pattern, replacement in self.youtube_title_replacements:
                self.logger.debug(
                    f"Applying title replacement pattern: {pattern} -> {replacement}"
                )
        ...
        """determine_youtube_description function."""

                f"Are you happy with the generated title: {video_title}?"
            ):
                prompt_message = (
                    "Please type the title you would like to use (max CONSTANT_100 chars): "
                )
                video_title = self.prompt_user_text(
                    prompt_message, default_response=video_title
                )
    ...
    def determine_youtube_description(self, video_file, youtube_title):
        self.logger.info(
            f"Determining YouTube description for video file: {video_file}..."
        )

        description = ""
        if self.youtube_description_template_file is not None:
            with open(
                self.youtube_description_template_file, "r", encoding="utf-8"
            ) as file:
                description = file.read()

        if self.youtube_description_replacements is not None:
            self.logger.info(
                f"Applying replacement patterns to description text with length: {len(description)}"
            )
            ...
            for pattern, replacement in self.youtube_description_replacements:
                ...
                if "{{youtube_title}}" in replacement:
                    self.logger.debug(
                        f"Replacing youtube title template with actual title: {youtube_title}"
                    )
                ...
                self.logger.debug(
                    f"Applying description replacement pattern: {pattern} -> {replacement}"
                )
        ...
        if not description and self.interactive_prompt:
            self.logger.warning(
                f"Unable to load YouTube description from file for video file: {video_file}..."
            )
            prompt_message = "No description template file found. Please type the description you would like to use: "
            description = self.prompt_user_text(
                prompt_message, default_response=description
            )
    ...
    def process(self):
        ...
        for video_file in video_files:
            ...
            try:
                ...
                youtube_description = self.determine_youtube_description(
                    video_file, youtube_title
                )
                thumbnail_filepath = self.determine_thumbnail_filepath(video_file)

                existing_video_matching_title_id = (
                    self.check_if_video_title_exists_on_youtube_channel(youtube_title)
                )
                if existing_video_matching_title_id is not None:
                    existing_video_matching_title_url = (
                        f"{YOUTUBE_URL_PREFIX}{existing_video_matching_title_id}"
                    )
                    self.logger.warning(
                        f"Video already exists on YouTube, skipping upload: {existing_video_matching_title_url}"
                    )
                    continue

                if self.interactive_prompt:
                    self.logger.info(
                        "Interactive prompt is enabled. Confirming upload details with user."
                    )
                    confirmation_prompt = (
                    ...
                    if self.prompt_user_bool(confirmation_prompt):
                        self.logger.info(
                            "User confirmed upload details. Proceeding with upload."
                        )
                    else:
                        self.logger.info(
                ...
                self.logger.error(
                    f"Failed to upload video {video_file} to YouTube: {e}"
                )
