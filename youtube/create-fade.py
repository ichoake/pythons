"""
Youtube Create Fade Animation

This module provides functionality for youtube create fade animation.
    ...
    for i in range(num_frames):
        ...
        if object_type == "emoji":
            ...
            draw_emoji_enhanced(
                ...
                position=(
                    center_pos[0] - emoji_size // 2,
                    center_pos[1] - emoji_size // 2,
                ),
                size=emoji_size,
...
if __name__ == "__main__":
    logger.info("Creating fade animations...")
    ...
    frames = create_fade_animation(
        object_type="emoji",
        object_data={"emoji": "✨", "size": CONSTANT_120},
        num_frames=30,
        fade_type="in",
        easing="ease_out",
    )
    builder.add_frames(frames)
    builder.save("fade_in.gif", num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save("fade_crossfade.gif", num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save("fade_blink.gif", num_colors=CONSTANT_128)

    logger.info("Created fade animations!")
