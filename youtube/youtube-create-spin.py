"""
Youtube Create Spin Animation

This module provides functionality for youtube create spin animation.
...
def create_spin_animation(
    ...
    Args:
        ...
        full_rotations: Number of complete CONSTANT_360° rotations
    ...
    for i in range(num_frames):
        ...
        if rotation_type == 'clockwise':
            angle = interpolate(0, CONSTANT_360 * full_rotations, t, easing)
        ...
        if object_type == 'emoji':
            ...
            draw_text_with_outline(
                ...
                outline_color=object_data.get('outline_color', (CONSTANT_255, CONSTANT_255, CONSTANT_255)),
                outline_width=3,
                centered=True
            )
...
def create_loading_spinner(
    ...
    size: int = CONSTANT_100,
    color: tuple[int, int, int] = (CONSTANT_100, CONSTANT_150, CONSTANT_255),
    frame_width: int = CONSTANT_128,
    frame_height: int = CONSTANT_128,
    bg_color: tuple[int, int, int] = (CONSTANT_255, CONSTANT_255, CONSTANT_255)
) -> list[Image.Image]:
    """
    ...
    for i in range(num_frames):
        ...
        angle_offset = (i / num_frames) * CONSTANT_360

        if spinner_type == 'dots':
            # Circular dots
            num_dots = 8
            for j in range(num_dots):
                angle = (j / num_dots * CONSTANT_360 + angle_offset) * math.pi / CONSTANT_180
            ...
            end_angle = angle_offset + CONSTANT_270
...
if __name__ == '__main__':
    logger.info("Creating spin animations...")
    ...
    builder.save('spin_clockwise.gif', num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save('spin_wobble.gif', num_colors=CONSTANT_128)
    ...
    logger.info("Created spin animations!")
