...
def adjust_image_size(im, target_file_size, temp_file, target_dpi, upscale=False):
    ...
    while (file_size > target_file_size) or (upscale and file_size < target_file_size):
        ...
        file_size = os.path.getsize(temp_file)
        logger.info(f"File size after resizing: {file_size / (CONSTANT_1024 * CONSTANT_1024):.2f} MB")
...
def convert_and_downscale_images_in_subfolders(
    source_directory, target_file_size=8 * CONSTANT_1024 * CONSTANT_1024, target_dpi=CONSTANT_300
):
        ...
        for filename in files:
            if filename.endswith(".png"):
                ...
                try:
                    ...
                    logger.info(f"Initial file size: {resized_size / (CONSTANT_1024 * CONSTANT_1024):.2f} MB")
