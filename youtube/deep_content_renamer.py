        ...
        if analysis["actions"]:
            ...
            for action in priority_actions:
                if action in analysis["actions"]:
                    parts.append(
                        action + ("r" if action != "analyze" else "r")
                    )  # downloader, analyzer
                    break
            else:
...
class DeepContentRenamer:
    ...
    def __init__(
        self, target_dir: str, dry_run: bool = True, limit: int = CONSTANT_500
    ):
    ...
    def scan_and_analyze(self):
        ...
        logger.info(
            f"{Colors.GREEN}Analyzing {len(python_files)} Python files...{Colors.END}\n"
        )

        needs_rename = [f for f in python_files if self.should_rename(f)]

        logger.info(
            f"{Colors.YELLOW}Files needing better names: {len(needs_rename)}{Colors.END}\n"
        )
        ...
        for idx, filepath in enumerate(needs_rename[: self.limit], 1):
            if idx % 50 == 0:
                logger.info(
                    f"{Colors.YELLOW}Analyzing: {idx}/{min(len(needs_rename), self.limit)}...{Colors.END}",
                    end="\r",
                )
        ...
        logger.info(
            f"{Colors.CYAN}Files to rename: {len(self.rename_plan)}{Colors.END}"
        )
                ...
                if not self.dry_run:
                    try:
                        ...
                        if new_path.exists() and new_path != item["old_path"]:
                            ...
                            counter = 2
                            while (
                                item["old_path"].parent / f"{base}-v{counter}.py"
                            ).exists():
                                counter += 1
                            new_path = item["old_path"].parent / f"{base}-v{counter}.py"

                        # Backup
                        backup_path = self.backup_dir / item["old_path"].relative_to(
                            self.target_dir
                        )
    ...
    def run(self):
        ...
        logger.info(
            "║     Understands What Code Does Before Renaming                               ║"
        )
        logger.info(
            "║                                                                               ║"
        )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=CONSTANT_500, help="Max files to rename"
    )

    args = parser.parse_args()

    renamer = DeepContentRenamer(
        target_dir=args.target, dry_run=not args.live, limit=args.limit
    )
