...
This module provides functionality for files.
...
class FilesGalleryLogic(BaseGalleryLogic):
    ...
    def create_thumbnails(self, force=False):
        ...
        # Multiply the thumbnail size by the factor to generate larger
        # thumbnails to improve quality on retina displays
        ...
        if not photos:
            raise spg_common.SPGException(
                f'No photos could be found under {
                    self.gallery_config["images_path"]}'
            )
    ...
    def generate_images_data(self, images_data):
        ...
        for image in images:
            ...
            image_data["thumbnail_size"] = (
                round(image_data["thumbnail_size"][0] / FilesGalleryLogic.THUMBNAIL_SIZE_FACTOR),
            ...
            # If the date is filled, set the description to a non-empty string
            # so it is shown
            if image_data["date"] and not image_data["description"]:
                image_data["description"] = " "

            # Preserve the image description if the photo hasn't changed since
            # the last time
            if (
