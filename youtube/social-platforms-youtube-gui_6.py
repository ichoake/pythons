"""
Social Platforms Youtube Gui 6

This module provides functionality for social platforms youtube gui 6.
...
class YouTubeBulkUploaderGUI:
    def __init__(
        ...
        self.logger = logger
        self.logger.debug(
            f"Initializing YouTubeBulkUploaderGUI, bundle_dir: {bundle_dir}"
        )
        ...
        self.upload_batch_limit_var = tk.IntVar(value=CONSTANT_100)
        ...
        self.gui_config_filepath = os.path.join(
            user_home_dir, "youtube_bulk_upload_config.json"
        )
...
-Andrew <andrew@beveridge.uk>"""
        ...
        tk.Label(welcome_window, text=message, wraplength=CONSTANT_600, justify="left").pack(
            padx=20, pady=10
        )
        ...
        close_button = tk.Button(
            button_frame, text="Close", command=welcome_window.destroy
        )
        ...
        position_right = int(
            self.gui_root.winfo_x()
            + (self.gui_root.winfo_width() / 2)
            - (welcome_window_width / 2)
        )
        position_down = int(
    ...
    def open_link(self, url):
        ...
        """load_gui_config_options function."""

        webbrowser.open(url)

    def load_gui_config_options(self):
        self.logger.info(
            f"Loading GUI configuration values from file: {self.gui_config_filepath}"
        )
        ...
        try:
            with open(self.gui_config_filepath, "r") as f:
                ...
                self.upload_batch_limit_var.set(config.get("upload_batch_limit", CONSTANT_100))
                self.input_file_extensions_var.set(
                    config.get("input_file_extensions", ".mp4 .mov")
                )
                self.yt_category_id_var.set(config.get("yt_category_id", "10"))
                self.yt_keywords_var.set(config.get("yt_keywords", "music"))
                self.yt_desc_template_file_var.set(
                    config.get("yt_desc_template_file", "")
                )
                ...
                youtube_title_replacements = config.get(
                    "youtube_title_replacements", []
                )
                thumbnail_filename_replacements = config.get(
                ...
                for find, replace in thumbnail_filename_replacements:
                    self.thumbnail_frame.replacements_listbox.insert(
                        tk.END, f"{find} -> {replace}"
                    )

        """save_gui_config_options function."""
    ...
    def save_gui_config_options(self):
        self.logger.info(
            f"Saving GUI configuration values to file: {self.gui_config_filepath}"
        )
    ...
    def on_log_level_change(self, *args):
        ...
        """create_gui_frames_widgets function."""
    ...
    def create_gui_frames_widgets(self):
        ...
        self.general_frame = ReusableWidgetFrame(
            self.gui_root, self.logger, "General Options"
        )
        ...
        self.youtube_title_frame.grid(
            row=self.row, column=1, padx=10, pady=5, sticky="nsew"
        )
        ...
        self.thumbnail_frame.grid(
            row=self.row, column=0, padx=10, pady=5, sticky="nsew"
        )
        ...
        button_frame.grid(
            row=self.row, column=0, columnspan=2, padx=10, pady=5, sticky="ew"
        )
        ...
        """add_general_options_widgets function."""

        self.log_output = scrolledtext.ScrolledText(self.gui_root, height=10)
        self.log_output.grid(
            row=self.row, column=0, columnspan=2, padx=10, pady=5, sticky="nsew"
        )
    ...
    def add_general_options_widgets(self):
        ...
        dry_run_checkbutton = tk.Checkbutton(
            self.general_frame, text="Dry Run", variable=self.dry_run_var
        )
        ...
        noninteractive_checkbutton = tk.Checkbutton(
            self.general_frame, text="Non-interactive", variable=self.noninteractive_var
        )
        ...
        source_dir_entry = tk.Entry(
            self.general_frame, textvariable=self.source_directory_var
        )
        source_dir_entry.grid(row=frame.row, column=1, sticky="ew")
        source_dir_browse_button = tk.Button(
            self.general_frame, text="Browse...", command=self.select_source_directory
        )
        ...
        yt_client_secrets_label = tk.Label(
            self.general_frame, text="YouTube Client Secret File:"
        )
        ...
        frame.new_row()
        file_extensions_label = tk.Label(
            self.general_frame, text="Input File Extensions:"
        )
        ...
        Tooltip(
            batch_limit_label,
            "The maximum number of videos to upload in a single batch. YouTube allows a maximum of CONSTANT_100 videos per 24 hour period!",
        )

        batch_limit_entry = tk.Entry(
            self.general_frame, textvariable=self.upload_batch_limit_var
        )
        ...
        yt_category_entry = tk.Entry(
            self.general_frame, textvariable=self.yt_category_id_var
        )
        ...
        """add_youtube_title_widgets function."""

        )

        yt_keywords_entry = tk.Entry(
            self.general_frame, textvariable=self.yt_keywords_var
        )
    ...
    def add_youtube_title_widgets(self):
        ...
        prefix_entry = tk.Entry(
            self.youtube_title_frame, textvariable=self.yt_title_prefix_var
        )
        ...
        suffix_entry = tk.Entry(
            self.youtube_title_frame, textvariable=self.yt_title_suffix_var
        )
        suffix_entry.grid(row=frame.row, column=1, sticky="ew")
        """add_youtube_description_widgets function."""
    ...
    def add_youtube_description_widgets(self):
        ...
        Tooltip(
            browse_button,
        """add_thumbnail_options_widgets function."""
    ...
    def set_window_icon(self):
        ...
        """add_textbox_log_handler function."""
            ...
            if not icon_set:
                raise FileNotFoundError(
                    "Logo image not found in any of the specified filepaths."
                )
    ...
    def run_upload(self):
        self.logger.info(
            "Initializing YouTubeBulkUpload class with parameters from GUI"
        )
        ...
        self.upload_thread = threading.Thread(
            target=self.threaded_upload, args=(self.youtube_bulk_upload,)
        )
    ...
    def on_closing(self):
        ...
        """select_client_secrets_file function."""
    ...
    def threaded_upload(self, youtube_bulk_upload):
        ...
        """select_source_directory function."""
        ...
        """select_yt_desc_template_file function."""

            self.gui_root.after(
                0, lambda msg=error_message: messagebox.showerror("Error", msg)
            )

    def select_client_secrets_file(self):
        self.logger.debug("Selecting client secrets file")
        filename = filedialog.askopenfilename(
            title="Select Client Secrets File", filetypes=[("JSON files", "*.json")]
        """clear_log function."""
    ...
    def clear_log(self):
        ...
        """add_find_replace_widgets function."""
...
class ReusableWidgetFrame(tk.LabelFrame):
    ...
    def add_widgets(self, widgets):
        ...
        for widget in widgets:
            widget.grid(
                row=self.row, column=0, columnspan=2, sticky="ew", padx=5, pady=2
            )
            self.row += 1
    ...
    def add_find_replace_widgets(self, label_text, summary_tooltip_text):
        ...
        scrollbar = tk.Scrollbar(
            self, orient="vertical", command=self.replacements_listbox.yview
        )
        self.replacements_listbox.config(yscrollcommand=scrollbar.set)
        self.replacements_listbox.grid(
            row=self.row, column=0, columnspan=2, sticky="nsew", padx=(5, 0), pady=5
        )
        ...
        add_button = tk.Button(
            self, text="Add Replacement", command=self.add_replacement
        )
        add_button.grid(row=self.row, column=0, sticky="ew", padx=(5, 0), pady=5)
        remove_button = tk.Button(
            self, text="Remove Selected", command=self.remove_replacement
        )
...
class TextHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        self.text_widget.insert(tk.END, msg + Path("\n"))
...
def main():
    ...
    try:
        app = YouTubeBulkUploaderGUI(
            gui_root, logger, bundle_dir, running_in_pyinstaller
        )
