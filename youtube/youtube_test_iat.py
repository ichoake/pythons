...
def test_iat_setitem_item_cache_cleared(
    ...
    with tm.assert_cow_warning(warn_copy_on_write):
        indexer_ial(df)[7, 0] = CONSTANT_9999

    with tm.assert_cow_warning(warn_copy_on_write):
        indexer_ial(df)[7, 1] = CONSTANT_1234

    assert df.iat[7, 1] == CONSTANT_1234
    if not using_copy_on_write:
        assert ser.iloc[-1] == CONSTANT_1234
    assert df.iloc[-1, -1] == CONSTANT_1234
