...
class ClipCompilationCreator:
    def __init__(self, game: str, asset_path: str, output_path: str):
        ...
        self.target_res = (CONSTANT_1920, CONSTANT_1080)
        self.compressor_settings = dict(
            threshold="-12dB", ratio=9, attack=CONSTANT_200, release=CONSTANT_1000
        )
    ...
    def write_clips(self):
        ...
        for idx, (clip_id, clip) in enumerate(composite_clips.items()):
            ...
            raw_audio_path = os.path.join(
                self.compilation_dir, f"clip_{clip_id}.base.mp4"
            )
    ...
    def composite_clips(self, clips: dict):
        ...
        for clip_id, clip in clips.items():
            ...
            composite_clips[clip_id] = CompositeVideoClip(
                composition, size=self.target_res
            )
    ...
    def load_clips(self) -> dict:
        ...
        for data in self.metadata["clips"]:
            ...
            if os.path.isfile(clip_path):
                ...
                try:
                    clips[data["clip_id"]] = VideoFileClip(
                        clip_path, target_resolution=clip_res
                    ).afx(afx.audio_normalize)
                except ZeroDivisionError:
                    logging.warning(
                        "Can't normalize clip because it has no Audio -> will be added without audio"
                    )
                    clips[data["clip_id"]] = VideoFileClip(
                        clip_path, target_resolution=clip_res
                    )
