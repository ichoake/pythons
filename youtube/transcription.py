...
class TranscriptionAnalyzer:
    ...
    def analyze_transcript(self, transcript: str, language: str = "English") -> Dict[str, Any]:
    ...
    def _process_short_file(self, input_file: Path, audio_path: Path, dirs: dict) -> bool:
        ...
        try:
            ...
            transcript_file = dirs["transcripts_dir"] / f"{input_file.stem}_transcript.txt"
            timestamped_file = dirs["transcripts_dir"] / f"{input_file.stem}_timestamped.txt"
            ...
            analysis = self.analyze_transcript(
                transcription_result["full_transcript"], transcription_result.get("language", "English")
            )
            ...
            self._create_summary_file(input_file, audio_path, dirs, transcription_result, analysis, False)
    ...
    def _process_long_file(self, input_file: Path, audio_path: Path, dirs: dict) -> bool:
        ...
        try:
            ...
            for i, chunk_file in enumerate(chunk_files, 1):
                logger.info(f"Processing chunk {i}/{len(chunk_files)}: {Path(chunk_file).name}")
            ...
            merged_transcript = self.chunker.merge_transcripts(chunk_transcripts, chunk_files)
            ...
            analysis = self.analyze_transcript(
                merged_transcript["full_transcript"], merged_transcript.get("language", "English")
            )
            ...
            # Create summary report
            self._create_summary_file(input_file, audio_path, dirs, merged_transcript, analysis, True, len(chunk_files))

            # Clean up chunk files
            self.chunker.cleanup_chunks(chunk_files)

            logger.info(f"Successfully processed {input_file.name} ({len(chunk_files)} chunks)")
            logger.info(f"Output directory: {dirs['base_dir']}")
            return True
    ...
    def _create_summary_file(
        ...
        try:
            ...
            with open(summary_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"Language: {transcription_result.get('language', 'Unknown')}\n")

                if was_chunked:
                    f.write(f"Processing: Chunked into {chunk_count} pieces\n")
                    f.write(f"Max Chunk Duration: {config.MAX_CHUNK_DURATION_MINUTES} minutes\n")
                    f.write(f"Chunk Overlap: {config.CHUNK_OVERLAP_SECONDS} seconds\n")

                if "total_duration" in transcription_result:
                    f.write(f"Duration: {self._format_timestamp(transcription_result['total_duration'])}\n")
                elif "segments" in transcription_result and transcription_result["segments"]:
                    f.write(f"Duration: {self._format_timestamp(transcription_result['segments'][-1]['end'])}\n")
                ...
                if was_chunked and isinstance(analysis, dict) and "chunking_info" in analysis:
                    f.write(f"\nChunking Details:\n")
                    f.write(f"- Original file was split into {chunk_count} chunks\n")
                    f.write(f"- Each chunk max {config.MAX_CHUNK_DURATION_MINUTES} minutes\n")
                    f.write(f"- {config.CHUNK_OVERLAP_SECONDS}s overlap between chunks\n")
