    ...
    for i in range(num_frames):
        ...
        if object_type == "emoji":
            ...
            draw_text_with_outline(
                ...
                outline_color=current_object.get(
            ...
            if flip_axis == "horizontal":
                ...
                text_cropped = text_scaled.crop(
                    (left, top, left + min(new_width, frame_width), top + frame_height)
                )
            else:
                left = (canvas_size - frame_width) // 2
                top = (
                    (new_height - frame_height) // 2 if new_height > frame_height else 0
                )
                paste_x = 0
                paste_y = center_pos[1] - min(new_height, frame_height) // 2

                text_cropped = text_scaled.crop(
                    (left, top, left + frame_width, top + min(new_height, frame_height))
                )
...
if __name__ == "__main__":
    ...
    frames = create_flip_animation(
        object1_data={
            "text": "YES",
            "font_size": 80,
            "text_color": (CONSTANT_100, CONSTANT_200, CONSTANT_100),
        },
        object2_data={
            "text": "NO",
            "font_size": 80,
            "text_color": (CONSTANT_200, CONSTANT_100, CONSTANT_100),
        },
        num_frames=30,
        flip_axis="vertical",
        object_type="text",
