        ...
        for file_path in documents_path.rglob(f'*{ext}'):
            ...
            try:
                ...
                out_of_place_files.append({
                    ...
                    'file_size_mb': round(file_size / (CONSTANT_1024 * CONSTANT_1024), 2),
...
def create_csv_report(out_of_place_files):
    ...
    csv_filename = f'/Users/steven/Documents/python/out_of_place_files_report_{timestamp}.csv'
    ...
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        sorted_files = sorted(out_of_place_files, key=lambda x: (x['file_extension'], x['current_location']))
...
def print_summary(out_of_place_files):
    ...
    largest_files = sorted(out_of_place_files, key=lambda x: x['file_size_bytes'], reverse=True)[:10]
    for i, file_info in enumerate(largest_files, 1):
        logger.info(f"  {i:2d}. {file_info['file_name']} ({file_info['file_size_mb']} MB) - {file_info['current_location']}")
...
def main():
    ...
    if not out_of_place_files:
        logger.info("✅ No out-of-place files found! Your Documents directory is well organized.")
        return
