...
This module provides functionality for giphy downloader v3.
...
def transcribe_audio(config_file="config.yaml"):
    ...
    # Load the model
    model = whisper.load_model(config["model"]["name"], device=config["model"]["device"])
...
def write_srt_with_word_timestamps(
    ...
    for segment in result["segments"]:
        for word in segment["words"]:
            ...
            if (
                (max_line_width and len(current_line) + len(word["word"]) + 1 > max_line_width)
