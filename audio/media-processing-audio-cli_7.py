...
logger = logging.getLogger(__name__)
    ...
    RedditScrape  # Importing reddit scraping class to acquire posts and authors
from TextToSpeech import \
    TextToSpeech  # Importing tts class to make mp3 of posts
from VideoEdit import \
    VideoEditor  # Edits all the tts mp3 and Images into a mp4 video
...
def main() -> int:
    ...
    try:
        ...
        for entry in lines:
            data = entry.split(
                " "
            )  # split string at spaces... file format is: link num_comments title
            input_metadata.append(
                {"url": data[0], "n_entries": int(data[1]), "title": data[2]}
            )

    except FileNotFoundError:
        logger.info("Invalid path to file")
        return 1
    ...
    for video_meta in input_metadata:
        ...
        try:
            ...
            print(
                f"""Something went wrong in the Reddit Scrape...
