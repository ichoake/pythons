...
This module provides functionality for script 54.
...
class AdvancedDemoGenerator:
    ...
    def create_advanced_audio(self, text, emotion, theme, output_path, duration=CONSTANT_25000):
        """
        ...
        logger.info(f"🎵 Created {emotion} {theme} audio: {os.path.basename(output_path)}")
    ...
    def create_base_pattern(self, profile, duration):
        ...
        # Create base tone
        base_tone = Sine(base_freq).to_audio_segment(duration=min(CONSTANT_2000, duration))

        # Add harmonics
        for i, harmonic_freq in enumerate(harmonics):
            harmonic = Sine(harmonic_freq).to_audio_segment(duration=min(CONSTANT_1500, duration))
    ...
    def apply_flowing_rhythm(self, audio, duration):
        ...
        for i in range(0, duration, CONSTANT_800):
            wave = Sine(CONSTANT_440 + i // CONSTANT_100).to_audio_segment(duration=CONSTANT_400)
            wave = wave - 20
    ...
    def apply_sustained_rhythm(self, audio, duration):
        ...
        if len(audio) < duration:
            extension = Sine(CONSTANT_440).to_audio_segment(duration=duration - len(audio))
    ...
    def apply_playful_rhythm(self, audio, duration):
        ...
        for i in range(0, duration, CONSTANT_600):
            playful = Sine(CONSTANT_550 + random.randint(-50, 50)).to_audio_segment(duration=CONSTANT_200)
            playful = playful - 12
    ...
    def apply_theme_modifications(self, audio, theme, duration):
        ...
        if theme == "motivational":
            ...
            for i in range(0, duration, CONSTANT_3000):
                sweep = Sine(CONSTANT_440 + i // CONSTANT_100).to_audio_segment(duration=CONSTANT_1000)
                sweep = sweep - 15
            ...
            for i in range(0, duration, CONSTANT_2500):
                story = Sine(CONSTANT_330 + i // CONSTANT_200).to_audio_segment(duration=CONSTANT_800)
                story = story - 20
            ...
            for i in range(0, duration, CONSTANT_1500):
                adventure = Sine(CONSTANT_660 + random.randint(-CONSTANT_100, CONSTANT_100)).to_audio_segment(
                    duration=CONSTANT_300
                )
            ...
            generated_files.append(
                {
                    ...
                    "emotion": emotion,
                    "theme": theme,
                    "text": text[:CONSTANT_100] + "..." if len(text) > CONSTANT_100 else text,
                }
            )
