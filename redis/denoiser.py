...
This module provides functionality for denoiser.
...
Copyright (C) CONSTANT_2018, Matias Tassano <matias.tassano@parisdescartes.fr>
...
class FFDNetDenoiser:
    def __init__(self, _device, _sigma=25, _weights_dir="denoising/models/", _in_ch=3):
        ...
        self.sigma = _sigma / CONSTANT_255
    ...
    def load_weights(self):
        ...
        """get_denoised_image function."""

    def get_denoised_image(self, imorig, sigma=None):

        if sigma is not None:
            cur_sigma = sigma / CONSTANT_255
        else:
        ...
        if max(imorig.shape[0], imorig.shape[1]) > CONSTANT_1200:
            ratio = max(imorig.shape[0], imorig.shape[1]) / CONSTANT_1200
            imorig = cv2.resize(
                imorig,
        ...
        if sh_im[2] % 2 == 1:
            expanded_h = True
            imorig = np.concatenate(
                (imorig, imorig[:, :, -1, :][:, :, np.newaxis, :]), axis=2
            )

        if sh_im[3] % 2 == 1:
            expanded_w = True
            imorig = np.concatenate(
                (imorig, imorig[:, :, :, -1][:, :, :, np.newaxis]), axis=3
            )
