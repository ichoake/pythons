...
class IntelligentRenamer:
    ...
    def extract_keywords(self, analysis: Dict, content: str) -> List[str]:
        ...
        content_types = ["image", "video", "audio", "text", "file", "data", "mp3", "mp4", "csv", "json", "pdf"]
...
"""
        ...
        try:
            ...
            response = openai.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert at naming Python files based on their functionality. Always respond with valid JSON.",
                    },
    ...
    def _query_anthropic(self, prompt: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = client.messages.create(
                model="claude-3-haiku-20240307", max_tokens=CONSTANT_300, messages=[{"role": "user", "content": prompt}]
            )
    ...
    def generate_pattern_name(self, analysis: Dict) -> str:
        ...
        if len(original) >= 15 and ("_" in original or "-" in original):
            ...
            if not any(
                bad in original for bad in ["temp", "test", "old", "backup", "copy", "2", "-.py", "..py", "untitled"]
            ):
    ...
    def clean_filename(self, name: str) -> str:
        ...
        redundant_words = ["enhanced_", "simple_", "comprehensive_", "fixed_", "direct_", "_read_", "_deep_read"]
    ...
    def scan_and_analyze(self) -> List[Dict]:
        ...
        for idx, filepath in enumerate(python_files, 1):
            ...
            if analysis["suggested_name"] and analysis["suggested_name"] != analysis["original_name"]:
    ...
    def create_rename_plan(self, analyses: List[Dict]) -> None:
        ...
        for analysis in analyses:
            ...
            if any(bad in new_name for bad in ["usr-bin", "import-", "from-", "for-the"]):
                ...
                logger.info(
                    f"{Colors.RED}❌ BAD AUTO-NAME: {new_name} → keeping original {analysis['original_name']}{Colors.END}"
                )
                continue
        ...
        for idx, item in enumerate(sorted_plan, 1):
            ...
            try:
                if not self.dry_run:
                    ...
                    self.undo_commands.append(f"mv '{item['new_path']}' '{item['old_path']}'")
            ...
            json.dump(
                {
                    ...
                    "stats": self.stats,
                    "rename_plan": [
                        {**item, "old_path": str(item["old_path"]), "new_path": str(item["new_path"])}
                        for item in self.rename_plan
                    ],
                },
                f,
    ...
    def run(self) -> None:
        ...
        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}{Emojis.WARNING} This was a DRY RUN. Use --live to actually rename files.{Colors.END}\n"
            )
        else:
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", default=True, help="Dry run mode (default, safe)")
    parser.add_argument("--live", action="store_true", help="Live mode (actually renames files)")
    parser.add_argument(
        "--interactive", action="store_true", default=True, help="Interactive mode (asks for confirmation)"
    )
    parser.add_argument("--batch", action="store_true", help="Batch mode (no confirmation)")
    parser.add_argument("--no-ai", action="store_true", help="Disable AI suggestions (pattern-based only)")
    parser.add_argument(
        "--categorize", action="store_true", help="Move files to category folders (01_core_tools/, etc.)"
    )
    ...
    renamer = IntelligentRenamer(
        target_dir=args.target, dry_run=dry_run, interactive=interactive, use_ai=use_ai, categorize=args.categorize
    )
