"""
Leonardo Ai Generation Downloader

This module provides functionality for leonardo ai generation downloader.
    ...
    while True:
        ...
        for generation in tqdm(generations, desc=f"Processing Batch {file_index}"):
            ...
            for image in gen_images:
                ...
                if INCLUDE_IMAGES:
                    if image_url:
                        image_path = os.path.join(OUTPUT_DIR, f"{gen_id}_{image.get('id')}.jpg")
                        if download_image(image_url, image_path):
                            gen_metadata["images"].append({"image_url": image_url, "local_path": image_path})

                    if motion_url:
                        motion_path = os.path.join(OUTPUT_DIR, f"{gen_id}_{image.get('id')}.mp4")
                        if download_image(motion_url, motion_path):
                            gen_metadata["images"].append({"motion_url": motion_url, "local_path": motion_path})
