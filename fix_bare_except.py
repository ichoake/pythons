...
class BareExceptFixer:
        ...
        for idx, filepath in enumerate(python_files, 1):
            ...
            if bare_excepts:
                ...
                for except_info in bare_excepts:
                    logger.info(
                        f"   Line {except_info['line']}: {Colors.YELLOW}{except_info['original_line'].strip()}{Colors.END}"
                    )
                    logger.info(
                        f"   → Fix: {Colors.GREEN}except {except_info['suggested']}:{Colors.END}"
                    )
    ...
    def generate_report(self):
        ...
        with open(report_file, "w") as f:
            ...
            for context, count in sorted(
                by_context.items(), key=lambda x: x[1], reverse=True
            ):
        ...
        with open(csv_file, "w") as f:
            ...
            for fix in self.fixes:
                f.write(
                    f'"{fix["file"]}",{fix["line"]},"{fix["original"]}","{fix["fixed"]}",{fix["context"]},{fix["suggested"]}\n'
                )
    ...
    def run(self):
        ...
        logger.info(
            "║              Intelligent Exception Handling Improvement                      ║"
        )
        logger.info(
            "║                                                                               ║"
        )
        ...
        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}🛡️ MODE: DRY RUN (no files will be modified){Colors.END}\n"
            )
        else:
            logger.info(
                f"{Colors.RED}⚠️ MODE: LIVE FIX (files will be modified!){Colors.END}\n"
            )
        ...
        logger.info(
            f"  Files Scanned: {Colors.CYAN}{self.stats['files_scanned']:,}{Colors.END}"
        )
        logger.info(
            f"  Bare Excepts: {Colors.CYAN}{self.stats['bare_excepts_found']:,}{Colors.END}"
        )
        logger.info(
            f"  Files Fixed: {Colors.CYAN}{self.stats['files_fixed']:,}{Colors.END}"
        )
        logger.info(
            f"  Fixes Applied: {Colors.CYAN}{self.stats['fixes_applied']:,}{Colors.END}\n"
        )

        if self.dry_run:
            logger.info(
                f"{Colors.YELLOW}⚠️ DRY RUN complete. Use --live to apply fixes.{Colors.END}\n"
            )
        else:
            logger.info(
                f"{Colors.GREEN}✅ Fixes applied! Backups in: {self.backup_dir}{Colors.END}\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", default=True, help="Dry run (safe)"
    )
