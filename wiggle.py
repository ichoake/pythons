...
This module provides functionality for wiggle.
...
def create_wiggle_animation(
    ...
    for i in range(num_frames):
        ...
        if wiggle_type == "jello":
            ...
            rotation = (
                (math.sin(freq1 * t) * 10 + math.cos(freq2 * t) * 5) * intensity * decay
            )
            ...
            if abs(scale_x - scale_y) > 0.01 or abs(rotation) > 0.1:
                ...
                if abs(rotation) > 0.1:
                    emoji_canvas = emoji_canvas.rotate(
                        rotation, resample=Image.BICUBIC, expand=False
                    )
...
def create_excited_wiggle(
    emoji: str = "🎉", num_frames: int = 20, frame_size: int = CONSTANT_128
) -> list[Image.Image]:
    """
...
if __name__ == "__main__":
    logger.info("Creating wiggle animations...")
    ...
    builder.save("wiggle_jello.gif", num_colors=CONSTANT_128)
    ...
    builder.add_frames(frames)
    builder.save("wiggle_wave.gif", num_colors=CONSTANT_128)
    ...
    logger.info("Created wiggle animations!")
