"""
Media Processing Image Aws Sqs Queue Handler V2

This module provides functionality for media processing image aws sqs queue handler v2.
...
def resize_image(im, output_path):
    ...
    logger.info(f"🔄 Resizing to: {new_width}x{new_height} for aspect ratio {closest_ratio}")
...
def process_batch(batch, root, csv_rows):
    for file in batch:
        ...
        try:
            ...
            creation_date = datetime.fromtimestamp(os.path.getctime(new_file_path)).strftime(
                "%m-%d-%y"
            )
            csv_rows.append(
                [
