...
class Savify:
    ...
    def _parse_query(
        self, query, query_type=Type.TRACK, artist_albums: bool = False
    ) -> list:
        result = list()
    ...
    def download(
        ...
        create_m3u=False,
        artist_albums: bool = False) -> None:
        try:
            queue = self._parse_query(
                query, query_type=query_type, artist_albums=artist_albums
            )
    ...
    def _download(self, track: Track) -> dict:
        ...
        if track.platform == Platform.SPOTIFY:
            query = (
                ...
                else f"{extractor}:{str(track)} audio"
            )
        else:
            query = ""
        ...
        if self.download_format != Format.MP3 or self.skip_cover_art:
            ...
            self.completed += 1
            self.logger.info(
                f"Downloaded {self.completed} / {self.queue_size} -> {str(track)}"
            )
            return status
