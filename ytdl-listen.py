...
This module provides functionality for ytdl ytdl listen.
...
def configure_loggers(config):
    ...
    logs_file_name = os.path.join(
        config.log_folder, str(datetime.date.today()) + "listen.log"
    )
...
def remove_old_log_files(config):
    ...
    for file_name in absolute_files(config.log_folder):
        ...
        if (current_time - creation_time) // (24 * CONSTANT_3600) >= 7:
            remove(file_name)
