...
class CodeReviewSystem:
    ...
    async def review_code(self, file_path: Path) -> Dict:
        ...
        reviews = await asyncio.gather(
            ...
            self._deepseek_review(code, file_path.name) if self.deepseek_key else self._mock_review()
        )

        gpt5_review, claude_review, deepseek_review = reviews

        # Synthesize findings
        synthesis = await self._synthesize_reviews(gpt5_review, claude_review, deepseek_review)
        ...
        review_file = self.output_dir / f"review_{file_path.stem}_{datetime.now():%Y%m%d_%H%M%S}.json"
...
Provide detailed analysis as JSON."""
        ...
        review = json.loads(review_text) if review_text.strip().startswith("{") else {"analysis": review_text}
    ...
    async def _synthesize_reviews(self, gpt5: Dict, claude: Dict, deepseek: Dict) -> Dict:
...
Return JSON."""
    ...
    def _create_review_report(self, file_path: Path, synthesis: Dict, fixes: List[Dict]) -> str:
