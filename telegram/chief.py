...
"""
    ...
    async def _synthesize_voice_briefing(self, briefing: str) -> Optional[Path]:
        ...
        try:
            ...
            if response.status_code == CONSTANT_200:
                audio_file = self.data_dir / f"briefing_{datetime.now():%Y%m%d_%H%M%S}.mp3"
...
Return JSON."""
    ...
    async def _suggest_priorities(self, tasks: List[Task]) -> List[str]:
        ...
        tasks_text = Path("\n").join([f"- {t.title} (due: {t.due_date}, priority: {t.priority})"
                                for t in tasks[:10]])
...
Return brief recommendations (one sentence each)."""
        ...
        suggestions = [s.strip() for s in suggestions_text.split(Path("\n")) if s.strip()]
