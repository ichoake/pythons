...
class OrganizationExecutor:
    ...
    def execute_organization(self, start_with_immediate=True, max_files=None):
        ...
        if start_with_immediate:
            ...
            files_to_move = [
                f for f in self.organization_plan if f["immediate_action"] == "True"
            ]
    ...
    def save_execution_report(self):
        ...
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Space Freed: {self.stats['space_freed_mb']/CONSTANT_1024:.2f} GB\n\n"
            )
            ...
            if self.moved_files:
                ...
                for file_info in self.moved_files[:10]:  # First 10
                    f.write(
                        f"{file_info['file_name']} → {file_info['destination_path']}\n"
                    )
...
def main():
    ...
    if not csv_files:
        logger.info(
            "❌ No organization plan CSV found. Please run smart_organization_plan.py first."
        )
        return
