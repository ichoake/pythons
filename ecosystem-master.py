...
class EcosystemMasterMapper:
    ...
    def map_entire_ecosystem(self, root_path, max_depth=6):
        ...
        logger.info(
            f"   Found {len(all_python_files)} Python files across {len(project_directories)} categories"
        )
        ...
        reorganization_plan = self._generate_reorganization_plan(
            all_python_files, project_directories
        )
    ...
    def _analyze_file_content(self, file_info):
        ...
        suggested_name, suggested_path = self._generate_intelligent_name_and_path(
            file_info
        )
    ...
    def _find_project_relationships(self, all_files):
        ...
        logger.info(
            f"   Found relationships in {len(self.project_relationships)} project categories"
        )
    ...
    def _find_duplicates_and_consolidation(self, all_files):
        ...
        for i, file1 in enumerate(all_files):
            for j, file2 in enumerate(all_files[i + 1 :], i + 1):
                ...
                similarity = difflib.SequenceMatcher(
                    None, file1["content"], file2["content"]
                ).ratio()
        ...
        logger.info(
            f"   Found {len(self.consolidation_opportunities)} consolidation opportunities"
        )
...
def main():
    ...
    all_files, project_directories, reorganization_plan = mapper.map_entire_ecosystem(
        "~/Documents/python", max_depth=6
    )
