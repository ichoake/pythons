...
class AutoOrganizeDryRun:
    ...
    def analyze_file_content(self, file_path):
        ...
        try:
            ...
            title = filename.replace(".html", "").replace(".markdown", "").replace(".md", "")
            ...
            category_keywords = {
                "seo_analysis": ["seo", "keyword", "trending", "optimization", "ranking", "search", "google"],
                "creative_automation": ["creative", "automation", "ai", "generation", "art", "design", "content"],
                "brand_strategy": ["brand", "strategy", "marketing", "positioning", "identity", "logo"],
                "technical_docs": ["technical", "documentation", "api", "code", "development", "programming"],
                "content_creation": ["content", "writing", "blog", "article", "copy", "text"],
                "business_analysis": ["business", "analysis", "strategy", "plan", "report", "metrics"],
                "conversations": ["conversation", "chat", "discussion", "analysis", "cursor-agent"],
            }
            ...
            # Get top category
            suggested_category = max(category_scores.items(), key=lambda x: x[1])[0] if category_scores else "general"
            ...
            return {
                ...
                "content_preview": content[:CONSTANT_200] + "..." if len(content) > CONSTANT_200 else content,
            }

        except Exception as e:
            return {"filename": file_path.name, "error": str(e), "suggested_category": "general", "suggested_tags": []}
    ...
    def analyze_directory_structure(self):
        ...
        for project_dir in (self.base_dir / "Projects").iterdir():
            if project_dir.is_dir():
                ...
                structure_analysis["current_structure"][f"Projects/{project_dir.name}"] = file_count
        ...
        if structure_analysis["current_structure"].get("Projects/cursor-agent-analysis", 0) > 5:
            structure_analysis["suggestions"].append(
                {
    ...
    def run_dry_run(self):
        ...
        for file_analysis in all_files:
            if "error" not in file_analysis:
                ...
                logger.info(f"   📂 Suggested Category: {file_analysis['suggested_category']}")
                logger.info(f"   🏷️  Suggested Tags: {', '.join(file_analysis['suggested_tags'])}")
                logger.info(f"   📏 File Size: {file_analysis['file_size']:,} characters")
        ...
        for file_analysis in all_files:
            if "error" not in file_analysis:
                category_groups[file_analysis["suggested_category"]].append(file_analysis)
            ...
            if len(files) > 0:
                ...
                for file_analysis in files:
                    ...
                    if file_analysis["suggested_tags"]:
                        logger.info(f"      Tags: {', '.join(file_analysis['suggested_tags'])}")
                    print()
