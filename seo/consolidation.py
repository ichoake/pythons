...
class FinalConsolidator:
    def __init__(self, destination_dir=Path("/Users/steven/tehSiTes/DrAdu-SEO-OPTIMIZED")):
        """__init__ function."""
    ...
    def merge_files(self, files, output_path):
        ...
        try:
            ...
            merged_content = Path("\n").join(file_sources) + Path("\n\n") + Path("\n").join(sorted(all_content))

            # Write merged file
            with open(output_path, "w", encoding="utf-8") as f:
                f.write(merged_content)

            self.merged_files.append({"output": output_path, "sources": files, "lines": len(all_content)})
    ...
    def consolidate_files(self, file_list, dry_run=True):
        ...
        for group_name, files in self.file_groups.items():
            ...
            # Get existing files
            existing_files = [f for f in files if os.path.exists(f) and not os.path.isdir(f)]
                ...
                if not dry_run:
                    try:
                        ...
                        self.consolidated_files.append(
                            {"source": source_file, "destination": dest_file, "action": "copied"}
                        )
                ...
                for file_path in existing_files:
                    ...
                    if file_info and file_info["hash"]:
                        if file_info["hash"] not in file_hashes:
                            ...
                            logger.info(f"  Duplicate found: {file_path} (same as {file_hashes[file_info['hash']]})")
                ...
                if len(unique_files) == 1:
                    ...
                    if not dry_run:
                        try:
                            ...
                            logger.info(f"  Copied (duplicate removed): {source_file} -> {dest_file}")
                        except Exception as e:
                            self.errors.append(f"Error copying {source_file}: {e}")
                    else:
                        logger.info(f"  Would copy (duplicate removed): {source_file} -> {dest_file}")
                else:
                    ...
                    if not dry_run:
                        if self.merge_files(unique_files, dest_file):
                            self.consolidated_files.append(
                                {"source": unique_files, "destination": dest_file, "action": "merged"}
                            )
                            logger.info(f"  Merged {len(unique_files)} files -> {dest_file}")
                    else:
                        logger.info(f"  Would merge {len(unique_files)} files -> {dest_file}")
            ...
            report["duplicates"].append(
                {
                    ...
                    "modification_time": datetime.fromtimestamp(dup["keep"]["mtime"]).isoformat(),
...
def parse_complete_file_list():
    ...
    file_list = [
        ...
        "/Users/steven/tehSiTes/02_Creative_Portfolio/AvaTarArTs/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/4.png",
        "/Users/steven/tehSiTes/03_Business_Platforms/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/4.png",
        "/Users/steven/tehSiTes/03_Business_Platforms/seo-professional-portfolio/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/02_Creative_Portfolio/AvaTarArTs/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/02_Creative_Portfolio/AvaTarArTs/dr/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/03_Business_Platforms/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/02_Creative_Portfolio/AvaTarArTs/seo-professional-portfolio/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/06_Archives_Backups/dr/Dr Adu-Upscale Image/5.png",
        "/Users/steven/tehSiTes/02_Creative_Portfolio/AvaTarArTs/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/6.png",
        "/Users/steven/tehSiTes/03_Business_Platforms/Dr_Adu_GainesvillePFS_SEO_Project/Dr Adu-Upscale Image/6.png",
    ]
...
def main():
    ...
    # Save dry-run report
    with open("/Users/steven/final_consolidation_dry_run_report.json", "w") as f:
    ...
    if report["consolidated_files"]:
        ...
        for item in report["consolidated_files"]:
            if isinstance(item["source"], list):
                logger.info(f"  • {os.path.basename(item['destination'])} (merged from {len(item['source'])} files)")
            else:
                logger.info(f"  • {os.path.basename(item['destination'])} ({item['action']})")
    ...
    # Save final report
    with open("/Users/steven/final_consolidation_report.json", "w") as f:
