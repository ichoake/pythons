...
class TimestampedFileFixer:
    ...
    def suggest_better_name(self, filepath: Path) -> Tuple[str, str]:
        ...
        while (
            filepath.parent / suggested_name
        ).exists() and suggested_name != current_name:
            name_part = original_suggested.rsplit(".", 1)[0]
            ext_part = (
                original_suggested.rsplit(".", 1)[1]
                if "." in original_suggested
                else ""
            )
            suggested_name = (
...
def main():
    ...
    parser.add_argument(
        "path", type=Path, nargs="?", default=Path.cwd(), help="Directory to scan"
    )
    parser.add_argument(
