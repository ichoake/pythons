...
def analyze_directory_structure():
    ...
    subdirs = [
        d for d in CLAUDE_DIR.iterdir() if d.is_dir() and not d.name.startswith(".")
    ]
    ...
    for subdir in sorted(subdirs):
        ...
        most_recent_date = (
            datetime.fromtimestamp(most_recent).strftime("%Y-%m-%d")
            if most_recent
            else "N/A"
        )
    ...
    for stat in sorted(dir_stats, key=lambda x: -x["size_mb"]):
        logger.info(f"\n📁 {stat['name']}")
        logger.info(
            f"   Files: {stat['files']} | Dirs: {stat['dirs']} | Size: {stat['size_mb']:.2f} MB"
        )
...
def identify_duplicates():
    ...
    files_to_check = [
        f for f in all_files if f.is_file() and not f.name.startswith(".")
    ]
    ...
    if duplicates:
        ...
        for file_hash, files in sorted(
            ...
            logger.info(
                f"📄 {files[0].name} ({size / CONSTANT_1024:.1f} KB) - {len(files)} copies"
            )
            for f in files:
        ...
        logger.info(
            f"💾 Total wasted space: {total_wasted / (CONSTANT_1024 * CONSTANT_1024):.2f} MB"
        )
    else:
...
def analyze_file_types():
    ...
    all_files = [
        f for f in CLAUDE_DIR.rglob("*") if f.is_file() and not f.name.startswith(".")
    ]
    ...
    for ext, stats in sorted_exts[:15]:
        ...
        logger.info(
            f"  {ext:20s} {count:4d} files  {size_mb:8.2f} MB  (avg: {avg_size_kb:.1f} KB)"
        )
...
def analyze_zip_files():
    ...
    for zip_file in zip_files:
        ...
        corresponding_dirs = [
            ...
            if d.is_dir() and base_name.lower() in d.name.lower()
        ]
