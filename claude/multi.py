...
class MultiAPITTSGenerator:
    ...
    def get_fallback_analysis(self, text: str) -> Dict:
        ...
        if any(
            word in text_lower
            for word in ["divine", "soul", "spirit", "mystical", "eternal"]
        ):
            tone = "mystical"
        elif any(
            word in text_lower
            for word in ["power", "strength", "master", "command", "law"]
        ):
            tone = "authoritative"
        elif any(
            word in text_lower
            for word in ["joy", "happiness", "peace", "love", "beautiful"]
        ):
            tone = "inspiring"
        elif any(
            word in text_lower
            for word in ["wisdom", "truth", "understanding", "knowledge"]
        ):
            tone = "wise"
    ...
    def generate_audio_with_local_tts(
        self, text: str, voice_type: str
    ) -> Optional[bytes]:
    ...
    def generate_audio_segment(
        self, text: str, filename: str, voice_type: str = "narrator"
    ) -> Optional[Path]:
    ...
    def process_chapter(self, chapter_name: str, chapter_text: str) -> List[Path]:
        ...
        for i, segment in enumerate(segments):
            ...
            # Create filename
            segment_filename = (
                f"{chapter_name.lower().replace(' ', '-')}-segment-{i+1:02d}.mp3"
            )

            # Generate audio
            audio_path = self.generate_audio_segment(
                segment, segment_filename, analysis["primary_tone"]
            )
...
"So You will be what you will to be; Let failure find its false content In that poor word, 'environment,' But spirit scorns it, and is free. It masters time, it conquers space; It cowes that boastful trickster, Chance, And bids the tyrant Circumstance Uncrown, and fill a servant's place. The human Will, that force unseen, The offspring of a deathless Soul, Can hew a way to any goal, Though walls of granite intervene. Be not impatient in delays But wait as one who understands; When spirit rises and commands The gods are ready to obey." """
        ...
        return self.process_chapter(
            "03-Effect-of-Thought-on-Circumstances", chapter_text
        )
    ...
    def generate_complete_audiobook(self) -> bool:
        ...
        logger.info(
            f"💾 Total size: {sum(f['size'] for f in self.generated_files):,} bytes"
        )
        ...
        for file_info in self.generated_files:
            logger.info(
                f"  - {file_info['filename']} ({file_info['voice_type']}) - {file_info['size']:,} bytes"
            )
